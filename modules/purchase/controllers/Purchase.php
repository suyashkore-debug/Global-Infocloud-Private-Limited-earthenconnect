...
//------------end new dashboard ------------------------------------		
		...
		public function AddEditVendor($id = '')
		{
			...
			$data['VendorType'] = $this->purchase_model->GetVendorType();
			$data['itemlist'] = $this->purchase_model->GetItemList();
			$data['Tdssection'] = $this->purchase_model->get_tds_sections();
			// echo "<pre>";print_r($data['table_data']);die;
		...
		public function SaveVendor()
		{
			...
			// $prefix = "V";
			// $next_cust_numberval = (int) get_option('next_vendor_number');
			...
			$DisData = json_decode($this->input->post('DisData'), true);
			$Clientdata = array(
			...
			'SubActGroupID'=> $this->input->post('vendor_type'),
			'state'=>$this->input->post('state'),
			...
			'contact_person_mobile2'=>$this->input->post('contact_person_whatsapp'),
			'upi_id'=>$this->input->post('upi_no'),
			'acc_no'=>$this->input->post('bank_ac_no'),
			'acc_name'=>$this->input->post('ac_holder_name'),
			'ifsc_code'=>$this->input->post('ifsc_code'),
			'bank_name'=>$this->input->post('bank_name'),
			'bank_branch'=>$this->input->post('bank_branch'),
			'bank_add'=>$this->input->post('bank_address'),
			'payment_term'=>$this->input->post('pay_term'),
			'credit_limit'=>$this->input->post('credit_days'),
			'active'=>$this->input->post('active'),
			'StartDate'=>to_sql_date($this->input->post('StartDate')).' '.date('H:i:s'),
			'IsTDS'=>$this->input->post('Tds'),
			'TDSPer'=>$this->input->post('TdsPercent'),
			'TDSSection'=>$this->input->post('Tdsselection'),
			...
			$Dis_Arr = [];
			foreach ($DisData as $index => $Disc) {
				$DisDays = $Disc['DisDays']; 
				$Percentage = $Disc['DisPercentage']; 
			...
			$VendorData  = $this->purchase_model->SaveVendor($Clientdata,$Contactdata,$Balancedata,$Items_Arr,$Dis_Arr);
		...
		public function UpdateVendor()
		{
			...
			// echo "<pre>";print_r($Dis_Arr);die;
			$VendorData  = $this->purchase_model->UpdateVendor($Clientdata,$Contactdata,$Balancedata,$AccountID,$itemdata,$Dis_Arr);
			echo json_encode($VendorData);
		}
		...
		public function EditPurchaseOrder($id="")
		{
			...
			if ($this->input->post()) {
				...
				$idd = $this->purchase_model->update_purchase_order_PO($pur_order_data,$id);
				if ($idd) {
			...
			$data['purchase_details'] = $this->purchase_model->get_purchase_order_master_data($id);
			// echo "<pre>";
			// print_r($data['purchase_details']->AccountID);
			// die;
			$data['vendors'] = $this->purchase_model->GetRMVendor();
			$data['units'] = $this->purchase_model->get_units();
			$data['items'] = $this->purchase_model->get_items();
			// $data['item_code'] = $this->purchase_model->get_items_code();
			$data['item_code'] = $this->purchase_model->GetVendorWiseItemsEdit($data['purchase_details']->AccountID);
			$data['accounts'] = $this->purchase_model->get_accounts_list();
