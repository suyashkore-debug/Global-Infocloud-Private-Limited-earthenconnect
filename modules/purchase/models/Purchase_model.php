	...
	{   
		...
		*/
		...
		public function GetAccountList()
		{
			...
			$Data = $this->db->get('tblaccountgroupssub')->result_array();
		...
		public function GetItemDetails($ItemID)
		{
			...
			$this->db->select('tblitems.item_code,tblitems.description,tblitems.unit,tblitems.case_qty');
		...
		public function get_data_vendor($id = '')
		{
			...
			$this->db->select('tblclients.*,tblxx_citylist.*,tblxx_statelist.*,tblTDSMaster.*,tblaccountbalances.BAL1');
			...
			if($data){
				...
				$data->Listitems = $this->GetVendorWiseItems($id);
			}
			return $data;
		}
		...
		public function GetVendorList($id = '')
		{
			...
			$selected_company = $this->session->userdata('root_company');
			$year = $_SESSION['finacial_year'];
			$this->db->select(db_prefix() . 'clients.*,'.db_prefix() . 'accountbalances.BAL1,'.db_prefix() . 'xx_citylist.city_name,'.db_prefix() . 'xx_statelist.state_name,'.db_prefix() . 'contacts.FLNO1,'.db_prefix() . 'contacts.email,'.db_prefix() . 'contacts.Pan,'.db_prefix() . 'contacts.Aadhaarno');
			...
			$this->db->join(db_prefix() . 'accountbalances', db_prefix() . 'accountbalances.AccountID = ' . db_prefix() . 'clients.AccountID AND '.db_prefix() . 'accountbalances.PlantID = ' . db_prefix() . 'clients.PlantID AND '.db_prefix() . 'accountbalances.FY ="'.$year.'"', 'left');
			$this->db->where(db_prefix() . 'clients.PlantID', $selected_company);
			//$this->db->where_in(db_prefix() . 'clients.SubActGroupID', ["1000058","50003008","50003009"]);
			$this->db->where_in(db_prefix() . 'clients.SubActGroupID', $commaSeparatedSubActGroupIDs);
			if($id){
				$this->db->where(db_prefix() . 'clients.AccountID', $id);
		...
		public function GetVendorListNEW($id = '')
		{
			...
			if($Data){
				...
				$this->db->join(db_prefix() . 'items', db_prefix() . 'items.item_code = ' . db_prefix() . 'VendorWiseItems.ItemID AND '.db_prefix() . 'items.PlantID = ' . $selected_company . '', 'left');
		...
		// Add New Vendor
		public function SaveVendor($Clientdata,$Contactdata,$Balancedata,$itemdata,$DisData)
		{
			$this->db->insert(db_prefix() . 'clients', $Clientdata);
			$INSERT = $this->db->affected_rows();
			if($INSERT > 0){
				// $next_number = (int) get_option('next_vendor_number');
				// Update next number in settings
				// $next_number = $next_number+1;
				// $this->db->where('name', 'next_vendor_number');
				...
				$this->db->insert_batch(db_prefix() . 'VendorWiseItems', $itemdata);
		...
		*/
		...
		public function GetVendorWiseItems($AccountID)
		{
			$selected_company = $this->session->userdata('root_company');
			//   $year = $_SESSION['finacial_year'];
			return $this->db->query('select tblitems.id as id, CONCAT(tblitems.item_code," - ",tblitems.description) as label,tblitems.item_code from '.db_prefix().'VendorWiseItems
			Inner JOIN tblitems ON tblitems.item_code = tblVendorWiseItems.ItemID AND tblitems.PlantID = ' . $selected_company . '
		...
		public function GetVendorWiseItemsEditReturn($AccountID){
			$selected_company = $this->session->userdata('root_company');
			//   $year = $_SESSION['finacial_year'];
			return $this->db->query('select tblitems.item_code as id, CONCAT(tblitems.item_code," - ",tblitems.description) as label,tblitems.item_code from '.db_prefix().'VendorWiseItems
			Inner JOIN tblitems ON tblitems.item_code = tblVendorWiseItems.ItemID AND tblitems.PlantID = ' . $selected_company . '
			where tblVendorWiseItems.AccountID = "'.$AccountID.'" AND PlantID = '.$selected_company)->result_array();
		}
		...
		*/
		public function get_accounts_list()
		{
			$selected_company = $this->session->userdata('root_company');
			$ss = 'SELECT *
			FROM tblclients WHERE PlantID ='.$selected_company.' AND active = 1 AND SubActGroupID NOT IN("1000012")';
		...
		public function get_accounts_freightid($id ="")
		{
			...
			if(!empty($id)){
				$ss = 'SELECT *
				FROM tblclients  WHERE AccountID="'.$id.'" AND PlantID ='.$selected_company.' AND active = 1 AND SubActGroupID NOT IN("1000012")';
				
				$result_data = $this->db->query($ss)->row();
				return $result_data;
				}else{
				$ss = 'SELECT *
				FROM tblclients WHERE AccountID="209" AND PlantID ='.$selected_company.' AND active = 1 AND SubActGroupID NOT IN("1000012")';
				$result_data = $this->db->query($ss)->row();
				return $result_data;
			}
		} 
		public function get_accounts_othertid($id =""){
			...
			if(!empty($id)){
				$ss = 'SELECT *
				FROM tblclients  WHERE AccountID="'.$id.'" AND PlantID ='.$selected_company.' AND active = 1 AND SubActGroupID NOT IN("30000004","10022003","10022004","10022005","1002504","1002503","1002506","30000006","30000007","30001002","50003002","60001004")';
		...
		public function items_change($code){
			...
			LEFT JOIN `tblitems_sub_groups` ON `tblitems_sub_groups`.`id` = `tblitems`.`SubGrpID1` LEFT JOIN `tblitems_main_groups` ON `tblitems_main_groups`.`id` = `tblitems_sub_groups`.`main_group_id` WHERE `tblitems`.`id` = '".$code."'";    
			$query = $this->db->query($sql);
			
			$data = $query->row();
			$data->old_rate = $this->get_p_order_detail_old_item($data->item_code);
			$data->stock_avl = $this->GetItemStock($data->item_code);
			return $data;
			//   echo $this->db->last_query();die;
			// return $rs;
		...
		*/
		...
		public function get_p_order_detail($pur_request){
			...
			foreach($data  as $key => $value){
				...
				$data[$key]['stock_avl'] = $this->GetItemStock($value['item_code']);
			}
			return $data;
		...
		{	
			...
			if($this->db->affected_rows() > 0){
				...
				foreach($es_detail as $value){
					...
					$Po_item =  $this->db->get_where(db_prefix().'history',array('ItemID'=>$item_c->item_code,'OrderID'=>$data['po_number'],'TType2'=>'Order'))->row();
					$PEntry_item =  $this->SumEntryItem($data['po_number'],$item_c->item_code);
					...
					if($this->db->insert(db_prefix() . 'history',$data_array_result))
					{
						...
						$ChkQcMaster = $this->ChkQCMasterByItemID($item_c->item_code);
		...
		*/
			...
			if($chkQC == true){
				...
				if($this->db->affected_rows() > 0){
					...
					foreach($es_detail as $value){
						$item_c =  $this->db->get_where(db_prefix().'items',array('id'=>$value['item_code'],'PlantID'=>$selected_company))->row();
						$Po_item =  $this->db->get_where(db_prefix().'history',array('ItemID'=>$item_c->item_code,'OrderID'=>$data['Po_number'],'TType2'=>'Order'))->row();
						$PEntry_item =  $this->SumEntryItemEdit($data['Po_number'],$item_c->item_code,$PurchID);
		...
		public function table_data($data)
		{
			...
			company,state,address,StationName,city,tblclients.active as actstatus,tblxx_statelist.state_name, (SELECT GROUP_CONCAT(name SEPARATOR ",") FROM tblcustomers_groups WHERE tblcustomers_groups.id = tblclients.DistributorType) as customerGroups
			FROM tblclients
        ...
        public function GetItemListbyGroups($main_group,$SubGroup1,$SubGroup2)
        {
			...
			$this->db->join(db_prefix() . 'items_sub_groups', '' . db_prefix() . 'items_sub_groups.id = ' . db_prefix() . 'items.SubGrpID1', 'left');
			$this->db->join(db_prefix() . 'items_main_groups', '' . db_prefix() . 'items_main_groups.id = ' . db_prefix() . 'items_sub_groups.main_group_id', 'left');
			$this->db->join(db_prefix() . 'qc_master', '' . db_prefix() . 'qc_master.ItemID = ' . db_prefix() . 'items.item_code', 'left');
        ...
        public function get_data_table_qc_master()
        {
			...
			$this->db->select('tblqc_master.ItemID,tblitems.description,tblitems.unit,tblitems_main_groups.name as group_name,tblitems_sub_groups.name as subgroup_name');
			$this->db->join('tblitems', 'tblitems.item_code = tblqc_master.ItemID','INNER');
			$this->db->join(db_prefix() . 'items_main_groups', '' . db_prefix() . 'items_main_groups.id = ' . db_prefix() . 'items.group_id', 'left');
			$this->db->join(db_prefix() . 'items_sub_groups', '' . db_prefix() . 'items_sub_groups.id = ' . db_prefix() . 'items.SubGrpID1', 'left');
			$this->db->where(db_prefix().'items.PlantID', $selected_company);
		...
		public function add_pur_order_po($data)
		{
			...
			if($PlantID == 1){
				$purchase_orderNumbar = get_option('next_purchase_order_number_for_cspl');
			}elseif($PlantID == 2){
				$purchase_orderNumbar = get_option('next_purchase_number_for_cff');
			}elseif($PlantID == 3){
				$purchase_orderNumbar = get_option('next_purchase_number_for_cbu');
			...
			$Discamt = 0;
			$new_purchase_orderNumbar = 'PO'.$FY.$purchase_orderNumbar;   
			$ItCount = count($es_detail);
			$Transdate =  to_sql_date($data['prd_date'])." ".date('H:i:s');
			$PurchID =  $data['pur_order_number'];
			$prd_date =  $data['prd_date'];
			...
			$this->db->insert(db_prefix() . 'purchaseordermaster',$data_array);
			//print_r($data_array);
			if($this->db->affected_rows() > 0){
				$ord_n = 1;
				$this->increment_next_purchase_order_number();
				$i =1;
				foreach($es_detail as $value){
					$item_c =  $this->db->get_where(db_prefix().'items',array('id'=>$value['item_code'],'PlantID'=>$PlantID))->row();
					
					$get_purch_stock = $this->get_purch_stock($item_c->item_code);
					$VenItem =  $this->db->get_where(db_prefix().'VendorWiseItems',array('ItemID'=>$item_c->item_code,'AccountID'=>$AccountID))->row();
					$OldRate = $this->get_last_purch_order_detail_old_item($item_c->item_code);
					
					$daysToAdd    = (int)$VenItem->DeliveryDays; // dynamic number of days
					if(empty($daysToAdd)){
						$daysToAdd = 0;
					}
					$ExpectedDate = date('Y-m-d H:i:s', strtotime($Transdate . " +{$daysToAdd} days"));
					...
					$this->db->insert(db_prefix() . 'history',$data_array_result);
					if(!empty($OldRate)){
						$OldBasicRate = $OldRate->BasicRate;
						if($value['PurchRate'] > $OldBasicRate){
							$this->db->select(db_prefix() . 'staff.*');
							...
							foreach($AdminStaff as $admin){
								// echo "ok";die;
								$Notification_msg = "Purchase Order Rate Is Greater Than Previous Purchase Of (".$item_c->description.")(".$new_purchase_orderNumbar.")";
								$notification_data = [
								'description' => $Notification_msg,
								'touserid'    => $admin['staffid'],
								'link'        => 'purchase/EditPurchaseOrder/'.$new_purchase_orderNumbar,
								]; 
								$notification_data['additional_data'] = serialize([
								'Purch Order No. '.$new_purchase_orderNumbar,
								]);
		...
		{  
			...
			INNER JOIN tblitems ON tblitems.item_code = DataHistory.ItemID
			WHERE '.$sql1;
			$result = $this->db->query($sql)->result_array();
			return $result;
		}
		...
		{	
			...
			$old_pur_details = $this->purchase_model->get_purchase_detail_order($id);
			$back_ItCount =  $this->db->select('*')->get_where(db_prefix().'purchaseordermaster',array('PurchID'=>$id,'PlantID'=>$selected_company))->row();
			...
			$RoundOffAmt =  $data['Round_OFF'];
			$igstamt =  str_replace(",","",$data['IGST_amt']);
			$TDS_amt =  str_replace(",","",$data['TDS_amt']);
			$Invamt =  str_replace(",","",$data['Invoice_amt']);
			$purchAmt = str_replace(",","",$data['total_mn']);
			...
			$this->db->LIKE('FY', $fy);
			$this->db->where('PurchID',$id);
			$this->db->update(db_prefix() . 'purchaseordermaster',$data_array);
			if($this->db->affected_rows() > 0){
				$this->db->where('PlantID', $selected_company);
				$this->db->LIKE('FY', $fy);
				$this->db->where('AccountID', $data['vendor']);
				$this->db->where('OrderID', $PurchID);
				$this->db->delete(db_prefix() . 'history');
				...
				foreach($es_detail as $value){
					$item_c =  $this->db->get_where(db_prefix().'items',array('id'=>$value['item_code'],'PlantID'=>$selected_company))->row();
					$VenItem =  $this->db->get_where(db_prefix().'VendorWiseItems',array('ItemID'=>$item_c->item_code,'AccountID'=>$data['vendor']))->row();
					$OldRate = $this->get_last_purch_order_detail_old_item($item_c->item_code);
					$daysToAdd    = (int)$VenItem->DeliveryDays; // dynamic number of days
					if(empty($daysToAdd)){
						$daysToAdd = 0;
					}
					$ExpectedDate = date('Y-m-d H:i:s', strtotime($new_date . " +{$daysToAdd} days"));
					...
					if(!empty($OldRate)){
						...
						if($value['PurchRate'] > $OldBasicRate){
							...
							foreach($AdminStaff as $admin){
								// echo "ok";die;
								$Notification_msg = "Purchase Order Rate Is Greater Than Previous Purchase Of (".$item_c->description.")(".$id.")";
								$notification_data = [
								'description' => $Notification_msg,
								'touserid'    => $admin['staffid'],
								'link'        => 'purchase/EditPurchaseOrder/'.$id,
								]; 
								$notification_data['additional_data'] = serialize([
								'Purch Order No. '.$id,
								]);
		...
		public function GetItemStock($itemId)
		{
			...
			$this->db->from(db_prefix() .'items');
			$this->db->join(db_prefix() . 'stockmaster', db_prefix() . 'stockmaster.ItemID = ' . db_prefix() . 'items.item_code AND '.db_prefix() . 'stockmaster.PlantID = ' . db_prefix() . 'items.PlantID AND '. db_prefix() . 'stockmaster.FY = '.$fy.'', 'left');
			$this->db->where(db_prefix() .'items.PlantID', $selected_company);
			$this->db->where(db_prefix() .'items.item_code', $itemId);
		...
		public function GetQCData_byQCno($qc_no){
			...
			$this->db->join(db_prefix() . 'clients', '' . db_prefix() . 'clients.AccountID = ' . db_prefix() . 'purchasemaster.AccountID', 'Inner');
			$this->db->join(db_prefix() . 'items', db_prefix() . 'items.item_code = ' . db_prefix() . 'qc_item.ItemID', 'INNER');
		...
		public function get_Total_Inspection_Done_By_PO_Item($id,$item){
			...
			$this->db->where(db_prefix() . 'history.PlantID', $selected_company);
			$this->db->where(db_prefix() . 'items.PlantID', $selected_company);
			$this->db->where(db_prefix() . 'items.item_code', $item);
			$this->db->where(db_prefix() . 'history.FY', $year);
			return $this->db->get()->result_array();
		}
		...
		public function get_unique_history_QC($id){
			...
			$this->db->from(db_prefix() . 'history');
			$this->db->join(db_prefix() . 'items', db_prefix() . 'items.item_code = ' . db_prefix() . 'history.ItemID', 'left');
			$this->db->join(db_prefix() . 'qc_item', db_prefix() . 'qc_item.ItemID = ' . db_prefix() . 'items.item_code', 'left');
			$this->db->join(db_prefix() . 'clients', db_prefix() . 'clients.AccountID = ' . db_prefix() . 'history.AccountID', 'left');
			// $this->db->join(db_prefix() . 'history', db_prefix() . 'history.OrderID = ' . db_prefix() . 'purchasemaster.PurchID', 'left');
			$this->db->where_in(db_prefix() . 'history.OrderID',$id);
			$this->db->where(db_prefix() . 'history.PlantID', $selected_company);
		...
		{  
			...
			JOIN tblitems ON tblitems.item_code = tblqc_item.ItemID
			WHERE '.$sql1;
			$result = $this->db->query($sql)->result_array();
			return $result;
		}
		...
		public function get_items_code_qc(){
			$selected_company = $this->session->userdata('root_company');
			//   $year = $_SESSION['finacial_year'];
			return $this->db->query('select item_code as id, CONCAT(item_code," - ",description) as label,item_code from '.db_prefix().'items where PlantID = '.$selected_company)->result_array();
		}
		...
		{ 
			...
			foreach ($result as &$each) {
				...
				'PaidAmt' => 0, 'PurchRtnAmt' => 0, 'DebitNoteAmt' => 0,
				'JournalDebitAmt' => 0, 'JournalCreditAmt' => 0
				];
		...
		public function ItemSubGroups()
        {
            ...
            $this->db->where_in('main_group_id', $id);
            $this->db->where_in('sub_group_id1', $id2);
            return $this->db->get()->result_array();
		}
		...
		{  
			...
			LEFT JOIN tblitems ON UPPER(tblitems.item_code) = UPPER(tblhistory.ItemID)
			WHERE '.$sql1;
			$result = $this->db->query($sql)->result_array();
			return $result;
		}
		...
		public function GetAllVendorList()
		{
			$fy = $this->session->userdata('finacial_year');
			$selected_company = $this->session->userdata('root_company');
			$sql = 'SELECT tblclients.AccountID,tblclients.company FROM tblclients 
			WHERE tblclients.SubActGroupID1 = "100023" AND tblclients.PlantID = "'.$selected_company.'" 
			AND tblclients.Blockyn = "N" Order By company ASC';
			$result = $this->db->query($sql)->result_array();
			return $result;
		}
		...
		public function GetMonthlyPurchase($filterdata)
		{
			...
			if(!empty($MainItemGroup) && empty($SubGroup1)){
				$this->db->select('DATE_FORMAT(tblhistory.TransDate2, "%b-%Y") as month,tblitems_sub_groups.id as ItemID,tblitems_sub_groups.name as ItemName,SUM(tblhistory.NetChallanAmt) as NetChallanAmt,tblhistory.TType,tblhistory.TType2');
				}elseif(!empty($SubGroup1) && empty($SubGroup2)){
				...
				$this->db->select('DATE_FORMAT(tblhistory.TransDate2, "%b-%Y") as month,tblitems.item_code as ItemID,tblitems.description as ItemName,SUM(tblhistory.NetChallanAmt) as NetChallanAmt,tblhistory.TType,tblhistory.TType2');
			}
		...
		public function GetDailyPurchase($filterdata)
		{
			...
			* ------------------------ */
			...
			if (!empty($MainItemGroup) && empty($SubGroup1)) {
				$this->db->select('DATE_FORMAT(tblhistory.TransDate2, "%d-%b-%Y") as day,tblitems_sub_groups.id as ItemID,tblitems_sub_groups.name as ItemName,SUM(tblhistory.NetChallanAmt) as NetChallanAmt,tblhistory.TType,tblhistory.TType2');
				} elseif (!empty($SubGroup1) && empty($SubGroup2)) {
				...
				$this->db->select('DATE_FORMAT(tblhistory.TransDate2, "%d-%b-%Y") as day,tblitems.item_code as ItemID,SUM(tblhistory.NetChallanAmt) as NetChallanAmt,tblhistory.TType,tblhistory.TType2');
			}
			$this->db->from('tblhistory');
			$this->db->join(db_prefix() . 'items', db_prefix() . 'items.item_code = ' . db_prefix() . 'history.ItemID AND ' . db_prefix() . 'items.PlantID = ' . db_prefix() . 'history.PlantID');
			$this->db->join(db_prefix() . 'clients', db_prefix() . 'clients.AccountID = ' . db_prefix() . 'history.AccountID AND '.db_prefix() . 'clients.PlantID = ' . db_prefix() . 'history.PlantID');
			...
			* ------------------------ */
			...
			foreach ($Group as $g) {
				...
				foreach ($itemTrans as $tr) {
					...
					$Amount = 0;
					if ($tr["TType"] == "P" && $tr["TType2"] == "Purchase") $Amount = $tr['NetChallanAmt'];
		...
		public function GetTopPurchaseRateByItemGroup($filterdata)
		{
			...
			// ---------- 1. Get Purchases ----------
			if(!empty($MainItemGroup) && empty($SubGroup1)){
				$this->db->select('tblitems_sub_groups.id as ItemID,tblitems_sub_groups.name as ItemName,SUM(NetChallanAmt) as purchase_amt');
				} elseif(!empty($SubGroup1) && empty($SubGroup2)){
				$this->db->select('tblitems_sub_group2.id as ItemID,tblitems_sub_group2.name as ItemName,SUM(NetChallanAmt) as purchase_amt');
				} else {
				$this->db->select('tblitems.item_code as ItemID,tblitems.description as ItemName,SUM(NetChallanAmt) as purchase_amt');
			}
			...
			if(!empty($MainItemGroup) && empty($SubGroup1)){
				$this->db->join('tblitems_sub_groups', 'tblitems_sub_groups.id = tblitems.SubGrpID1');
			}
			if(!empty($SubGroup1) && empty($SubGroup2)){
...
//----------------------- Sub group 1 -----------------------------
	...
	{ 
		...
		$this->db->join('tblitems', 'tblitems.item_code = tblhistory.ItemID AND tblitems.PlantID = tblhistory.PlantID', 'INNER');
		$this->db->join('tblitems_sub_groups', 'tblitems_sub_groups.id = tblitems.SubGrpID1', 'INNER');
		$this->db->where(db_prefix() . 'items_sub_groups.main_group_id', $MainGroupId);
		$this->db->where('tblhistory.TType', "P");
		$this->db->where('tblhistory.TType2', "Purchase");		 
		$this->db->where('tblhistory.TransDate >=', $from_date.' 00:00:00');
	...
	{		 
		...
		$this->db->join('tblitems', 'tblitems.item_code = tblhistory.ItemID AND tblitems.PlantID = tblhistory.PlantID', 'INNER');
		$this->db->join('tblitems_sub_group2', 'tblitems_sub_group2.sub_group_id1 = tblitems.SubGrpID1', 'INNER');
