...
foreach ($rResult as $aRow) {
    ...
    $subjectOutput = '<a href="' . admin_url('account/contract/' . $aRow['id']) . '">' . $aRow['name_contracttype'] . '</a>';
    ...
    $subjectOutput .= ' <a href="' . admin_url('account/contract/' . $aRow['id']) . '">' . _l('edit') . '</a>';
    }

    if (has_permission('hrm', '', 'delete')) {
        $subjectOutput .= ' | <a href="' . admin_url('account/delete_contract/' . $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
    }

    $subjectOutput .= '</div>';
    $row[] = $subjectOutput;

    $row[] = ' <a href="' . admin_url('account/member/' . $aRow['staff']) . '">' . $aRow['firstname'] .' '. $aRow['lastname'] . '</a>';
