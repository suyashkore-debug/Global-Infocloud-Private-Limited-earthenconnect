<?php
	...
	class Accounting extends AdminController
	{
		public function __construct()
		{
			...
			$this->load->model('paymententry_model');
		...
		public function TFormatBalanceSheet($date = "")
		{
			...
			if(!empty($date)){
				...
				$GrossProfitC_F = ($TotalRevenueIncome + $ClosingInventoryAmt->CurrentYear) - ($OpeningInventoryAmt->CurrentYear + $TransactionAmt->PurchCurrentYear + $DirectExp->CurrentYear); 
				$Total = $GrossProfitC_F + ($OpeningInventoryAmt->CurrentYear + $TransactionAmt->PurchCurrentYear + $DirectExp->CurrentYear);
				
				$IndirectExp = ($OtherExpensesData->CurrentYear + $EMPBenData->CurrentYear + $FinanceCostData->CurrentYear + $DeprecData->CurrentYear);
				$NetProfit = $GrossProfitC_F + $OtherIncome->CurrentYear - $IndirectExp; 
				$AllTotal = $NetProfit + $IndirectExp;
				
				$Total2 = $TotalRevenueIncome + $ClosingInventoryAmt->CurrentYear;
				$TotalRevenueIncome += $OtherIncome->CurrentYear;
				$TotalRevenueIncomePre += $OtherIncome->PriviousYear;
				$AllTotal2 = $GrossProfitC_F + $OtherIncome->CurrentYear;
				
				$PL_for_the_period->CurrentYear = $NetProfit;
				$PL_for_the_period->PriviousYear = 0;
				$PL_for_the_period->isPrimary = "Y";
    ...
    $passage_from = $this->input->post('passage_from');

    if (strtoupper($passage_from) == 'RECEIPTS') {
        $permission = 'accounting_receipt_entry';
    } elseif (strtoupper($passage_from) == 'JOURNAL') {
        $permission = 'accounting_journal_entry';
    } elseif (strtoupper($passage_from) == 'PAYMENTS') {  
        $permission = 'accounting_payment_entry';
    } else {
        $permission = 'accounting_contra_entry';
    }

    if (!has_permission_new($permission, '', 'view')) {
        echo json_encode(['success' => false, 'message' => 'Access denied']);
        return;
    }

    $selected_date = to_sql_date($this->input->post('selected_date'));

    if (empty($selected_date)) {
        echo json_encode(['success' => false, 'message' => 'Date is empty']);
        return;
    }
    ...
    if (empty($plant_id)) {
        echo json_encode(['success' => false, 'message' => 'Plant ID not found']);
        return;
    }

    try {
        $voucher_number = $this->receipt_model->generateNextVoucherIDNew(
            $selected_date,
            $plant_id,
            $passage_from
        );
        ...
        if (empty($voucher_number)) {
            echo json_encode(['success' => false, 'message' => 'Could not generate voucher number']);
            return;
        }
		...
		//======================= New Receipt Entry 04May26 ==========================
		public function receipt_entry($id = '')
		{			
			...
			if ($this->input->post()) {
				...
				if($id == ''){
					...
					$success = $this->receipt_model->add_receipts_entry($data);
					if ($success === 'close_the_book') {
						$message = _l('has_closed_the_book');
						set_alert('warning', _l('has_closed_the_book'));
						}elseif ($success) {
					...
					$success = $this->receipt_model->update_receipts_entry($data, $id);
					if ($success === 'close_the_book') {
						$message = _l('has_closed_the_book');
						set_alert('warning', _l('has_closed_the_book'));
						}elseif ($success) {
			...
			if($id != ''){
				$data['receipts_entry'] = $this->receipt_model->get_receipts_entry($id);
			...
			$data['account_to_select'] = $this->receipt_model->get_data_account_to_select_for_receipts();
			$data['genral_account_to_select'] = $this->receipt_model->get_data_ganeral_account_to_select();
			
			$this->load->view('receipt/receipt_entry', $data);
		}
		...
		//============= New Add Edit Payment Entry Page Load ===============================
		public function payment_entry($id = '')
		{
			...
			if ($this->input->post()) {
				...
				if($id == ''){
					...
					$success = $this->paymententry_model->add_payment_entry($data);
					if ($success === 'close_the_book') {
						$message = _l('has_closed_the_book');
						set_alert('warning', _l('has_closed_the_book'));
						}elseif ($success) {
					...
					$success = $this->paymententry_model->update_payments_entry($data, $id);
					if ($success === 'close_the_book') {
						$message = _l('has_closed_the_book');
						set_alert('warning', _l('has_closed_the_book'));
						}elseif ($success) {
			...
			if($id != ''){
				$data['payment_entry'] = $this->paymententry_model->get_payments_entry($id);				
			}
			$this->load->model('currencies_model');
			$data['currency'] = $this->currencies_model->get_base_currency();
			$data['title'] = "Add Edit Payment Voucher";
			
			$data['account_to_select'] = $this->paymententry_model->get_data_account_to_select_for_payment();
			$data['genral_account_to_select'] = $this->paymententry_model->get_data_ganeral_account_to_select();
