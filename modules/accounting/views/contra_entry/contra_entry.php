				...
				<div class="panel-body">
					<nav aria-label="breadcrumb">
						<ol class="breadcrumb custombreadcrumb" style="background-color:#fff !important; margin-Bottom:0px !important;">
							...
							<li class="breadcrumb-item active" aria-current="page"><b>Contra Entry</b></li>
						</ol>
					</nav>
					<hr class="hr_style">
					<?php $arrAtt = array();
					...
					<div class="row">
						...
						<div class="col-md-2">
							...
							?>
							<?php $value = (isset($contra_entry) ? _d(substr($contra_entry->Transdate,0,10)) : _d($date)); ?>
							<?php echo render_date_input('contra_date','Voucher Date',$value,$data_attr); ?>
							<?php
								if(isset($contra_entry)){
								?>
								<input type="hidden" name="contra_date1" value="<?php echo _d(substr($contra_entry->Transdate,0,10)); ?>">
								<input type="hidden" name="VoucheriD" value="<?php echo $contra_entry->VoucherID; ?>">
								<?php
								}
							?>
						</div>
						<div class="col-md-2">
							...
							<?php
								...
								if($selected_company == 1){
									...
									$new_contraNumber = get_option('next_contra_number_for_cbupl');
								}
							?>
						...
						<div class="col-md-2">
							<?php $value = (isset($contra_entry) ? $contra_entry->TransType : ''); ?>
							<label > Type</label>
						...
						<div class="col-md-4">
							<?php $value = (isset($contra_entry) ? $contra_entry->MainAccount : ''); ?>
							<?php 
								$select_attrs = [];
							...
							<?php
								...
								<?php
									if (has_permission_new('accounting_contra_entry', '', 'edit')) {
										...
										$contra_date = substr($contra_entry->Transdate,0,10);
										$curr_date = date('Y-m-d');
									...
									?>
									<button type="button" class="btn btn-info contra-entry-form-submiter" onclick="this.disabled = true">Update</button>
									<?php
									}?>
									<?php
									}
								...
								<?php
									...
									?>
									<button type="button" class="btn btn-info contra-entry-form-submiter" onclick="this.disabled = true"><?php echo _l('submit'); ?></button>
									<?php
									}
								?>    
								<?php
						...
						<?php
							...
							<div class="col-md-1" style="margin-top: 20px;">
								<?php
									...
									?>  
									<a href="<?php echo admin_url('accounting/delete_contra_entry/' . $contra_entry->VoucherID) ?>" class="btn btn-danger <?php echo $return;?>">Delete</a>
									<?php
									}
								?>    
							</div>
						...
						<table class="table text-right">
							<tbody>
								...
								<tr>
									...
									<td class="text-right bold" style="width: 38%;">Total Amount</td>
									<td class="total_amt" style="width: 17%;">
										<?php $value = (isset($contra_entry) ? $contra_entry->TotalAmt : 0); ?>
...
<script>
	...
	$(document).on("click", ".sortablePop", function () {
		var table = $("#table_contra_report tbody");
