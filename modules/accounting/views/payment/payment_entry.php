    ...
    <div class="row">
      <div class="panel_s col-md-12">
		...
		<hr class="hr_style">
          ...
          <div class="row">
            ...
            <div class="col-md-2">
                ...
                ?>
              <?php $value = (isset($payment_entry) ? _d(substr($payment_entry->Transdate,0,10)) : _d($date)); ?>
              <?php echo render_date_input('payment_date','Trans Date',$value,$data_attr); ?>
                <?php  if(isset($payment_entry)){  ?>
                    <input type="hidden" name="payment_date1" value="<?php echo _d(substr($payment_entry->Transdate,0,10)); ?>">
            ...
            <div class="col-md-2">
				<?php
					...
					if($selected_company == 1){
						$new_paymentNumber = get_option('next_payment_number_for_cspl');
					}elseif($selected_company == 2){
						$new_paymentNumber = get_option('next_payment_number_for_cff');
					}elseif($selected_company == 3){
						$new_paymentNumber = get_option('next_payment_number_for_cbu');
					}elseif($selected_company == 4){
						$new_paymentNumber = get_option('next_payment_number_for_cbupl');
					}
				?>
              <?php $value = (isset($payment_entry) ? $payment_entry->VoucherID : $new_paymentNumber); ?>
              <?php echo render_input('payment_number','Voucher No.',$value,'text',$data_attr2); ?>
            </div>
            <!--<div class="col-md-3">
                <?php //$value = (isset($payment_entry) ? $payment_entry->d_act : 'CASH'); ?>
				<?php //echo render_select('ganeral_account',$genral_account_to_select,array('AccountID','company'),'Pymt Mode',$value); ?>
            </div>-->
			...
			<div class="col-md-3">
				...
				<?php 
						...
						echo render_select('ganeral_account', $genral_account_to_select, array('AccountID','company'), 'Pymt Mode', $value,$select_attrs); 
				?>
			</div>
		...
		<?php
		    if(isset($payment_entry))
		    {
                ...
                if (has_permission_new('accounting_payment_entry', '', 'edit')) 
				{
                    ...
                    $payments_date = substr($payment_entry->Transdate,0,10);
                    $curr_date = date('Y-m-d');
					...
					$day = $result2->days;
						...
						if($day == 0){
                            ...
                            $payments_date_new    = new DateTime($payments_date);
                            if ($payments_date_new < $tillDate_new) {
                                $return = 'disabled';
                            }else{
                                $return = '';
                            }
							...
							<?php }else{ ?>
								<button type="button" class="btn btn-info payment-entry-form-submiter" onclick="this.disabled=true">Update</button>
							<?php } ?>
						 
                <?php } ?>
            <?php  }else{ ?>
						<?php if (has_permission_new('accounting_payment_entry', '', 'create')) { ?>
								<button type="button" class="btn btn-info payment-entry-form-submiter"><?php echo _l('submit');?></button>
						<?php } ?>
					<?php  }  ?>
            </div>
            <?php  if(isset($payment_entry)){  ?>
                <?php  if(has_permission_new('accounting_payment_entry', '', 'delete') && $payment_entry->Status == 'N') { ?>
                        <div class="col-md-1" style="margin-top: 20px;">
							<a href="<?php echo admin_url('accounting/ApproveVoucherEntry/' . $payment_entry->UniquID."/PAYMENTS") ?>" class="btn btn-success approve-voucher-btn <?php echo $return;?>">Approve</a>
                
                        </div>
                    <?php }  ?>
                <?php  if(has_permission_new('accounting_payment_entry', '', 'delete')) { ?>
                    <div class="col-md-1" style="margin-top: 20px;">
                
							<a href="<?php echo admin_url('accounting/delete_payment_entry/' . $payment_entry->UniquID."/PAYMENTS") ?>" class="btn btn-danger <?php echo $return;?>">Delete</a>
          ...
          <?php } ?>
            ...
            <?php
                if (has_permission_new('accounting_payment_entry', '', 'view')) {
                    ?>  
              <a href="#" class="btn btn-warning add-new-transfer mbot15"> Voucher List</a>
            <?php
                    }
                ?>
          </div>
        ...
        <table class="table text-right">
            <tbody>
				<tr>
				  <td><span class="bold" style="width:14%;">Account Opening Balance :</span></td>
				  <td class="acc_opt_bal" style="width:10%;">
					<?php echo app_format_money(0, $currency->name); ?>
				  </td>                  
				  <td><span class="bold" style="width: 19%;">Total Payment Amount :</span></td>
				  <td class="total_debit" style="width: 10%;">
						<?php $value = (isset($payment_entry) ? $payment_entry->damt : 0); ?>
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header">
            ...
            <h4 class="modal-title">Payment Voucher List</h4>
         </div>
         ...
         <div class="modal-body">
            ...
            ?> 
                ...
                <div class="col-md-3">
                    <div class="form-group" app-field-wrapper="entryStatus">
                        <label for="entryStatus" class="control-label"> <small class="req text-danger">* </small>Entry Status</label>
                        <div class="dropdown bootstrap-select bs3" style="width: 100%;">
                            <select id="entryStatus" name="entryStatus" class="selectpicker" data-width="100%" data-none-selected-text="Non selected" data-live-search="true" tabindex="-98">
...
<script type="text/javascript" language="javascript" >
    $(document).ready(function(){
        ...
        function load_data(from_date,to_date,entryStatus)
        {
            $.ajax({
            url:"<?php echo admin_url(); ?>accounting/load_data_for_payment",
            dataType:"JSON",
            method:"POST",
            data:{from_date:from_date, to_date:to_date,Isapprove:entryStatus},
            beforeSend: function () {
            ...
            success:function(data){
                ...
                var url = "'<?php echo admin_url() ?>accounting/new_payment_entry/"+data[count].VoucherID+"'";
        ...
        $('#search_data').on('click',function(){
                ...
                $(".report_for").text(msg);
                load_data(from_date,to_date,entryStatus);
    ...
    $('input[name="payment_date"]').on('change input', function () {
        fetchNextVoucherNumber();
    })

    function fetchNextVoucherNumber() {
        let date = $('input[name="payment_date"]').val();
        $('input[name="payment_number"]').val('Generating...');

        $.ajax({
            url:      '<?= admin_url('accounting/get_next_voucher_number'); ?>',
            type:     'POST',
            dataType: 'JSON',
            data:     { selected_date: date, passage_from: 'PAYMENTS' },
            success: function (r) {
                if (r && r.success) {
                    $('input[name="payment_number"]').val(r.voucher_number);
                } else {
                    var msg = (r && r.message) ? r.message : 'Unknown error';
                    $('input[name="payment_number"]').val('ERROR: ' + msg);
                }
            },
            error: function () {
                $('input[name="payment_number"]').val('AJAX ERROR');
            }
        });
    }
</script>
...
<?php require 'modules/accounting/assets/js/payment/handson_js.php';?>