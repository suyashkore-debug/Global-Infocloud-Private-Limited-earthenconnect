		...
		<nav aria-label="breadcrumb">
          ...
          <div class="row">
            ...
            <div class="col-md-3">
              ...
              <?php
                if(isset($receipts_entry)){
                    ...
                    <input type="hidden" name="UniqueID" id= "UniqueID" value="<?php echo $receipts_entry->UniquID; ?>">
                <?php
                }
              ?>
            </div>
            <div class="col-md-2">
                ...
                // }
                // $value = (isset($receipts_entry) ? $receipts_entry->VoucherID : $new_tax_transactionNumber);
                echo render_input('receipt_number','Voucher No.', '','text',$data_attr2);
                ?>
            </div>
            <!--<div class="col-md-3">
			...
			<div class="col-md-3">
				...
				<?php 
						...
						echo render_select('ganeral_account', $genral_account_to_select, array('AccountID','company'), 'Rcpt Mode', $value,$select_attrs); 
				?>
			</div>
            ...
            if(isset($receipts_entry)){
                ?>
                <?php
                    if (has_permission_new('accounting_receipt_entry', '', 'delete') && $receipts_entry->Status == 'N') {
                    ?>
                    <div class="col-md-1" style="margin-top: 20px;">
                        <a href="<?php echo admin_url('accounting/ApproveVoucherEntry/' . $receipts_entry->UniquID."/RECEIPTS") ?>" class="btn btn-success approve-voucher-btn <?php echo $return;?>">Approve</a>
                    </div>
                <?php
                    } 
                ?>  
                <div class="col-md-1" style="margin-top: 20px;"> 
                  <?php
                    ...
                    ?>
                        <a href="<?php echo admin_url('accounting/delete_receipt_entry/' . $receipts_entry->UniquID."/RECEIPTS") ?>" class="btn btn-danger <?php echo $return;?>">Delete</a>
                <?php
                    }
                    ?>    
                </div>
              ...
              <table class="tree table table-striped table-bordered table_receipts_report" id="table_receipts_report" width="100%">
                ...
                <thead>
                  <tr>
                    ...
                    <th class="sortablePop" style="text-align:left;">Is Approve</th>
                  </tr>
                </thead>
                <tbody>
                </tbody>
...
<script type="text/javascript" language="javascript" >
    $(document).ready(function(){
        ...
        function load_data(from_date,to_date,entryStatus)
        {
            ...
            dataType:"JSON",
            method:"POST",
            data:{from_date:from_date, to_date:to_date,entryStatus:entryStatus},
            beforeSend: function () {
            ...
            success:function(data){
                ...
                var url = "'<?php echo admin_url() ?>accounting/receipt_entry/"+data[count].VoucherID+"'";
    ...
    $('input[name="receipt_date"]').on('change input', function () {
        fetchNextVoucherNumber();
    })

    function fetchNextVoucherNumber() {
        let date = $('input[name="receipt_date"]').val();
        $('input[name="receipt_number"]').val('Loading...');

        $.ajax({
            url:      '<?= admin_url('accounting/get_next_voucher_number'); ?>',
            type:     'POST',
            dataType: 'JSON',
            data:     { selected_date: date, passage_from: 'RECEIPTS' },
            success: function (r) {
                if (r && r.success) {
                    $('input[name="receipt_number"]').val(r.voucher_number);
                } else {
                    var msg = (r && r.message) ? r.message : 'Unknown error';
                    $('input[name="receipt_number"]').val('ERROR: ' + msg);
                }
            },
            error: function () {
                $('input[name="receipt_number"]').val('AJAX ERROR');
            }
        });
    }
</script>
<script>
    ...
    // Receipt mode selected value change
    $(document).ready(function() {
...
<script>
    $(document).ready(function(){
        ...
        console.log('mindate ', minStartDate);
        console.log('maxdate ', maxEndDate_new);
    });
...
<?php require 'modules/accounting/assets/js/receipt/handson_js.php';?>
</body>
</html>