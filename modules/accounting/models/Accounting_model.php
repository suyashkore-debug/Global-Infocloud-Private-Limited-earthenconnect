<?php
	...
	class Accounting_model extends App_Model
	{
		...
		public function GetOpeningInventoryAmt($fromdate="",$todate="")
		{
			...
			$this->db->select('SUM(tblstockmaster.OQty) AS OQty,tblstockmaster.BasicRate,tblstockmaster.ItemID,tblstockmaster.FY,tblitems.MainGrpID');
			...
			foreach($ItemWiseInventory as $value){
				if($value["OQty"] > 0){
					...
					array_push($ItemListKeyVal,["ItemID"=>$value["ItemID"],"MainGrpID"=>$value["MainGrpID"],"OQty"=>$value["OQty"],"FY"=>$value["FY"],"BasicRate"=>$value["BasicRate"]]);
				}
			}
			...
			foreach($ItemListKeyValNew as $key=>$val){
				...
				if($val["MainGrpID"] == "1"){
					...
					if($val["OQty"] > 0 && $val["FY"] == $fy){
						$ItemAmt = $val["BasicRate"] * $val["OQty"];
						$TotalOpnAmt += $ItemAmt;
						$TotalFGOpnAmt += $ItemAmt;
					}
					if($val["OQty"] > 0 && $val["FY"] == $last_fy){
						$ItemAmtPre = $val["BasicRate"] * $val["OQty"];
					...
					if($val["OQty"] > 0 && $val["FY"] == $fy){
						$ItemAmt = $val["OQty"] * $val["BasicRate"];
						$TotalOpnAmt += $ItemAmt;
						$TotalRMOpnAmt += $ItemAmt;
					}
					if($val["OQty"] > 0 && $val["FY"] == $last_fy){
						$ItemAmtPre = $val["OQty"] * $val["BasicRate"];
						$TotalOpnAmtPre += $ItemAmtPre;
					}
				}
				$ItemWiseInventory[$key]["rate"] = $val["BasicRate"];
				$i++;
			}
		...
		public function GetClosingInventoryAmt($fromdate = "",$todate = "")
		{
			...
			foreach($ItemListKeyValNew as $kitem=>$ItemID){
				...
				foreach($ItemWiseOpnInventory as $kOpn =>$vOpn){
					if($ItemID["ItemID"] == $vOpn["ItemID"] && $vOpn["FY"] == $fy){
						$OpnQty = $vOpn["OQty"] * $vOpn["BasicRate"];
					}elseif($ItemID["ItemID"] == $vOpn["ItemID"] && $vOpn["FY"] == $last_fy){
						$OpnQtyPre = $vOpn["OQty"] * $vOpn["BasicRate"];
					}
				}
				...
				$BalQty = $TotalPurchQty - $TotalIssueQty + $TotalPrdQty - $TotalSaleQty + $TotalFrtRtnSaleQty - $TotalFreeQty - $TotalFreeDistQty;
				$BalQtyPre = $TotalPurchQtyPre - $TotalIssueQtyPre + $TotalPrdQtyPre - $TotalSaleQtyPre + $TotalFrtRtnSaleQtyPre - $TotalFreeQtyPre - $TotalFreeDistQtyPre;
				$ItemListKeyValNew[$kitem]["BalanceQty"] = $BalQty;
				$ItemListKeyValNew[$kitem]["BalanceQtyPre"] = $BalQtyPre;
				$ItemListKeyValNew[$kitem]["OpnAmt"] = $OpnQty;
				$ItemListKeyValNew[$kitem]["OpnAmtPre"] = $OpnQtyPre;
				$i++;
			}
			$RMRateData = $this->GetRMLastPurchaseRate($ItemList,$todate);
			$FGRateData = $this->GetFGRateItemWise($ItemList,$todate);
			...
			foreach($ItemListKeyValNew as $key=>$val){
				...
				if($val["MainGrpID"] == "1"){
					...
					if($val["BalanceQty"] > 0 || $val["BalanceQty"] < 0){
						$ItemAmt = $val["BalanceQty"] * $BasicRate;
						$FGTotalClsAmt += $ItemAmt;
						$TotalClsAmt += $ItemAmt;
					}
					if($val["BalanceQtyPre"] > 0 || $val["BalanceQtyPre"] < 0){
