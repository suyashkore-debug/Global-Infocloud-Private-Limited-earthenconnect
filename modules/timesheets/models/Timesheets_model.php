...
class timesheets_model extends app_model
{
	...
	public function get_staff($id = '', $where = [])
	{
		...
		//$SubActGroupID = array("30001002","5","7","8");
		//$this->db->where('active', 1);
		//$this->db->where_in('SubActGroupID', $RoleID);
		//$this->db->where('tblstaff.staff_comp REGEXP',$regExp);
...
public function delete_shift($id){
	...
	public function get_staff_timekeeping_applicable_object(){
		...
		$where .= ' AND  (login_access ="Yes" OR app_access = "Yes")';   
...
public function count_type_leave($department, $type,$custom_date_select){
	...
	public function check_in($data){
		...
		if(($date != '') && ($data['staff_id'] != '')){
			...
			unset($data['point_id']);  
			//print_r($data);
			$date_split_point = strpos($data['date']," ");
			$date = substr($data['date'],0,$date_split_point);
			$time = substr($data['date'],$date_split_point);
			...
			if ($insert_id) {
				...
				if($data['type_check'] == 1){
				    ...
				    $this->db->insert(db_prefix().'check_in_out_app2', $app_tbl_date);
				    
				}else {
				    
				    $this->db->where('staff_id',$data['staff_id']);
				    $this->db->where('date',$date);
	 	            $this->db->update(db_prefix() . 'check_in_out_app2', $app_tbl_date1);
	...
	public function get_in_out_shift_staff($staff_id, $date){
		...
		return $result; */  
		$this->db->where('staff_id',$staff_id);
       // $this->db->where('staff_id',$date);
		$this->db->like('date', $date, 'after');
	    $ss = $this->db->get(db_prefix().'check_in_out')->result_array();
		return $ss;
...
public function get_location($ip = ''){
	...
	public function get_reports()
	{
			...
			$this->db->where('staff_id', 97);
			$this->db->where('date', "2021-09-22");
			return  $this->db->get(db_prefix() . 'travel_report')->result_array();
		
	}
	
	public function get_reportsByID($date,$id)
	{
...
//echo "<pre>";
	...
	foreach($staffs_list as $s){
		...
		$dt_ts = [];
		$dt_ts_detail = [];
		$dt_ts = [_l('staff_id') => $s['AccountID'],_l('staff') => $s['firstname'].' '.$s['lastname'],'Department'=>''];
		$note = [];
		$list_dtts = [];
		...
		foreach ($list_date as $key => $value) {  
			...
			$max_hour = $this->get_hour_shift_staff($s['staffid'],$value);
			$get_in_out_data = "";
			$get_in_out_data = $this->get_in_out_shift_staff($s['staffid'],$value);
		
			$ci = "";
			$co = "";
			if($value < $s['datecreated']){
        	    $co = "Not Register";
        	    $ci = "Not Register";
        	}
        	$cur_date = date('Y-m-d');
			...
			foreach ($get_in_out_data as $key => $value) {
        		# code...
        		if($value["type_check"] == "1"){
        		    $pos = strpos($value["date"]," ");
        		    $val = substr($value["date"],$pos);
        		    if($ci == ""){
        		        $ci = $val;
        		    }else {
			...
			$check_holiday = $this->check_holiday($s['staffid'], $value);
			$result_lack = '';
			if($max_hour > 0 || !empty($get_in_out_data)){
				if(!$check_holiday){
					$ts_lack = '';
					...
					//$result_lack = $this->merge_ts($total_lack, $get_in_out_data->date);
					if($ci == "" && $co == ""){
					    //$result_lack = "";
					    $result_lack = "AB";
					}
					...
					if($ci !=="" && $co !== ""){
					    //$result_lack = "CI-".$ci.' CO-'.$co;
					    $result_lack = "P";
					    
					}
					if($ci =="Not Register" && $co == "Not Register"){
					    //$result_lack = "CI-".$ci.' CO-'.$co;
					    $result_lack = "NR";
					    
					}
					if($ci =="Next Day" && $co == "Next Day" ){
					    //$result_lack = "CI-".$ci.' CO-'.$co;
					    $result_lack = "";
...
public function role_name($role_id){
	...
	$data = $this->db->get(db_prefix().'roles')->row();
...
public function get_data_attendance_export($month_filter, $department_filter, $role_filter, $staff_filter, $state_filter){
	...
	$querystring = 'active=1 AND (login_access = "Yes" OR app_access = "Yes")';
	...
	if($state_filter != ''){
   			$state_querystring .= 'state = "'.$state_filter.'"';
   		}
	if($job_position != ''){
	...
	$arrQuery = array($staff_querystring,$department_querystring, $month_year_querystring, $state_querystring, $job_position_querystring, $querystring);
	$newquerystring = '';
