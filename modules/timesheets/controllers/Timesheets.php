	...
	class timesheets extends AdminController
	{
		...
		*/
		...
		public function staff_details(){
			...
			$staff_data = $this->staff_model->get2($staff_id);
			//$staff_data->state_name = $staff_data->state;
			$staff_data->state_name = get_state_name_by_id($staff_data->state);
			$staff_data->city_name = get_city_name_by_city_id($staff_data->city);
			$staff_data->staff_role = get_role_name_by_id($staff_data->role);
			$staff_data->report_to = get_staff_full_name($staff_data->team_manage);
		...
		*/
		public function timekeeping(){
			...
			$data['set_col_tk'][] = ['data' => _l('staff'), 'type' => 'text','readOnly' => true,'width' => 150];
			
			for ($d = 1; $d <= $days_in_month; $d++) {
				$time = mktime(12, 0, 0, $month, $d, $month_year);
				if (date('m', $time) == $month) {
					array_push($data['day_by_month_tk'], date('d', $time));
					array_push($data['set_col_tk'],[ 'data' => date('D d', $time), 'type' => 'text','width' => 40]);
				}
			}
		...
		*/
		public function reload_timesheets_byfilter(){
			...
			$querystring = 'active = 1 AND (login_access="Yes" OR app_access = "Yes") AND PlantID = "'.$selected_company.'"';
			...
			if($state != ''){
				...
				//$state_querystring = 'FIND_IN_SET(state, "'.$state.'")';
				$state_querystring = 'state = "'.$state.'"';
			}
			if($job_position != ''){
			...
			if($staff != ''){
				...
				$staff_querystring != '';
				if($data['state'] != ''){
					$staff_querystring .= 'state = "'.$data['state'].'"';
				}
		...
		*/
		public function add_requisition_ajax(){
			...
			if ($this->input->post()) {
				...
				unset($data['staff']);
				unset($data['number_day_offinput']);
		...
		*/
		...
		public function GetAnnualLeave(){
			...
			$day_off = $this->timesheets_model->get_day_off($staffid);
			if($day_off != null){
				$day_off->number_day_off = $day_off->remain;
				if($day_off->number_day_off < 0){
					$day_off->number_day_off = 0;
				}
				$day_off->days_off = $day_off->days_off;
				if($day_off->days_off > $day_off->total){
					$day_off->days_off = $day_off->total;
				}
			}
			echo json_encode($day_off);
			die;
		}
		/**
		...
		*/
		...
		{    
			...
			$data['staff']     = $this->timesheets_model->get_staff();
			$data['department']     = $this->departments_model->get();
			$data['roles']         = $this->roles_model->get();
			$data['workplace'] = $this->timesheets_model->get_workplace();
		...
		*/
		...
		{    
			...
			$data['word_shift'] = $this->timesheets_model->get_shift_type();
			$data['title'] = _l('hr_reports');
			$this->load->view('reports/manage_reports2', $data);
		}
		
		
		public function reports_new1($date,$id) 
		{    
			...
			$data['travel_report'] = $this->timesheets_model->get_reportsByID($date,$id);
			// $data['travel_report'] = $this->timesheets_model->travel_report_table();
			
			// print_r($data['travel_report']);exit;
			
			$this->load->view('reports/manage_report_list3', $data);
		}
		
		
		public function travel_report_table_filter()
		{
			$this->load->model('staff_model');
			$this->load->model('timesheets_model');
			...
			if (count($date_parts) == 3) {
				$report_date = $date_parts[2] . '-' . $date_parts[1] . '-' . $date_parts[0];
				} else {
			...
			// Fetch staff details
			$staffquery = $this->db->get_where('tblstaff', array('staffid' => $staff));
			...
			// Fetch the travel report data
			$travel_report = $this->timesheets_model->get_reportsByID($report_date, $staff);
			
			// Debugging
			// echo $this->db->last_query(); // Show the executed query for debugging purposes
			// print_r($travel_report); // Uncomment for debugging
			
			if ($travel_report) {
				echo json_encode(['status' => 'success', 'data' => $travel_report]);
				
				//   redirect(admin_url('timesheets/reports_new1/'.$report_date.'/'.$staff));
				
				} else {
				echo json_encode(['status' => 'no_data', 'message' => 'No data found for the selected filters.']);
			}
		}
		...
		*/
		...
		{    
			...
			$data['travel_reports'] = $this->timesheets_model->get_reports();
			$data['title'] = _l('hr_reports');
			$this->load->view('reports/reports_test', $data);
		}
		
		public function travel_report_table()
		{
			
			
			$this->app->get_table_data('travel_report_new');
		}
			...
			if($start_time != '' && $end_time != ''){
				if($start_time && $end_time){
					if(strtotime($start_time) <= strtotime($end_time)){
						...
						// echo "</pre>";print_r($list_date);die;
		...
		*/
		...
		public function attendance(){
			...
			$this->load->view('shift_management2', $data);
		}
		...
		*/
		...
		/**
			...
			* @return json 
		*/
		function shift_table2(){
			if ($this->input->is_ajax_request()) {
				if($this->input->post()){
					$months_report = $this->input->post('months_filter');
					
					//$staff_fillter = $this->input->post("staff_2_fillter");
					$staff_fillter2 = $this->input->post("staff_2_fillter2");
					...
					if(has_permission('report_management', '', 'view') || is_admin()){
						...
						if(isset($staff_fillter2)){
							$staffid_list = implode(',', $staff_fillter2);
							$query .= 'staff_id in ('.$staffid_list.') and';
						} 
					}
					else{
					...
					//$query .= db_prefix().'check_in_out_app2.staff_id== staffVisitLoc2.staff_id and';
					...
					$sIndexColumn = 'id';
					$sTable       = db_prefix() . 'check_in_out_app2';
					$join         = [];
					...
					foreach ($rResult as $aRow) {
						...
						//$row[] = unserialize($aRow['check_in_loc']);
						//$row[] = "<a href='#'><i class='fa fa-map-marker'></i></a>";
						...
						// End shift
						//$name = unserialize($aRow['check_in_loc_name']);
						//$row[] = $name;
						
						$row[] = unserialize($aRow['check_out']);
						//$row[] = unserialize($aRow['check_out_loc']);
						//$row[] = "<a href='#'><i class='fa fa-map-marker'></i></a>";
						
						//$row[] = unserialize($aRow['check_out_loc_name']);
						//$row[] = unserialize($aRow['location_trav']);
						//$row[] = unserialize($aRow['location_name_list']);
						//$row[] = "<a href='#'><i class='fa fa-map-marker'></i></a>";
		...
		public function reload_shiftwork_byfilter(){
			...
			foreach ($list_staff_id as $key => $value) {
				...
				foreach ($list_date as $kdbm => $day) {
					...
					$color = '';
					$list_shift = $this->timesheets_model->get_shift_work_staff_by_date($value, $day);
					// echo "<pre>";print_r($list_shift);die;
					foreach ($list_shift as $ss) {
						$data_shift_type = $this->timesheets_model->get_shift_type($ss);
						if($data_shift_type){
							if($color == ''){
		...
		*/
		...
		public function export_attendance_excel()
		{
			...
			if($this->input->post()){
				...
				$dep_name = $this->timesheets_model->dep_name($department_filter);
				if(empty($dep_name)){
					$dname = "All";
					}else{
					$dname = $dep_name->name;
				...
				$role_name = $this->timesheets_model->role_name($role_filter);
				if(empty($role_name)){
					$rname = "All";
					}else{
					$rname = $role_name->name;
				}
				$staff_filter = $this->input->post('staff');
				$state_filter = $this->input->post('state');
				if($this->input->post('state') == ""){
					$state_name = "All";
					}else{
					$state_name = $this->input->post('state');
				}
				$list = $this->timesheets_model->get_data_attendance_export($month_filter, $department_filter, $role_filter, $staff_filter, $state_filter);
				$selected_company_details    = $this->timesheets_model->get_selected_company_details();
				//$month      = date('m');
				$month      = substr($month_filter,5);
				$month_year = substr($month_filter,0,4);
				$days_in_month = cal_days_in_month(CAL_GREGORIAN, $month, $month_year);
				
				$set_col_tk = [];
				$set_col_tk[_l('staff_id')] =  'StaffID';
				$set_col_tk[_l('staff')] = 'StaffName';
				$set_col_tk['Department'] = 'Department';
				$widthst = [];
				$widthst[] = 10;
				$widthst[] = 40;
				for ($d = 1; $d <= $days_in_month; $d++) {
				...
				$address = $selected_company_details->ADDRESS1." ".$selected_company_details->ADDRESS2;
				$company_name = array("","","","","",$selected_company_details->FIRMNAME);
				$writer->writeSheetRow('Sheet1', $company_name, $style_c);
				$company_addr = array("","","","",$address);
				$writer->writeSheetRow('Sheet1', $company_addr, $style);
				$heading = array("","","Month : ",$month_filter,"Department : ",$dname,"Role : ",$rname,"State : ",$state_name);
				//$writer->writeSheetRow('Sheet1', $heading, $style2);
				$writer->writeSheetRow('Sheet1', $heading, $style);
				$writer->writeSheetRow('Sheet1', $writer_header, $style1);
		...
		*/
		...
		/**
			...
			* @return 
		*/
		public function history_check_in_out_report2(){
			if ($this->input->is_ajax_request()) {
				if($this->input->post()){
					$months_report = $this->input->post('report_months');
					
					//$staff_fillter = $this->input->post("staff_2_fillter");
					$staff_fillter2 = $this->input->post("staff_filter");
					/*$department_fillter = $this->input->post("department_2_fillter");
						$roles_fillter = $this->input->post("roles_2_fillter");
					...
					if(has_permission('report_management', '', 'view') || is_admin()){
						...
						if(!empty($staff_fillter2)){
							/*$staffid_list = implode(',', $staff_fillter2);
							$query .= 'staff_id in ('.$staffid_list.') and';*/
							$query .= 'staff_id = '.$staff_fillter2.' and';
						} 
					}
					else{
						$query .= 'staff_id = '.get_staff_user_id().' and';
					...
					$query .= ' staff_id in (SELECT staffid FROM '.db_prefix().'staff where active = '.$status.') and';
					...
					foreach ($rResult as $aRow) {
						...
						// End shift
						$name = unserialize($aRow['check_in_loc_name']);
						$sentence = str_replace(', India', '', $name);
						$row[] = $sentence;
						if($aRow['check_out']){
							$chack_out_tm = unserialize($aRow['check_out']);
							}else {
							$chack_out_tm = "";
						}
						...
						$check_out_loc = str_replace(', India', '', unserialize($aRow['check_out_loc_name']));
						$row[] = $check_out_loc;
						/*$sss = unserialize($aRow['location_trav']);
							...
							$last_value = end($t);
							$row[] = round($last_value, 2);
						}*/
						
						
						$travel_detail = get_travel_detail_by_staff_id($aRow['staff_id'],$aRow['date']);
						...
						$l_list = unserialize($aRow['location_name_list']);
						$l_list_new = str_replace(', India', '', $l_list);
						$l_list_new = str_replace('6kGfZZZxzK09GwwAfvDS2t0+RPFcoIyvHBana/OP6WyxSrImOLC9FMKhVFKd8lwS', '', $l_list_new);
						$l_list_new = str_replace('Panchayat.enayatngar.block.goraul.dist', '', $l_list_new);
						$l_array = explode("|", $l_list_new);
						$unique = array_unique($l_array);
						$first_threee_loc = array();
						$i = 1;
						foreach ($unique as $value) {
							# code...
							if($i<=4 && $i>1){
								
								array_push($first_threee_loc, $value);
							}
							
							$i++;
						}
						
						$f_list = implode(" | ",$unique);
						$first_threee_loc_new = implode(" | ",$first_threee_loc);
						//$row[] = $f_list;
						$row[] = $first_threee_loc_new;
						
						//$row[] = round($dist_cal_new, 2);
						$row[] = '<a href="' . admin_url('timesheets/reports_new/' . $aRow['date'].'/' . $aRow['staff_id']) . '" target="_blank">read more..</a>';
		...
		*/
		public function travel_report(){
			if ($this->input->is_ajax_request()) {
				if($this->input->post()){
					$months_report = $this->input->post('report_months2');
					
					//$staff_fillter = $this->input->post("staff_2_fillter");
					$staff_fillter2 = $this->input->post("staff_filter2");
					/*$department_fillter = $this->input->post("department_2_fillter");
						$roles_fillter = $this->input->post("roles_2_fillter");
					...
					if($months_report == 'custom'){
						$from_date = to_sql_date($this->input->post('report_from2'));
						$to_date   = to_sql_date($this->input->post('report_to2'));                                    
					}//12 thang qua 
					...
					foreach ($rResult as $aRow) {
						...
						$staff_detail = get_staff($aRow['staff_id']);
						
						$row[] = _d($aRow['date']);
						$row[] = unserialize($aRow['check_in']);
						$check_out = unserialize($aRow['check_out']);
						if($check_out){
							$check_out = $check_out;
							}else {
							$check_out = "";
						...
						$row[] = $check_out;
						$row[] = get_state_name_by_id($staff_detail->state);
						
						
						$row[] = get_city_name_by_city_id($staff_detail->city);
						
						$row[] = get_role_name_by_id($staff_detail->role);
						...
						$row[] = round($dist_cal_new, 2);
						$row[] = get_staff_full_name($staff_detail->team_manage);
		...
		/**
			...
			* @return 
		*/
		public function travel_report2(){
			if ($this->input->is_ajax_request()) {
				if($this->input->post()){
					$months_report = $this->input->post('report_months2');
		...
		public function history_check_in_out_report3(){
			if ($this->input->is_ajax_request()) {
				if($this->input->post()){
					$months_report = $this->input->post('report_months');
					...
					foreach ($rResult as $aRow) {
						...
						$f_list = implode(" | ",$unique);
						$first_threee_loc_new = implode(" |<br> ",$first_threee_loc);
						//$row[] = $f_list;
						$row[] = $first_threee_loc_new;
