	...
	*/
	class Hr_profile extends AdminController
	{
		...
		*/
		...
		{
			if (!has_permission_new('hrm_dashboard', '', 'view')) {
		...
		//================= Check in Out Staff Wise Date Wise ==========================
		public function DailyAttendanceRegister()
		{
			...
			$this->load->view('attendance/StaffWiseCheckInOut', $data);
		}
		
		public function GetStaffByDepartment()
		{
			$Department = $this->input->post('Department');
			$getStaff = $this->staff_model->GetStaffByDepartment($Department);
			// echo "<pre>";print_r($getStaff);die;
			$html = '';
			$i=1;
			foreach($getStaff as $key=>$val){
				
				$html .= '<tr class="StaffWiseAttendance">';
				$html .= '<td style="text-align:center;">' . $i . '</td>';
				$html .= '<td style="text-align:center;">' .$val["AccountID"]. '</td>';
				$html .= '<td class="AccountID">' .$val["firstname"].' '.$val["lastname"].'<input type="hidden" name="AccountID[]" id="AccountID" value="' .$val["AccountID"].'"></td>';
				$html .= '<td class="CheckInTime"><input type="time" name="CheckINTime[]" id="CheckINTime" class="form-control" ></td>';
				$html .= '<td class="CheckOutTime"><input type="time" name="CheckOutTime[]" id="CheckOutTime" class="form-control" ></td>';
				$html .= '</tr>';
				$i++;
			}
		...
		public function AttendanceSheet(){
			...
			if (!(has_permission('AttendanceSheet', '', 'view_own') || has_permission('AttendanceSheet', '', 'view') || is_admin())) {          
			...
			$this->load->view('attendance/AttendanceSheet', $data);
		}
		
		public function GetAttendanceSheet()
		{
			// Get the year and month from POST
			...
			$this->load->model('staff_model');
			$body_data = $this->staff_model->get();
			$Attendance_data = $this->hr_profile_model->GetMonthWiseAttendance(['month' => $month_input]);
			$get_Shift_data = $this->hr_profile_model->get_Shift_data(['month' => $month_input]);
			// echo "<pre>";print_r($get_Shift_data);die;
			$get_leaves_data = $this->hr_profile_model->get_leaves_data(['month' => $month_input]);
			$Holidays = $this->hr_profile_model->get_break_dates();
			// echo "<pre>";print_r($get_leaves_data);die;
			// $body_data = array();
			...
			$html .= '<th class="sortable" align="center">Emp Name</th>';
			$html .= '<th class="sortable" align="center">Designation</th>';
			
			// Add date columns dynamically
			for ($day = 1; $day <= $days_in_month; $day++) {
				$date = strtotime("$selected_year-$selected_month-$day");
				// $formatted_date = strtoupper(date('M-d', $date)); // Format: NOV-01
				$formatted_date = strtoupper(date('D - d', $date));
				$html .= '<th class="sortable" align="center">' . $formatted_date . '</th>';
			}
			...
			// Add rows dynamically (example placeholder)
			if (!empty($body_data)) {
				$sr_no = 1;
				foreach ($body_data as $row) {
					...
					$html .= '<td align="left"><b>'.$row['AccountID'].'<b></td>';
					$html .= '<td align="left"><b>' . $row['firstname']." ".$row['lastname']. '<b></td>';
					$html .= '<td align="left"><b>'.strtoupper($row['position_name']).'<b></td>';
					...
					for ($day = 1; $day <= $days_in_month; $day++) {
						...
						$found = false; // Track if attendance exists for this day
						
						
						// $Shift = $this->hr_profile_model->get_Shift_data($date);
						
						$WorkingHr = 0;
						foreach ($Attendance_data as $each) {
							if ($row['AccountID'] == $each['AccountID'] && $date == date('Y-m-d', strtotime($each['InDateTime']))) {
						...
						if($WorkingHr > 0){
							if($WorkingHr >= $row['working_hour']){
								$html .= '<td align="center" style="background-color:green;color:white;">P</td>';
								}else{
								$html .= '<td align="center" style="background-color:#fc0;color:black;">HD</td>';
							}
							}else{
							$chkSplShift = 0;
							foreach($get_Shift_data as $shift){
								if($shift['staff_id'] == $row['staffid'] && $shift['from_date'] <= $date && $shift['to_date'] >= $date){
									$chkSplShift++;
								}
							}
							...
							foreach($get_Shift_data as $shift){
								if($chkSplShift >0){
									if($shift['staff_id'] == $row['staffid'] && $shift['number'] == $dayOfWeekNumber){
										$IsShift++;
									}
									}else{
									if($shift['staff_id'] == 0 && $shift['number'] == $dayOfWeekNumber){
										$IsShift++;
									}
								}
							}
							...
							if($IsShift >0){
								...
								foreach($get_leaves_data as $leave){
									$startLeave = date('Y-m-d', strtotime($leave['start_time']));
									$EndLeave = date('Y-m-d', strtotime($leave['end_time']));
									// echo $startLeave;die;
									if($date >= $startLeave  && $date <= $EndLeave  && $leave['staff_id'] == $row['staffid']){
										$IsOnLeave ++;
										$text = "AL";
										
										$style = "background-color:#0c0;color:white;";
									}
								}
								
								$IsHoliday = 0;
								foreach($Holidays as $holiday){
									if($holiday['break_date'] == $date){
										$IsHoliday ++;
										if($holiday['off_type'] == "holiday"){
											$text = "HO";
										}
										if($holiday['off_type'] == "event_break"){
											$text = "EB";
										}
										if($holiday['off_type'] == "unexpected_break"){
											$text = "UB";
										}
										$style = "background-color:#00c;color:white;";
									}
								}
								
								if($IsHoliday > 0 || $IsOnLeave > 0){
									$html .= '<td align="center" style="'.$style.'">'.$text .'</td>';
									}else{
									if($date <= date('Y-m-d')){
										$html .= '<td align="center" style="background-color:red;color:white;">A</td>';
										}else{
										$html .= '<td align="center" style="'.$style.'">'.$text .'</td>';
									}
								}
								...
								$html .= '<td align="center" style="background-color:gray;color:white;">NS</td>';
							}
						}
				...
				$html .= '<tr><td colspan="' . ($days_in_month + 2) . '" align="center">No data found</td></tr>';
			}
		...
		public function ExportAttendanceSheet(){
			...
			if($this->input->post()){
				...
				$header_style = ['font' => 'Arial', 'font-size' => 12, 'font-style' => 'bold', 'fill' => '#D9D9D9', 'halign' => 'center'];
				$absent_style = ['font' => 'Arial', 'font-size' => 12, 'color' => '#FF0000', 'halign' => 'center']; // Red text for 'A'
				$present_style = ['font' => 'Arial', 'font-size' => 12, 'color' => '#008000', 'halign' => 'center']; // Green text for 'P'
				...
				$msg = "Report : ".$this->input->post('month');
				...
				$set_col_tk["Emp Name"] =  'Emp Name';
				$set_col_tk["Designation"] =  'Designation';
				for ($day = 1; $day <= $days_in_month; $day++) {
					$date = strtotime("$selected_year-$selected_month-$day");
					// $formatted_date = strtoupper(date('M-d', $date)); // Format: NOV-01
					$formatted_date = strtoupper(date('D - d', $date));
					$set_col_tk["$formatted_date"] =  $formatted_date;
				}
				...
				if (!empty($body_data)) {
					...
					foreach ($body_data as $row) {
						...
						$list_add = [];
						$list_add[] = $row['AccountID'];
						$list_add[] = $row['firstname']." ".$row['lastname'];
						$list_add[] = strtoupper($row['position_name']);
		...
		public function SaveUserAttendance()
		{
			if (!has_permission_new('DailyAttendanceRegister', '', 'create')) {
				access_denied('Invoice Items');
			}
			
			$data = array(
		    "UserAttendanceString" =>$this->input->post('UserAttendanceString'),
		    "date" =>$this->input->post('date'),
		    "Department" =>$this->input->post('Department'),
			);
			$return = $this->hr_profile_model->SaveUserAttendance($data);
			echo json_encode($return);
		}
		
		/* Get User Attendance / ajax */
		public function GetUserAttendance()
		{
			$date = $this->input->post('date');
			$AccountDetails                    = $this->hr_profile_model->GetUserAttendance($date);
			echo json_encode($AccountDetails);
		}
		
		public function ExportAttendanceRegister(){
			if(!class_exists('XLSXReader_fin')){
				require_once(module_dir_path(TIMESHEETS_MODULE_NAME).'/assets/plugins/XLSXReader/XLSXReader.php');
			}
			require_once(module_dir_path(TIMESHEETS_MODULE_NAME).'/assets/plugins/XLSXWriter/xlsxwriter.class.php');
			...
			if($this->input->post()){
				...
				$AttendanceDetails                    = $this->hr_profile_model->GetUserAttendance($this->input->post('date'));
				$stafflist = $this->staff_model->GetStaffByDepartment($this->input->post('Department'));
				...
				$msg = "Attendance Report : ".$this->input->post('date');
				...
				$set_col_tk["Check In"] =  'Check In';
				$set_col_tk["Check Out"] =  'Check Out';
				...
				if (!empty($stafflist)) {
					...
					foreach ($stafflist as $row) {
						...
						$list_add[] = $row['AccountID'];
						$list_add[] = $row['firstname']." ".$row['lastname'];
						foreach($AttendanceDetails as $each){
							if($each['AccountID'] == $row['AccountID']){
								$InTime = '';
								$OutTime = '';
								if(!empty($each['InDateTime'])){
									$InTime = date('g:i A', strtotime($each['InDateTime']));
								}
								if(!empty($each['OutDateTime'])){
									$OutTime = date('g:i A', strtotime($each['OutDateTime']));
								}
									$list_add[] = $InTime;
									$list_add[] = $OutTime;
							}
		...
		*/
		public function organizational_chart(){
			if (!has_permission_new('staffmanage_orgchart', '', 'view')) {
				access_denied('hr_profile');
			}
			$this->load->model('staff_model');
		...
		*/
		public function reception_staff(){
			...
			if (!is_admin() && !has_permission_new('hrm_reception_staff','','view')) {
		...
		*/
		public function setting()
		{
			...
			/*if(!has_permission_new('hrm_setting', '', 'delete') ) {
				access_denied('hr_profile');
			}*/
			...
			if(has_permission_new('hrm_salary_type', '', 'view') || has_permission_new('hrm_salary_type', '', 'edit') || has_permission_new('hrm_salary_type', '', 'create') || has_permission_new('hrm_salary_type', '', 'delete')){
				$data['tab'][] = 'salary_type';
			}
			if(has_permission_new('hrm_allowance_type', '', 'view') || has_permission_new('hrm_allowance_type', '', 'create') || has_permission_new('hrm_allowance_type', '', 'delete') || has_permission_new('hrm_allowance_type', '', 'edit')){
				$data['tab'][] = 'allowance_type';
			}
			//$data['tab'][] = 'procedure_retire';
			//$data['tab'][] = 'reception_staff';
			//$data['tab'][] = 'workplace';
			if(has_permission_new('hrm_hedquarter', '', 'view') || has_permission_new('hrm_hedquarter', '', 'create') ){
				$data['tab'][] = 'hedquarter';
			}
			/*if(is_admin()){
				$data['tab'][] = 'hr_profile_permissions';
			}*/
			if(has_permission_new('hrm_gen_setting', '', 'view') || has_permission_new('hrm_gen_setting', '', 'edit') ){
			...
			if($data['group'] == ''){
				...
				}elseif ($data['group'] == 'hedquarter') {
				$this->load->model('accounts_master_model');
				$data['company_detail'] = $this->accounts_master_model->get_company_detail();
				$data['hedquarter'] = $this->hr_profile_model->get_headquarter();
				$data['state'] = $this->hr_profile_model->get_state_list();
			}
		...
		*/
		public function delete_salary_form($id)
		{
			...
			if(!has_permission_new('hrm_setting', '', 'delete')  &&  !is_admin()) {
				access_denied('hr_profile');
			}
		...
		*/
		...
		{
			if (!has_permission_new('staffmanage_training', '', 'view') && !is_admin() ) {
		...
		*/
		...
		{
			if (!has_permission_new('staffmanage_training', '', 'view')) {
				access_denied('job_position');
			}
			if ($this->input->post()) {
				$data                    = $this->input->post();
				...
				if ($id == '') {
					if (!has_permission_new('staffmanage_training', '', 'create')) {
						access_denied('job_position');
					}
					...
					if (!has_permission_new('staffmanage_training', '', 'edit')) {
						access_denied('job_position');
					}
		...
		{
			if (!has_permission_new('staffmanage_training', '', 'edit') && !has_permission_new('staffmanage_training', '', 'create')) {
				echo json_encode([
				'success' => false,
		...
		{
			if (!has_permission('staffmanage_training', '', 'edit') && !has_permission_new('staffmanage_training', '', 'create')) {
				echo has_permission_new([
				'success' => false,
				'message' => _l('access_denied'),
				]);
				die();
		...
		*/
		...
		{
			if (!has_permission_new('staffmanage_job_position', '', 'view') && !is_admin() && !has_permission_new('staffmanage_job_position', '', 'view_own')) {
				access_denied('job_position');
			}
			$this->load->model('staff_model');
		...
		*/
		...
		{
			if (!has_permission_new('staffmanage_job_position', '', 'view') && !has_permission_new('staffmanage_job_position', '', 'view_own')) {
				access_denied('job_position');
			}
		...
		*/
		...
		{
			if (!has_permission_new('staffmanage_job_position', '', 'delete') && !is_admin()) {
				access_denied('job_position');
			}
		...
		*/
		public function job_position_salary_add_edit(){
			if (!has_permission_new('staffmanage_job_position', '', 'create')) {
				access_denied('job_position');
			}
		...
		*/
		public function add_new_reception()
		{
			if($this->input->post()){
				...
				if(isset($data['training_type'])){
					if($data['training_type'] && $data['training_type'] != ''){
						if($list_staff->job_position != '' && $data['training_type'] != ''){
							$list_training = $this->hr_profile_model->get_jp_interview_training($list_staff->job_position,$data['training_type']); 
							if(isset($list_training)){
								if($list_training){
									$this->hr_profile_model->add_training_staff($list_training,$list_staff->staffid);
									if(isset($list_staff->email)){
										if($list_staff->email!=''){
											$this->send_training_staff($list_staff->email, $list_staff->job_position, $data['training_type']);
										}
									}
								}
							}
		...
		/**
			...
			* @return [type]                
		*/
		public function send_training_staff($email,$position_id,$training_type=''){
			$list_position_training = $this->hr_profile_model->get_jp_interview_training($position_id,$training_type);
			if(isset($list_position_training)){
				if($list_position_training!=''){
					$data_training = $this->hr_profile_model->get_list_position_training_by_id_training($list_position_training->position_training_id);
		...
		*/
		public function get_percent_complete($id=''){
			if($id!=''){
				...
				$data_marks = $this->get_mark_staff($id);
		...
		/**
			...
			* @return array           
		*/
		public function get_mark_staff($id_staff){
			$array_training_point=[];
			$training_program_point=0;
			
			//Get the latest employee's training result.
			$trainig_resultset = $this->hr_profile_model->get_resultset_training($id_staff);
				...
				if(($data['staff']->job_position) && (is_numeric($data['staff']->job_position))){
					...
					if(isset($data['list_training_allocation'])){
						...
						if($data['list_training_allocation']){
							...
							//Get the latest employee's training result.
							$training_results=$this->get_mark_staff($data['staff']->staffid);
							
							$data['training_program_point'] = $training_results['training_program_point'];
							$data['staff_training_result'] 	= $training_results['staff_training_result'];
		...
		*/
		public function staff_infor()
		{
			...
			$data['staff_type'] = $this->hr_profile_model->get_staff_type();
		...
		*/
		public function attendance()
		{
			...
			$data['staff']     = $this->staff_model->get();
			$data['rootcompany'] = $this->clients_model->get_rootcompany();
			$this->load->view('attendance/manage_attendance', $data);
		}
		...
		*/
		public function delete_staff()
		{
			...
			if (has_permission_new('hrm_hr_records', '', 'delete')) {
		...
		*/
		public function member($id = '', $group = '')
		{
			...
			if ($id == '') {
				if ( !is_admin() && !has_permission_new('hrm_hr_records', '', 'create') && !has_permission_new('hrm_hr_records', '', 'edit')) {
					access_denied('staff');
				}
				$data['hr_profile_member_add'] = true;
				$title = _l('add_new', _l('staff_member_lowercase'));
				}else{
				if(get_staff_user_id() != $id && !is_admin() && !has_permission_new('hrm_hr_records', '', 'edit') && !has_permission_new('hrm_hr_records', '', 'view')){
					access_denied('staff');
				}
				...
				if($data['group'] == 'travel'){
					$data['travel_distance']   = $this->hr_profile_model->get_travel_distance($id);
				}
				if($data['group'] == 'training'){
					
					$data['training_allocation_min_point'] = 0 ;
					$data['list_training_allocation'] = $this->hr_profile_model->get_training_allocation_staff($id);
					...
					if(isset($data['list_training_allocation'])){
						...
						if($data['list_training_allocation']){
							...
							//Get the latest employee's training result.
							$training_results=$this->get_mark_staff($id);
		...
		*/
		public function upload_file()
		{
			...
			foreach($hr_profile_staff as $key => $attachment) {
				...
				if($attachment['staffid'] == get_staff_user_id() || is_admin() || has_permission_new('hrm_hr_records', '', 'edit')){
		...
		*/
		...
		{
			$file = $this->misc_model->get_file($attachment_id);
			if ($file->staffid == get_staff_user_id() || is_admin() || has_permission_new('hrm_hr_records', '', 'edit')) {
		...
		*/
		public function update_staff_profile(){
			...
			if ($data['id'] == '') {
				unset($data['id']);
				if (!has_permission_new('hrm_hr_records', '', 'create') && !has_permission_new('hrm_hr_records', '', 'edit') && !is_admin()) {
		...
		*/
		public function headquarter($id = '')
		{
			...
			if ($this->input->post()) {
				...
				if (!$this->input->post('id')) {
					$id = $this->hr_profile_model->add_headquarter($data);
					...
					redirect(admin_url('hr_profile/setting?group=hedquarter'));
					} else {
					$id = $data['id'];
					unset($data['id']);
					$success = $this->hr_profile_model->update_headquarter($data, $id);
					
					if ($success) {
						$message = _l('updated_successfully', _l('HeadQuarter'));
						set_alert('success',$message);
		...
		*/
		public function delete_headquarter($id)
		{
			...
			$response = $this->hr_profile_model->delete_headquarter($id);
			if ($response == true) {
				set_alert('success', _l('deleted', _l('hedquarter')));
				} else {
				set_alert('warning', _l('problem_deleting', _l('hedquarter')));
			}
			redirect(admin_url('hr_profile/setting?group=hedquarter'));
		}
		...
		*/
		public function delete_workplace($id)
		{
			...
			if (is_array($response) && isset($response['referenced'])) {
				...
				set_alert('warning', _l('Not Deleted Headquarter Assigned to some Sales team....'));
			}
			redirect(admin_url('hr_profile/setting?group=workplace'));
		}
		...
		public function hr_profile_permission_table()
		{
			if ($this->input->is_ajax_request()) {
				...
				foreach ($rResult as $aRow) {
					...
					if(has_permission_new('hrm_setting', '', 'edit')){
		...
		*/
		...
		{
			if (!has_permission_new('staffmanage_job_position', '', 'delete')) {
				access_denied('job_position');
			}
		...
		*/
		public function contracts($id = '')
		{
			...
			if (!has_permission_new('hrm_contract', '', 'view') && !is_admin()) {
		...
		*/
		public function contract($id = '')
		{
			...
			if ($this->input->post()) {
				...
				$count_file = 0;
				if ($id == '') {
					if (!has_permission_new('hrm_contract', '', 'create') && !is_admin()) {
						access_denied('staff_contract');
					}
					...
					if (!has_permission_new('hrm_contract', '', 'edit') && !is_admin()) {
						access_denied('staff_contract');
					}
		...
		*/
		public function dependent_persons()
		{
			...
			if( !is_admin() && !has_permission_new('hrm_dependent_person', '', 'view')){
		...
		*/
		...
		{
			if(!is_admin() && !has_permission_new('hrm_dependent_person','','create')) {
		...
		*/
		public function admin_delete_dependent_person($id)
		{
			...
			if (is_array($response) && isset($response['referenced'])) {
				...
				set_alert('success', _l('dependent_persons', _l('hr_deleted')));
				} else {
				set_alert('warning', _l('problem_deleting', _l('hr_dependent_persons')));
			}
			redirect(admin_url('hr_profile/dependent_persons'));
		...
		*/
		public function resignation_procedures(){
			$this->app_scripts->add('circle-progress-js','assets/plugins/jquery-circle-progress/circle-progress.min.js');
			if(!has_permission_new('hrm_procedures_for_quitting_work', '', 'view') && !is_admin() ){   
		...
		*/
		public function delete_resignation_procedure($id){
			...
			if(!has_permission_new('hrm_procedures_for_quitting_work', '', 'edit') && !is_admin() ){   
		...
		*/
		...
		{
			if (!has_permission_new('hr_manage_q_a', '', 'delete')) {
		...
		*/
		...
		{
			if (!has_permission_new('hrm_contract', '', 'delete') && !is_admin()) {
				access_denied('hrm');
			}
		...
		public function member_modal()
		{
			...
			if ($this->input->post('slug') === 'create') {
				...
				if(isset($staff_id)) {
					$data['member']  = $this->staff_model->get($staff_id);
					/*	$dd = 138;
					$data['city_name'] = get_city_name_id($dd);*/
				}
				...
				$data['positions'] = $this->hr_profile_model->get_job_position();
				$data['workplace'] = $this->hr_profile_model->get_workplace();
				$data['staff_hierarchy'] = $this->hr_profile_model->get_staff_hierarchy($staff_id);
				$data['asm_list'] = $this->hr_profile_model->get_all_asm($asm_id);
				$data['rsm_list'] = $this->hr_profile_model->get_all_rsm($rsm_id);
				$data['ase_list'] = $this->hr_profile_model->get_all_ase($ase_id);
				$data['allso_list'] = $this->hr_profile_model->get_all_so2($so_id);
				//$data['not_assigned_ase'] = $this->hr_profile_model->get_all_ase2($ase_id);
				$data['not_assigned_an_selected_asm'] = $this->hr_profile_model->get_not_assigned_an_selected_asm($asm_id, $staff_id);
				$data['not_assigned_an_selected_ase'] = $this->hr_profile_model->get_not_assigned_an_selected_ase($ase_id, $staff_id);
				$data['not_assigned_an_selected_so'] = $this->hr_profile_model->get_not_assigned_an_selected_so($so_id, $staff_id);
				$data['not_assigned_an_selected_dist'] = $this->hr_profile_model->get_not_assigned_an_selected_distributor($staff_id);
				/*$data['not_assigned_rsm'] = $this->hr_profile_model->get_not_assigned_rsm();
					//$data['not_assigned_asm'] = $this->hr_profile_model->get_all_asm();
					$data['not_assigned_ase'] = $this->hr_profile_model->get_all_ase2($data['member']->job_position);
				$data['not_assigned_so'] = $this->hr_profile_model->get_all_so($data['member']->job_position);*/
		...
		*/
		public function new_member($id = '')
		{
			...
			$title = _l('add_new', _l('staff_member_lowercase'));
			if($id){
				$title = _l('edit', _l('staff_member_lowercase'));
				//$data['member'] = $this->hr_profile_model->get_staff($id,['tblaccountbalances.PlantID' => $selected_company,'tblaccountbalances.FY' => $fy]);
			...
			$data['SLDTYPE'] = $this->hr_profile_model->SLDTYPE();
			$data['OfficeLocation'] = $this->hr_profile_model->GetOfficeLocation();
			$data['SubGroup'] = $this->clients_model->VendorActSubgroup();
			$this->load->view('hr_record/new_member', $data);
		}
		
		public function AddEditStaff($id = '')
		{
			if (!has_permission_new('hrm_hr_records', '', 'create')) {
				access_denied('staff');
			}
			...
			$this->load->model('clients_model');
			$data['StaffType'] = $this->hr_profile_model->GetStaffType();
			$data['departments']   = $this->departments_model->get();
			$data['title']         = $title;
			$data['list_staff'] = $this->staff_model->get();
			$data['state'] = $this->staff_model->get_state();
			$data['rootcompany'] = $this->clients_model->get_rootcompany();
			$this->load->view('hr_record/AddEditStaff', $data);
		}
		...
		public function GetAccountDetailByID()
		{
			...
			$AccountDetails = $this->hr_profile_model->get_AccountDetails($AccountID);
			//print_r($AccountDetails); die;
			echo json_encode($AccountDetails);
		}
		
		/* Get Account Details by AccountID / POPUP */
		public function GetAllStaffList()
		{
			$AccountDetails = $this->hr_profile_model->GetAllStaffList($AccountID);
			$html = "";
			foreach($AccountDetails as $key=>$val){
				$html .= '<tr class="get_AccountID" data-id="'.$val["AccountID"].'">';
				$html .= "<td>".$val["AccountID"]."</td>";
				$html .= "<td>".$val["firstname"]." ".$val["lastname"]."</td>";
				$html .= "<td>".$val["SubActGroupName"]."</td>";
				$html .= "<td>".$val["name"]."</td>";
				$html .= "<td>".$val["position_name"]."</td>";
		...
		{
			$id=$this->input->post('id'); 
			$district_data = $this->hr_profile_model->get_city_by_id($id);
		...
		public function head_quarter()
		{
			...
			if ($this->input->is_ajax_request()) {
				...
				if ($data['id'] == '') {
					unset($data['id']);
					$id      = $this->hr_profile_model->add_head_quarter($data);
					$message = $id ? _l('added_successfully', 'Head Quarter') : '';
					echo json_encode([
		...
		//--------------------------------------------------
		public function select_quarter()
		{
			$id=$this->input->post('id'); 
			$quarter_data = $this->hr_profile_model->get_quarter_by_id($id);
			echo json_encode($quarter_data);
		}
		
		//--------------------------------------------------
		public function get_all_rsm()
		{
			$id= 4; 
			$all_asm = $this->hr_profile_model->get_all_rsm($id);
			
			
			echo json_encode($all_asm);
			//return $all_asm;
		}
		
		//--------------------------------------------------
		public function get_all_asm()
		{
			$id= 5; 
			$all_asm = $this->hr_profile_model->get_all_asm($id);
		...
		//--------------------------------------------------
		public function get_all_asm2()
		{
			$id= 5; 
			$all_asm = $this->hr_profile_model->get_all_asm2($id);
		...
		//--------------------------------------------------
		public function get_all_ase()
		{
			$id= 6; 
			$all_ase = $this->hr_profile_model->get_all_ase($id);
			
			
			echo json_encode($all_ase);
			//return $all_asm;
		}
		
		//--------------------------------------------------
		public function get_all_so2()
		{
			$id= 8; 
			$all_so = $this->hr_profile_model->get_all_so2($id);
			
			
			echo json_encode($all_so);
			//return $all_asm;
		}
		
		//--------------------------------------------------
		public function get_all_ase2()
		{
			$id= 6; 
			$all_ase = $this->hr_profile_model->get_all_ase2($id);
		...
		{
			//$id= 6; 
			$all_distributor = $this->hr_profile_model->get_all_distributor();
			
			
			echo json_encode($all_distributor);
			//return $all_asm;
		}
		...
		{
			$id= 8; 
			$all_ase = $this->hr_profile_model->get_all_so($id);
		...
		*/
		...
		{
			if (!has_permission_new('hrm_hr_records', '', 'view') && get_staff_user_id() != $id) {
				access_denied('staff');
			}
			...
			if ($this->input->post()) {
				...
				$fy = $this->session->userdata('finacial_year');
				$data = $this->input->post();
				if($this->input->post('company_id1') == ""){
					$_company_assigned = array($selected_company);
					}else{
					$_company_assigned = $this->input->post('company_id1');
				}
				$comp_ids= serialize($_company_assigned);
				//$_company_assigned = $data['company_id1'];
				unset($data['company_id1']);
				$data['staff_comp'] = $comp_ids;
				/*echo "<pre>";
					print_r($data);
				die;*/
				...
				if ($id == '') {
					...
					if (!has_permission_new('hrm_hr_records', '', 'edit') && get_staff_user_id() != $id) {
						access_denied('staff');
					}
					...
					if (is_array($response)) {
						...
						if($staff_user_id == "3"){
							$check_account = $this->hr_profile_model->get_account_balance_details($selected_company,$data['AccountID'],$fy);
							if($check_account > 0){
								$act_bal = array(
                                'BAL1' => $data['openingbal'],
                                'UserID2' => $UserID,
								...
								$this->db->where('FY', $fy);
								$this->db->update(db_prefix() . 'accountbalances', $act_bal);
								}else{
								$this->db->insert(db_prefix() . 'accountbalances', [
					...
					redirect(admin_url('hr_profile/new_member/'.$id));
				}
			}
		...
		*/
		...
		{
			if(!has_permission_new('hrm_report', '', 'view') && !is_admin() ){   
		...
		public function hrm_delete_bulk_action()
		{
			...
			if ($this->input->post()) {
				...
				switch ($rel_type) {
					...
					case 'hrm_staff':
					if (!has_permission_new('hrm_hr_records', '', 'delete') && !is_admin()) {
						access_denied('hr_hr_profile');
					}
					break;
					
					case 'hrm_training_library':
					if (!has_permission_new('staffmanage_training', '', 'delete') && !is_admin()) {
						access_denied('hr_hr_profile');
					}
					break;
					...
					if (!has_permission_new('hr_manage_q_a', '', 'delete') && !is_admin()) {
						access_denied('hr_hr_profile');
					}
					break;
					
					case 'hrm_reception_staff':
					if (!has_permission_new('hrm_reception_staff', '', 'delete') && !is_admin()) {
						access_denied('hr_hr_profile');
					}
					break;
					
					case 'hrm_resignation_procedures':
					if (!has_permission_new('hrm_procedures_for_quitting_work', '', 'delete') && !is_admin()) {
						access_denied('hr_hr_profile');
					}
					break;
		...
		public function hrm_delete_bulk_action_v2()
		{
			...
			if ($this->input->post()) {
				...
				switch ($rel_type) {
					...
					if (!has_permission_new('hrm_dependent_person', '', 'delete') && !is_admin()) {
						access_denied('hr_hr_profile');
					}
					break;
		...
		*/
		...
		//table load for head_quarter
		public function load_table_for_headquarter(){
			
			$data =$this->hr_profile_model->headquarter_table_data($this->input->post());
			// print_r($data);die;
			$html ='';
			if(count($data) > 0){
				...
				foreach($data as $w){
					...
					$html.= '<td align="">'.get_state_name_by_id($w['state_id']).'</td>';
					$html.= '<td>'.html_entity_decode($w['name']).'</td>';
					if($w['status'] == "1"){
						$html.= '<td>Active</td>';
						}else{
						$html.= '<td>Deactive</td>';
					...
					$html.= '<td>';
					if(is_admin() || has_permission_new('hrm_setting','','edit')) {
						$html.= '<a href="#" onclick="edit_workplace(this, "'.html_entity_decode($w['id']).'"); return false" data-name="'.html_entity_decode($w['name']).'" data-state="'. html_entity_decode($w['state_id']).'" data-status="'.html_entity_decode($w['status']).'" class="btn btn-default btn-icon"><i class="fa fa-pencil-square-o"></i></a>';
					} 
					if(is_admin() || has_permission_new('hrm_setting','','delete')) {
						$html.= '<a href="'. admin_url('hr_profile/delete_headquarter/'.$w['id']).'" class="btn btn-danger btn-icon _delete"><i class="fa fa-remove"></i></a>';
					} 
		...
		public function export_headquarter_list(){
         	if(!class_exists('XLSXReader_fin')){
				require_once(module_dir_path(TIMESHEETS_MODULE_NAME).'/assets/plugins/XLSXReader/XLSXReader.php');
			}
			require_once(module_dir_path(TIMESHEETS_MODULE_NAME).'/assets/plugins/XLSXWriter/xlsxwriter.class.php');
			...
			if($this->input->post()){
				...
				if($this->input->post('status_id') == 1){
    		    	$msg = "Headquarter List State: ".$this->input->post('state_id')." status: Active ";
					}else{
    		    	$msg = "Headquarter List State: ".$this->input->post('state_id')." status: Deactive ";
				}
				...
				$set_col_tk["Head Quarter"] = 'Head Quarter';
				...
				foreach ($data as $k => $w) {
					...
					$list_add = [];
					$list_add[] = get_state_name_by_id($w['state_id']);
					$list_add[] = html_entity_decode($w['name']);
					if($w['status'] == "1"){
						$list_add[] = "Active";
						}else{
						$list_add[] = "Deactive";
				...
				$filename = 'Headquarter List.xlsx';
				$writer->writeToFile(str_replace($filename, TIMESHEETS_PATH_EXPORT_FILE.$filename, $filename));
				echo json_encode([
    			'site_url'          => site_url(),
    			'filename'          => TIMESHEETS_PATH_EXPORT_FILE.$filename,
		...
		//staff info code here
		public function load_data_table(){
			$data =$this->hr_profile_model->staff_table_data($this->input->post());
			// print_r($data);die;
			if($this->input->post('hr_profile_report_to') != ''){
				$this->db->select('tblstaff.firstname ,tblstaff.lastname');
				$this->db->from(db_prefix() . 'staff');
				$this->db->where('tblstaff.team_manage',$data['hr_profile_report_to']);  
				$query = $this->db->get();
				$hr_profile_report_to = $query->row_array();
			}
			
			
			if($this->input->post('hr_profile_deparment') != ''){
				$this->db->select('tbldepartments.name');
				$this->db->from(db_prefix() . 'departments');
				$this->db->where('tbldepartments.departmentid',$this->input->post('hr_profile_deparment'));  
				$query = $this->db->get();
				$hr_profile_deparment = $query->row_array();
			}
			...
			if($this->input->post('staff_role') != ''){
            	...
            	$staff_role_where = implode(",",$staff_role_where);
				$this->db->select('tblhr_job_position.position_name');
				$this->db->from(db_prefix() . 'hr_job_position');
				$this->db->where_in('tblhr_job_position.position_id',$staff_role_where,'false');  
				$query = $this->db->get();
				$staff_role_data = $query->result_array();
				
				$staff_role_where_name =array();
            	foreach ($staff_role_data as $staff_role_data_a) {
					
					array_push($staff_role_where_name, $staff_role_data_a['position_name']);
					
				}
            	$staff_role_data_array = implode(",",$staff_role_where_name);
			}
            
			if($this->input->post('staff_type') != ''){
                $staff_type_role_where =array();
            	$staff_type_role      = $this->input->post('staff_type');
            	foreach ($staff_type_role as $staff_id) {
            		if($staff_id != '')
            		{
            			array_push($staff_type_role_where, $staff_id);
					}
				}
            	$staff_type_role_where_d = implode(",",$staff_type_role_where);
				$this->db->select('tblroles.name');
				$this->db->from(db_prefix() . 'roles');
				$this->db->where_in('tblroles.roleid',$staff_type_role_where_d,'false');  
				$query = $this->db->get();
				$staff_type_data = $query->result_array();
				
				$staff_type_where_name =array();
            	foreach ($staff_type_data as $staff_type_data_a) {
					
					array_push($staff_type_where_name, $staff_type_data_a['name']);
					
				}
            	$staff_type_data_array = implode(",",$staff_type_where_name);
			}
			...
			if(count($data) > 0){
				...
				foreach($data as $aRow){
					$html.= '<tr>';
					$html.= '<td>'.$aRow['AccountID'].'</td>';
					$html.= '<td class="sorting_1"><a href="'.admin_url().'/hr_profile/AddEditStaff/" target="_blank">'.$aRow['firstname'].' '.$aRow['lastname'].'</a></td>';
					...
					//$html.= '<td align="">'.$stf_comp_name.'</td>';
					$html.= '<td>'.$aRow['phonenumber'].'</td>';
					$html.= '<td>'.$aRow['state_name'].'</td>';
					$html.= '<td>'.$aRow['reported_f_n'].' '.$aRow['reported_l_n'].'</td>';
					/*if($aRow['staffid'] != ''){
						$team = $this->hr_profile_model->get_staff_departments($aRow['staffid']);
						$str = '';
						$j = 0;
						foreach ($team as $value) {
						$j++;
						$str .= '<div style="width:100%;">'.$value['name'];
			...
			$data_array = array('html'=>$html);
			
			if($this->input->post('hr_profile_report_to') != ''){
				$data_array['hr_profile_report_to'] =$hr_profile_report_to;
			}
			if($this->input->post('hr_profile_deparment') != ''){
				$data_array['hr_profile_deparment'] =$hr_profile_deparment['name'];
			}
			if($this->input->post('staff_role') != ''){
				$data_array['staff_role'] =$staff_role_data_array;
			}
			if($this->input->post('staff_type') != ''){
				$data_array['staff_type'] =$staff_type_data_array;
			}
			if($this->input->post('hr_profile_state') != ''){
				$data_array['hr_profile_state'] =$this->input->post('hr_profile_state');
			}
		...
		public function export_staff_info(){
         	if(!class_exists('XLSXReader_fin')){
				require_once(module_dir_path(TIMESHEETS_MODULE_NAME).'/assets/plugins/XLSXReader/XLSXReader.php');
			}
			require_once(module_dir_path(TIMESHEETS_MODULE_NAME).'/assets/plugins/XLSXWriter/xlsxwriter.class.php');
			...
			if($this->input->post()){
				...
				/*	$msg = "Staff info filter ";
					$filter = array($msg);
					$writer->markMergedCell('Sheet1', $start_row = $j, $start_col = 0, $end_row = $j, $end_col = 12);  //merge cells
					$writer->writeSheetRow('Sheet1', $filter);
				$j++;*/
				if($this->input->post('hr_profile_report_to') != ''){
					$msg = "Reported : ".$hr_profile_report_to['firstname']." ".$hr_profile_report_to['lastname'];
					$filter = array($msg);
					$writer->markMergedCell('Sheet1', $start_row = $j, $start_col = 0, $end_row = $j, $end_col = 12);  //merge cells
					$writer->writeSheetRow('Sheet1', $filter);
					$j++;
				...
				if($this->input->post('hr_profile_state') != ''){
					$msg1 = "State : ".$this->input->post('hr_profile_state');
					$filter1 = array($msg1);
					$writer->markMergedCell('Sheet1', $start_row = $j, $start_col = 0, $end_row = $j, $end_col = 12);  //merge cells
				...
				if($this->input->post('hr_profile_deparment') != ''){
					$msg2 = "Department : ".$hr_profile_deparment['name'];
					$filter2 = array($msg2);
					$writer->markMergedCell('Sheet1', $start_row = $j, $start_col = 0, $end_row = $j, $end_col = 12);  //merge cells
				...
				if($this->input->post('staff_role') != ''){
					$msg3 = "Designation : ".$staff_role_data_array;
					$filter3 = array($msg3);
					$writer->markMergedCell('Sheet1', $start_row = $j, $start_col = 0, $end_row = $j, $end_col = 12);  //merge cells
				...
				if($this->input->post('staff_type') != ''){
					$msg5 = "Staff Type : ". $staff_type_data_array;
					$filter5 = array($msg5);
					$writer->markMergedCell('Sheet1', $start_row = $j, $start_col = 0, $end_row = $j, $end_col = 12);  //merge cells
				...
				//$set_col_tk["Company Name"] =  'Company Name';
				$set_col_tk["Mobile"] =  'Mobile';
				$set_col_tk["State"] =  'State';
				$set_col_tk["Report To"] =  'Report To';
				//$set_col_tk["Departments"] =  'Departments';
				$set_col_tk["Designation"] =  'Designation';
				...
				foreach ($data as $k => $aRow) {
					...
					$list_add = [];
					$list_add[] = $aRow['AccountID'];
					$list_add[] = $aRow['firstname'].' '.$aRow['lastname'];
            		/*$staff_company = unserialize($aRow['staff_comp']);
						$stf_comp_name = "";
						$j =1;
						foreach ($staff_company as $key => $value) {
					...
					$list_add[] = $stf_comp_name;*/
					$list_add[] = $aRow['phonenumber'];
					$list_add[] = $aRow['state_name'];
					$list_add[] = $aRow['reported_f_n'].' '.$aRow['reported_l_n'];
		...
		public function SaveAccountID($id = '')
		{
			...
			//print_r($AccountDetails); die;
			$AccountDetails = $this->hr_profile_model->SaveAccountDetails($AccountDetails);
			//print_r($AccountDetails); die;
			echo json_encode($AccountDetails);
		}
		...
		{
			$AccountDetails = $this->input->post();
			$AccountDetails = $this->hr_profile_model->UpdateAccountDetails($AccountDetails);
			echo json_encode($AccountDetails);
		}
		//===================== Get Job Position List By Department ====================
		public function job_position_by_id()
		{
			$id = $this->input->post('value'); 
			$job_positions =$this->hr_profile_model->job_position_by_id($id);
			// print_r($job_positions);
			// die;
