...
class Date
{
    ...
    public static function timeDelayString($date, $localization = [])
    {
        $defaultLocalization['time_delay_just_now']  = 'Just now';
        $defaultLocalization['time_delay_minute']    = 'one minute delay';
        $defaultLocalization['time_delay_minutes']   = '%s minutes delay';
        $defaultLocalization['time_delay_hour']      = 'an hour delay';
        $defaultLocalization['time_delay_hours']     = '%s hrs delay';
        $defaultLocalization['time_delay_yesterday'] = 'yesterday';
        $defaultLocalization['time_delay_days']      = '%s days delay';
        $defaultLocalization['time_delay_week']      = 'a week delay';
        $defaultLocalization['time_delay_weeks']     = '%s weeks delay';
        $defaultLocalization['time_delay_month']     = 'a month delay';
        $defaultLocalization['time_delay_months']    = '%s months delay';
        $defaultLocalization['time_delay_year']      = 'one year delay';
        $defaultLocalization['time_delay_years']     = '%s years delay';

        $localization = array_merge($defaultLocalization, $localization);

        $time_delay     = strtotime($date);
        $cur_time     = time();
        $time_elapsed = $cur_time - $time_delay;
        $seconds      = $time_elapsed;
        $minutes      = round($time_elapsed / 60);
        $hours        = round($time_elapsed / 3600);
        $days         = round($time_elapsed / 86400);
        ...
        // Seconds
        if ($seconds <= 60) {
            return $localization['time_delay_just_now'];
        }

        //Minutes
        elseif ($minutes <= 60) {
            if ($minutes == 1) {
                return $localization['time_delay_minute'];
            }

            return sprintf($localization['time_delay_minutes'], $minutes);
        }
        //Hours
        elseif ($hours <= 24) {
            if ($hours == 1) {
                return $localization['time_delay_hour'];
            }

            return sprintf($localization['time_delay_hours'], $hours);
        }
        //Days
        elseif ($days <= 7) {
            if ($days == 1) {
                return $localization['time_delay_yesterday'];
            }

            return sprintf($localization['time_delay_days'], $days);
        }
        //Weeks
        elseif ($weeks <= 4.3) {
            if ($weeks == 1) {
                return $localization['time_delay_week'];
            }

            return sprintf($localization['time_delay_weeks'], $weeks);
        }
        //Months
        elseif ($months <= 12) {
            if ($months == 1) {
                return $localization['time_delay_month'];
            }

            return sprintf($localization['time_delay_months'], $months);
        }

        //Years
        if ($years == 1) {
            return $localization['time_delay_year'];
        }

        return sprintf($localization['time_delay_years'], $years);
    }
