<?php
	...
	abstract class App_import
	{
		...
		*/
		...
		public function setDatabaseFieldsStatementWise($fields)
		{
			$this->databaseFieldsStatement = $fields;
			
			return $this;
		}
		public function setDatabaseFieldsItemwise($fields)
		{
			
			$this->databaseFieldsItemWise = $fields;
		...
		*/
		protected function getImportableDatabaseFields()
		{
			...
			$fields = [];
			// print_r($this->databaseFields);die;
		...
		protected function getImportableStatementDatabaseFields()
		{
			if (!property_exists($this, 'notImportableFields')) {
				return $this->databaseFieldsStatement;
			}
			
			$fields = [];
			// print_r($this->databaseFields);die;
			foreach ($this->databaseFieldsStatement as $field) {
				if (in_array($field, $this->notImportableFields)) {
					continue;
				}
				$fields[] = $field;
		...
		protected function getImportableDatabaseFieldsItemWise()
		{
			if (!property_exists($this, 'notImportableFields')) {
				return $this->databaseFieldsItemWise;
			}
			
			$fields = [];
			
			foreach ($this->databaseFieldsItemWise as $field) {
				if (in_array($field, $this->notImportableFields)) {
					continue;
				}
				$fields[] = $field;
		...
		*/
		...
		{
			return property_exists($this, 'notImportableFields') ? $this->notImportableFields : ['PlantID','state_id'];
		}
		...
		*/
		...
		public function downloadSampleFormHtmlItemWise()
		{
			$form = '';
			$form .= form_open($this->ci->uri->uri_string());
			$form .= form_hidden('download_sample_itemwise', 'true');
			$form .= '<button type="submit" class="btn btn-success">Download Sample</button>';
			$form .= '<hr />';
			$form .= form_close();
		...
		public function downloadStatementSampleFormHtml()
		{
			$form = '';
			$form .= form_open($this->ci->uri->uri_string());
			$form .= form_hidden('download_statement_sample', 'true');
			$form .= '<button type="submit" class="btn btn-success">Download Sample</button>';
			$form .= '<hr />';
			$form .= form_close();
		...
		*/
		...
		public function importGuidelinesInfoHtmlItemWise()
		{
			$html = '<ul>';
			
            $html .= '<li class="" >1. In Downloaded sample file please fill only <b style="color:red;">Item Code</b> infront of Distributor Id You Want To Update From excel</li>';
		...
		*/
		public function downloadSample()
		{
			...
			foreach ($this->getImportableDatabaseFields() as $field) {
				if($field == "PlantID " || $field == "state_id  " || $field == "distributor_id  " || $field == "effective_date  " || $field == "UserId  " || $field == "UserID2  "){}else{
		...
		public function downloadSampleStatement()
		{
			$totalSampleFields = 0;
			$dbFieldKeys       = [];
			...
			header('Expires: 0');
			header('Content-Type: application/csv');
			header('Content-Disposition: attachment; filename="sample_statement_import_file.csv";');
			header('Content-Transfer-Encoding: binary');
			foreach ($this->getImportableStatementDatabaseFields() as $field) {
		...
		public function downloadSampleItemWise()
		{
			$totalSampleFields = 0;
			$dbFieldKeys       = [];
			...
			header('Expires: 0');
			header('Content-Type: application/csv');
			header('Content-Disposition: attachment; filename="sample_rate_import_itemwise_file.csv";');
			header('Content-Transfer-Encoding: binary');
			
			foreach ($this->getImportableDatabaseFieldsItemWise() as $field) {
				if($field == "PlantID " || $field == "state_id  " || $field == "distributor_id  " || $field == "effective_date  " || $field == "UserId  " || $field == "UserID2  "){}else{
					echo '"' . $this->formatFieldNameForHeading($field) . '",';
					$dbFieldKeys[$totalSampleFields] = $field;
					$totalSampleFields++;
			...
			$totalSampleRows = count($DistributorList);
		...
		public function DistributorList()
		{
			...
			return $this->ci->db->get(db_prefix().'customers_groups')->result_array();
		}
		...
		*/
		...
		private function setDefaultImportGuidelinesInfo()
		{
			...
			$this->addImportGuidelinesInfo('In Downloaded sample file please fill only two column one is <b>Item Code</b> in excel list name as Item Id and anather is <b>Basic rate </b>in excel sheet name as assigned rate and <br> other column leave it blank..');
		}
