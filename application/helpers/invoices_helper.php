...
/**
 * Format challan number based on description
 * @param  mixed $id
 * @return string
 */
function format_challan_number($id)
{
    $CI = &get_instance();
 ...
 * @return string
 */
function format_transaction_number($id)
{
    $CI = &get_instance();

    $CI->db->select('tras_date,voucher_id,prefix,number_format')
    ...
    /*if ($invoice->status == Invoices_model::STATUS_DRAFT) {
        ...
        $number = sales_number_format($invoice->voucher_id, $invoice->number_format, $invoice->prefix, $invoice->tras_date);
    //}
...
function get_challan_where_sql_for_staff($staff_id)
{
    $CI                                 = &get_instance();
    $has_permission_view_own            = has_permission('challan', '', 'view_own');
    //$allow_staff_view_invoices_assigned = get_option('allow_staff_view_invoices_assigned');
    $whereUser                          = '';
    if ($has_permission_view_own) {
        $whereUser = '((' . db_prefix() . 'challanmaster.UserID=' . $CI->db->escape_str($staff_id) . ' AND ' . db_prefix() . 'challanmaster.UserID IN (SELECT staff_id FROM ' . db_prefix() . 'staff_permissions WHERE feature = "challan" AND capability="view_own"))';
...
function get_order_where_sql_for_staff($staff_id)
{
    $CI                                 = &get_instance();
    ...
    $whereUser                          = '';
    if ($has_permission_view_own) {
        $whereUser = '((' . db_prefix() . 'ordermaster.UserID=' . $CI->db->escape_str($staff_id) . ' AND ' . db_prefix() . 'ordermaster.UserID IN (SELECT staff_id FROM ' . db_prefix() . 'staff_permissions WHERE feature = "orders" AND capability="view_own"))';
...
function get_main_group_name($id = '')
{
    ...
    $CI->db->select('name');
    $CI->db->where('id', $id);
    $contact = $CI->db->get(db_prefix() . 'items_main_groups')->row();
...
function get_main_group()
{
    ...
    $CI->db->select("*");
    //$CI->db->where('id', $id);
    $main_group_list = $CI->db->get(db_prefix() . 'items_main_groups')->result_array();

    return $main_group_list;
}
