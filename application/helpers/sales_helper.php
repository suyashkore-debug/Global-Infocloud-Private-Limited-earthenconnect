<?php
	...
	*/
	...
	function get_order_list($challan_id)
	{
		...
		$fy = $CI->session->userdata('finacial_year');
		$selected_company = $CI->session->userdata("root_company");
		$CI->db->select(db_prefix() .'ordermaster.*,'.db_prefix() . 'clients.company,'.db_prefix() . 'clients.state,'.db_prefix() . 'clients.vat,'.db_prefix() . 'salesmaster.BillAmt,'.db_prefix() . 'salesmaster.Transdate AS Saledate,tblStationMaster.StationName');
		...
		$CI->db->join(db_prefix() .'StationMaster', db_prefix() .'StationMaster.id = '.db_prefix() .'clients.StationName','LEFT');
		...
		$CI->db->order_by(db_prefix() .'ordermaster.AccountID2', 'ASC');
		return $CI->db->get()->result_array();
	}
	function get_order_list_groupByCustomer($challan_id)
	{
		
		$CI = &get_instance();
		$fy = $CI->session->userdata('finacial_year');
		$selected_company = $CI->session->userdata("root_company");
		$CI->db->select(db_prefix() .'ordermaster.*,'.db_prefix() . 'clients.company,'.db_prefix() . 'clients.state,'.db_prefix() . 'StationMaster.StationName,'.db_prefix() . 'clients.vat,'.db_prefix() . 'salesmaster.BillAmt,'.db_prefix() . 'salesmaster.Transdate AS Saledate,'.db_prefix() . 'salesmaster.DeliveryPoint');
		$CI->db->from(db_prefix() .'ordermaster');
		$CI->db->join(db_prefix() .'clients', db_prefix() .'clients.AccountID = '.db_prefix() .'ordermaster.AccountID AND '.db_prefix() .'clients.PlantID = '.db_prefix() .'ordermaster.PlantID');
		$CI->db->join(db_prefix() .'salesmaster', db_prefix() .'salesmaster.SalesID = '.db_prefix() .'ordermaster.SalesID AND '.db_prefix() .'salesmaster.PlantID = '.db_prefix() .'ordermaster.PlantID');
		$CI->db->join(db_prefix() .'StationMaster', db_prefix() .'StationMaster.id = '.db_prefix() .'clients.StationName','LEFT');
		...
		$CI->db->where(db_prefix() .'ordermaster.FY', $fy);
		$CI->db->where(db_prefix() .'ordermaster.PlantID', $selected_company);
		$CI->db->group_by(db_prefix() .'ordermaster.AccountID');
		$CI->db->order_by(db_prefix() .'salesmaster.DeliveryPoint','ASC');
		return $CI->db->get()->result_array();
	}
	...
	function get_item_by_order_id($order_id)
	{
		...
		$selected_company = $CI->session->userdata("root_company");
		$amt = 0.00;
		$CI->db->select(db_prefix() .'history.*,'.db_prefix() . 'items.description,'.db_prefix() . 'items.unit,tblArticleMaster.ArticleName,IFNULL('.db_prefix() .'history.eOrderQty / '.db_prefix() .'history.CaseQty,'.db_prefix() .'history.OrderQty / '.db_prefix() .'history.CaseQty) as caseqty,IFNULL('.db_prefix() .'history.eOrderQty,'.db_prefix() .'history.OrderQty) as qty');
        $CI->db->from(db_prefix() .'history');
        $CI->db->join(db_prefix() .'items', db_prefix() .'items.item_code = '.db_prefix() .'history.ItemID AND '.db_prefix() .'items.PlantID = '.db_prefix() .'history.PlantID');
        $CI->db->join(db_prefix() .'ArticleMaster', db_prefix() .'ArticleMaster.ItemID = '.db_prefix() .'history.ItemID AND '.db_prefix() .'ArticleMaster.AccountID = '.db_prefix() .'history.AccountID',"LEFT");
        $CI->db->where(db_prefix() .'history.OrderID', $order_id);
        $CI->db->where(db_prefix() .'history.PlantID', $selected_company);
        $CI->db->where(db_prefix() .'items.PlantID', $selected_company);
        $CI->db->where(db_prefix() .'history.TType', 'O');
        $CI->db->where(db_prefix() .'history.TType2', 'Order');
        //$CI->db->where(db_prefix() .'history.NetChallanAmt != 0.00');
        $CI->db->where(db_prefix() .'history.NetChallanAmt !=' , $amt);
        $CI->db->where(db_prefix() .'history.FY', $fy);
        return $CI->db->get()->result_array();
	}
	function get_item_by_order_id_free($order_id)
	{
		$CI = & get_instance();
		$fy = $CI->session->userdata('finacial_year');
		$selected_company = $CI->session->userdata("root_company");
        ...
        $CI->db->where(db_prefix() .'items.PlantID', $selected_company);
        $CI->db->where(db_prefix() .'history.TType', 'O');
        $CI->db->where(db_prefix() .'history.TType2', 'Free Distribution');
        $CI->db->where(db_prefix() .'history.FY', $fy);
        return $CI->db->get()->result_array();
	}
	...
	function GetItemDetailsNew($ChallanID)
	{
		...
		$selected_company = $CI->session->userdata("root_company");
		$amt = 0.00;
		$CI->db->select('IFNULL('.db_prefix() .'history.eOrderQty / '.db_prefix() .'history.CaseQty,'.db_prefix() .'history.OrderQty / '.db_prefix() .'history.CaseQty) as caseqty,IFNULL('.db_prefix() .'history.eOrderQty,'.db_prefix() .'history.OrderQty) as qty,'.db_prefix() .'history.OrderID,'.db_prefix() .'history.ItemID,'.db_prefix() .'history.AccountID');
        $CI->db->from(db_prefix() .'history');
	...
	function GetItemDetailsFrGatepass($ChallanID)
	{
		...
		$selected_company = $CI->session->userdata("root_company");
		$amt = 0.00;
		$CI->db->select('SUM(tblhistory.BilledQty / tblhistory.CaseQty) AS Cases,tblhistory.CaseQty,SUM(tblhistory.BilledQty) as qty,'.db_prefix() .'items.description,'.db_prefix() .'history.ItemID,'.db_prefix() .'history.SuppliedIn');
        $CI->db->from(db_prefix() .'history');
        $CI->db->join(db_prefix() .'items', db_prefix() .'items.item_code = '.db_prefix() .'history.ItemID AND '.db_prefix() .'items.PlantID = '.db_prefix() .'history.PlantID');
        $CI->db->where(db_prefix() .'history.BillID', $ChallanID);
        $CI->db->where(db_prefix() .'history.PlantID', $selected_company);
	...
	function get_sales_details($challan_id,$order_id)
	{
		...
		$fy = $CI->session->userdata('finacial_year');
		$selected_company = $CI->session->userdata("root_company");
		$CI->db->select(db_prefix() .'salesmaster.*,tblordermaster.Crates,tblordermaster.Cases');
		$CI->db->where('tblsalesmaster.ChallanID', $challan_id);
		$CI->db->join(db_prefix() .'ordermaster', db_prefix() .'ordermaster.OrderID = '.db_prefix() .'salesmaster.OrderID');
		if($order_id){
		    $CI->db->where('tblsalesmaster.OrderID', $order_id);
		}
		$CI->db->where('tblsalesmaster.FY', $fy);
		$CI->db->where('tblsalesmaster.PlantID', $selected_company);
		return $CI->db->get(db_prefix() . 'salesmaster')->row();
	}
	function GetChallanCratesCases($challan_id)
	{
		$CI = &get_instance();
		$fy = $CI->session->userdata('finacial_year');
		$selected_company = $CI->session->userdata("root_company");
		$CI->db->select('SUM(tblordermaster.Crates) AS TotalCrates,SUM(tblordermaster.Cases) AS TotalCases');
		$CI->db->where('tblordermaster.ChallanID', $challan_id);
		$CI->db->where('tblordermaster.FY', $fy);
		$CI->db->where('tblordermaster.PlantID', $selected_company);
		return $CI->db->get(db_prefix() . 'ordermaster')->row();
	}
	...
	function get_gst_taxable_amt($order_id,$gst_per)
	{
        ...
        $CI->db->select('tblhistory.OrderAmt,tblhistory.DiscAmt,tblhistory.TType2');
        $CI->db->from(db_prefix() .'history');
        $CI->db->where(db_prefix() .'history.OrderID', $order_id);
        ...
        {
            if($val["TType2"] == "Order"){
                $TaxableAmt += $val["OrderAmt"] - $val["DiscAmt"];
            }else{
                $TaxableAmt += $val["OrderAmt"];
            }
        }
        return $TaxableAmt;
	}
	...
	*/
	...
	function GetCrateLedger($ChallanID,$Accountid,$date)
	{
		$CI = & get_instance();
		$fy = $CI->session->userdata('finacial_year');
		$selected_company = $CI->session->userdata("root_company");
		...
		$TransDate = substr($date,0,10);
		$CI->db->select('
		COALESCE(SUM(Qty), 0) AS AllDispatch,
		(SELECT COALESCE(SUM(Qty), 0) 
		FROM `tblaccountcrates` 
		WHERE AccountID = "' . $Accountid . '" 
		AND PlantID = "' . $selected_company . '" 
		AND FY = "' . $fy . '" 
		AND Transdate BETWEEN "' . $firstDate . ' 00:00:00" AND "' . $TransDate . ' 23:59:59" 
		AND TType = "C") AS AllReturn,
		(SELECT COALESCE(SUM(Qty), 0) 
		FROM `tblaccountcrates` 
		WHERE AccountID = "' . $Accountid . '" 
		AND PlantID = "' . $selected_company . '" 
		...
		AND Transdate BETWEEN "' . $firstDate . ' 00:00:00" AND "' . $TransDate . ' 23:59:59" 
		AND TType = "D" 
		AND ChallanID = "' . $ChallanID . '") AS ChallanDisp,
		(SELECT COALESCE(SUM(Qty), 0) 
		FROM `tblaccountcrates` 
		WHERE AccountID = "' . $Accountid . '" 
		AND PlantID = "' . $selected_company . '" 
		...
		AND Transdate BETWEEN "' . $firstDate . ' 00:00:00" AND "' . $TransDate . ' 23:59:59" 
		AND TType = "C" 
		AND ChallanID = "' . $ChallanID . '") AS ChallanReturn
		');
		$CI->db->from('tblaccountcrates');
		$CI->db->where('tblaccountcrates.AccountID', $Accountid);
		$CI->db->where('tblaccountcrates.TType', 'D');
		$CI->db->where('tblaccountcrates.PlantID', $selected_company);
		$CI->db->where('tblaccountcrates.FY', $fy);
		$CI->db->where('tblaccountcrates.Transdate BETWEEN "' . $firstDate . ' 00:00:00" AND "' . $TransDate . ' 23:59:59"');
		return $CI->db->get()->row_array();
	}
	function getNew($ChallanID){
		$CI = & get_instance();
		$fy = $CI->session->userdata('finacial_year');
		$selected_company = $CI->session->userdata("root_company");
	    
		$sql ="SELECT `tblchallanmaster`.`ChallanID`, `tblclients`.`company`, `tblhistory`.`AccountID`, `tblhistory`.`ItemID`, SUM(`tblhistory`.`BilledQty`) AS BilledQty, `tblhistory`.`CaseQty`, `tblitems`.`local_supply_in`, `tblitems`.`outst_supply_in`, `tblitems`.`description`,(SELECT SUM(Crates) FROM tblordermaster WHERE tblordermaster.ChallanID = tblchallanmaster.ChallanID AND tblordermaster.AccountID= tblhistory.AccountID) AS Crates,(SELECT SUM(Cases) FROM tblordermaster WHERE tblordermaster.ChallanID = tblchallanmaster.ChallanID AND tblordermaster.AccountID= tblhistory.AccountID) AS Cases
		FROM `tblhistory`
		JOIN `tblchallanmaster` ON `tblchallanmaster`.`ChallanID` = `tblhistory`.`BillID`
		JOIN `tblclients` ON `tblclients`.`AccountID` = `tblhistory`.`AccountID`
		JOIN `tblitems` ON `tblitems`.`item_code` = `tblhistory`.`ItemID` AND `tblitems`.`PlantID` = `tblhistory`.`PlantID` AND tblitems.local_supply_in IN ('CR','CS')
		WHERE `tblchallanmaster`.`PlantID` = '".$selected_company."'
		AND `tblchallanmaster`.`FY` = '".$fy."'
		AND `tblchallanmaster`.`ChallanID` = '".$ChallanID."' GROUP BY tblhistory.ItemID,tblhistory.AccountID;";
