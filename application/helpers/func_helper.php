...
if (!function_exists('time_delay')) {
    /**
     * Short Time delay function
     * @param  datetime $date
     * @return mixed
     */
    function time_delay($date)
    {
        $CI = &get_instance();

        $localization = [];

        foreach (['time_delay_just_now', 'time_delay_minute', 'time_delay_minutes', 'time_delay_hour', 'time_delay_hours', 'time_delay_yesterday', 'time_delay_days', 'time_delay_week', 'time_delay_weeks', 'time_delay_month', 'time_delay_months', 'time_delay_year', 'time_delay_years'] as $langKey) {
            if (isset($CI->lang->language[$langKey])) {
                $localization[$langKey] = $CI->lang->language[$langKey];
            }
        }

        return \app\services\utilities\Date::timeDelayString($date, $localization);
    }
}
