...
function get_relation_data($type, $rel_id = '')
{
    ...
    if ($type == 'customer' || $type == 'customers') {
        ...
        if ($q) {
            ...
            $where_clients .= '(company LIKE "%' . $CI->db->escape_like_str($q) . '%" ESCAPE \'!\' OR CONCAT(firstname, " ", lastname) LIKE "%' . $CI->db->escape_like_str($q) . '%" ESCAPE \'!\' OR address LIKE "%' . $CI->db->escape_like_str($q) . '%" ESCAPE \'!\' OR Address3 LIKE "%' . $CI->db->escape_like_str($q) . '%" ESCAPE \'!\') AND ' . db_prefix() . 'clients.active = 1 AND ' . db_prefix() . 'clients.SubActGroupID1 = 100056';
        }
...
function get_relation_values($relation, $type)
{
    ...
    if ($type == 'customer' || $type == 'customers') {
        if (is_array($relation)) {
            ...
            $name = $relation['company']." ( ".$relation['Station']." )";
        } else {
            $id   = $relation->AccountID;
            $name = $relation->company." ".$relation->Station;
        }
