...
class Invoices extends AdminController
{
    ...
    public function pdf($challan_id)
    {
        if (!$challan_id) {
            redirect(admin_url('invoices/list_invoices'));
        }

        $canView = user_can_view_invoice($challan_id);
        if (!$canView) {
            access_denied('Invoices');
        } else {
            if (!has_permission('invoices', '', 'view') && !has_permission('invoices', '', 'view_own') && $canView == false) {
        ...
        $invoice        = $this->invoices_model->get_invoice($challan_id);
        
        //$invoice        = hooks()->apply_filters('before_admin_view_invoice_pdf', $invoice[0]["invoice_id"]);
        $invoice_number = format_invoice_number($invoice[0]["invoice_id"]);
        /*echo "<pre>";
        //echo $invoice;
        echo "<br>";
        echo $invoice_number;
...
*/
    ...
    /* Generates Dispatch PDF and senting to email of $send_to_email = true is passed */
