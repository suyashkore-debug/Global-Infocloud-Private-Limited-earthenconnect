...

{
    ...
    public function export_party_list(){
       ...
       if($post_data['division'] !=''){
        ...
        if($post_data['status'] !=''){
    		...
    		$set_col_tk["Mobile Number"] = 'Mobile Number';
    ...
    public function load_data_filter(){
       ...
       $html.= '<th class="sortable" data-column="account_code">Ac Code</th>'; 

       $html.= '<th class="sortable" data-column="firm_name">Firm Name</th>'; 

       $html.= '<th class="sortable" data-column="mobile_no">Mobile No.</th>'; 

       $html.= '<th class="sortable" data-column="distributor_type">Distributor Type</th>'; 

       $html.= '<th class="sortable" data-column="station">Station</th>'; 

       $html.= '<th class="sortable" data-column="state">State</th>'; 

       $html.= '<th class="sortable" data-column="city">City</th>'; 

       $html.= '<th class="sortable" data-column="town">Town</th>'; 

       $html.= '<th class="sortable" data-column="sales_person">Sales Person</th>'; 
...

    {
		...
		// print_r($data['routes']);die;

        $data['state'] = $this->clients_model->getallstate();

        $data['countries'] = $this->clients_model->getallcountries();
...
//========================== Get All Customer List =============================
...

    {
...

        {
			...
			$html .= '<td align="left">'.$value["firstname"].'</td>';

			$html .= '<td align="left">'.$value["lastname"].'</td>';

			$html .= '<td align="left">'.$value["name"].'</td>';

			$html .= '<td align="left">'.$value["state_name"].'</td>';

			$html .= '<td align="left">'.$value["city_name"].'</td>';
...

    {
        ...
        $StateList = $this->clients_model->get_states_by_country($CountryID);
...
//==================== Get Route Points Against Route ==========================
...

    {
        ...
        $RoutePointList = $this->clients_model->GetRoutePoints($routes);

        echo json_encode($RoutePointList);

    }

//==================== Check Distributor Type state ============================

    public function GetDistTypeState()

    {

        $DistType = $this->input->post('DistType');

        $StateID = $this->clients_model->GetDistTypeState($DistType);
	...
	public function validate_gst_mastergst()

		{
			...
			$url = "https://api.mastergst.com/ewaybillapi/v1.03/ewayapi/getgstindetails?email=" 

			. urlencode($authHeaders['email']) 

			. "&GSTIN=" . urlencode($GstNo);
			...
			if (isset($data['status_cd']) && $data['status_cd'] == "1") {
				...
				'ErrorMsg' => isset($data['error']['message']) ? $data['error']['message'] : 'Invalid GST No',
