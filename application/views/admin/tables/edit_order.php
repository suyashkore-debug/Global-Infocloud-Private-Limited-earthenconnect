...
if ($this->ci->input->post('date')) {
    $order_date = $this->ci->input->post('date');
    array_push($where, 'AND ' . db_prefix() . 'ordermaster.Transdate <= "'.$order_date.' 23:59:00"');
...
foreach ($rResult as $aRow) {
    ...
    $numberOutput = '<a href="' . admin_url('order/order/' . $aRow['OrderID']) . '" target="_blank">' . $aRow['OrderID'] . '</a>';
    $row[] = $numberOutput;
    //$row[] = substr($aRow['date'],0,10);
    //$row[] = $aRow['SalesID'];
