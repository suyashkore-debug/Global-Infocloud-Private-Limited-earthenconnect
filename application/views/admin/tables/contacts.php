...
$total_client_contacts = total_rows(db_prefix() . 'contacts', ['AccountID' => $client_id]);
...
$where = ['AND AccountID=' . $this->ci->db->escape_str($client_id)];
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [db_prefix() . 'contacts.id as id', 'AccountID', 'is_primary']);
...
foreach ($rResult as $aRow) {
    ...
    $rowName = '<img src="' . contact_profile_image_url($aRow['id']) . '" class="client-profile-image-small mright5"><a href="#" onclick="contact(' . $aRow['AccountID'] . ',' . $aRow['id'] . ');return false;">' . $aRow['full_name'] . '</a>';

    $rowName .= '<div class="row-options">';

    $rowName .= '<a href="#" onclick="contact(' . $aRow['AccountID'] . ',' . $aRow['id'] . ');return false;">' . _l('edit') . '</a>';
    ...
    if (has_permission('customers', '', 'delete') || is_customer_admin($aRow['AccountID'])) {
        if ($aRow['is_primary'] == 0 || ($aRow['is_primary'] == 1 && $total_client_contacts == 1)) {
            $rowName .= ' | <a href="' . admin_url('clients/delete_contact/' . $aRow['AccountID'] . '/' . $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
        }
    }
    ...
    $outputActive = '<div class="onoffswitch">
                <input type="checkbox"' . (total_rows(db_prefix() . 'clients', 'registration_confirmed=0 AND AccountID=' . $aRow['AccountID']) > 0 ? ' disabled' : '') . ' data-switch-url="' . admin_url() . 'clients/change_contact_status" name="onoffswitch" class="onoffswitch-checkbox" id="c_' . $aRow['id'] . '" data-id="' . $aRow['id'] . '"' . ($aRow['active'] == 1 ? ' checked': '') . '>
