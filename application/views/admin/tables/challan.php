...
$aColumns = [
    ...
    'tblordermaster.Crates as Crates',
    'tblordermaster.Cases as Cases',
    'ChallanAmt',
    'BillAmt',
    'tblchallanmaster.Transdate as date',
    'tblchallanmaster.Lupdate as UpdatedDate',
    ];
...
$join = [
	...
	'INNER JOIN '.db_prefix().'ordermaster ON '.db_prefix().'ordermaster.OrderID='.db_prefix().'salesmaster.OrderID AND '.db_prefix().'ordermaster.PlantID='.db_prefix().'salesmaster.PlantID AND '.db_prefix().'ordermaster.FY='.db_prefix().'salesmaster.FY',
...
// array_push($where, ' ORDER BY ' . db_prefix() . 'challanmaster.Transdate DESC');
...
foreach ($rResult as $aRow) {
    ...
    $row[] = $aRow['ConsolidatedEWayBillNo'];
    $row[] = $aRow['ConsolidatedEWayBillDate'];
    $row[] = $aRow['company'];
    ...
    // $row[] = $aRow['Cases'];
    $row[] = $qty;
    $row[] = round($aRow['BillAmt']);
    $row[] = _d(substr($aRow['date'],0,18));
    $row[] = _d(substr($aRow['UpdatedDate'],0,18));
