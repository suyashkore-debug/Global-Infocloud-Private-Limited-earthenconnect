	    ...
	    changes.forEach(([row, prop, oldValue, newValue]) => {
            ...
            vendor_id = $("#vendor").val();
  	        ...
  	        if(prop == 'id'){
  	                  ...
  	                  $.post(admin_url + 'Stock_adjustment/items_change/'+newValue+'/'+group_id+'/'+state).done(function(response){
              	         ...
              	         if(response.value.outst_supply_in == 'CS'){
              	              var case_create = 'Case';
              	               ...
              	               var case_create = 'Crate';
              	          }
              	          if(response.value.case_qty > 0){
              	              PackQty = response.value.case_qty;
              	          }else{
              	              PackQty = response.value.crate_qty;
              	          }
                          hot.setDataAtCell(row,1, response.value.description);
              	          hot.setDataAtCell(row,2, case_create);
              	          hot.setDataAtCell(row,3, PackQty);
  	          ...
  	          var Qty = hot.getDataAtCell(row,5);
  	          var PackQty = hot.getDataAtCell(row,3);
  	          if(PackQty < 1){
  	              PackQty = 1
  	          }
  	          var case_q =  Qty/PackQty;
            hot.setDataAtCell(row,4, parseFloat(case_q).toFixed(2));
                var Amt =  newValue*Qty;
  	      ...
  	      }else if(prop == 'all_qty'){ 
  	          var PackQty = hot.getDataAtCell(row,3);
  	          if(PackQty < 1){
  	              PackQty = 1
  	          }
  	          var case_q =  newValue/PackQty;
                hot.setDataAtCell(row,4, parseFloat(case_q).toFixed(2));
             var Amt =  newValue*hot.getDataAtCell(row,6);
  	       	           var gst = hot.getDataAtCell(row,7)
  	       	           //console.log(gst)
  	       var state = $("#state_id").val();
  	       if(state == 'UP'){
