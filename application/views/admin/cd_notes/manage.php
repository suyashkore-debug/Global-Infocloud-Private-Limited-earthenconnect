		...
		<div class="row" style="display:none;">
			<div class="col-md-12">
				<table id="print_table">
					...
					<tbody>
						<tr>
							<td colspan="13"><b>CD Note Details</b></td>
						</tr>
						<tr>
					...
					<div class="panel-body">
						...
						<nav aria-label="breadcrumb">
							<ol class="breadcrumb custombreadcrumb" style="background-color:#fff !important; margin-Bottom:0px !important;">
								...
								<li class="breadcrumb-item active" aria-current="page"><b>Credit Note</b></li>
							</ol>
						</nav>
						<hr class="hr_style">
								...
								<div class="row">
									<div class="col-md-8">
										...
										<div class="row">
											...
											<div class="col-md-3" style="padding-right: 0px;">
												...
												?>
												<div class="form-group credit_div" style="<?php if(isset($cd_notes_details) && $cd_notes_details->BT=="D"){ echo 'display:none'; }?>">
													<label for="number">
														Credit
												...
												<div class="form-group debit_div" style="<?php if((isset($cd_notes_details) && $cd_notes_details->BT=="C") || !isset($cd_notes_details)){ echo 'display:none'; }?>">
													<label for="number">
														Debit
														<!-- <i class="fa fa-question-circle" data-toggle="tooltip" data-title="<?php echo _l('invoice_number_not_applied_on_draft') ?>" data-placement="top"></i>-->
													</label>
									...
									<div class="col-md-4">
										<?php
											...
											<div class="row" style="display:none;">
												<div class="col-md-6">
													<input type="radio" id="credit" name="type_select" class="type_select" value="credit" checked>
													<label for="credit">Credit</label>
													<input type="radio" id="debit" name="type_select" class="type_select" value="debit">
													<label for="debit">Debit</label>
													<input type="hidden" name="type_select2" id="type_select2" value="credit">
												</div>
												
												<div class="col-md-6">
													<input type="radio" id="salecd" name="type_cd" class="type_cd" value="salecd" checked>
													<label for="salecd">Sale CD</label>
													<input type="radio" id="purchasecd" name="type_cd" class="type_cd" value="purchasecd">
													<label for="purchasecd">Purchase CD</label>
													<input type="hidden" name="type_cd2" id="type_cd2" value="salecd">
												</div>
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title">Credit Notes List</h4>
			</div>
					...
					<div class="col-md-12">
						...
						<div class="table_cd_report">
							...
							<table class="tree table table-striped table-bordered table_cd_report" id="table_cd_report" width="100%">
								...
								<thead>
									<tr>
										<th class="sortablePop" style="width:10%;">Credit Note No</th>
										<th class="sortablePop" style="width:10%;text-align:left;">Credit Type</th>
										...
										<th class="sortablePop" class="sortablePop" style="width:3%;text-align:left;">IGSTAmt</th>
...
<script type='text/javascript'>
	...
	$(document).ready(function(){
								...
								if(sale_item_array.includes(data[0].value)){
									...
									if(Act_group == ""){
										...
										if(data[0].hsn_code == ""){
											...
											$("#sale_id").focus();
											init_model(data[0].hsn_code,act_code,data[0].tax);
											return false;
										}
									}
									}else{
				...
				if(sale_item_array.includes(ui.item.value)){
					...
					if(Act_group == ""){
						...
						if(ui.item.hsn_code == ""){
							...
							$("#sale_id").focus();
							init_model(ui.item.hsn_code,act_code,ui.item.tax);
							return false;
						}
					}
		...
		/*function init_model(item_hsn,act_code,tax){
		...
		function init_model(item_hsn, act_code, tax) {
			...
			requestGetJSON('cd_notes/get_bill_id/' + item_hsn + "/" + act_code + "/" + Act_group + "/" + CDType).done(function (response) {
				// If tax is 0 and there is at least one response, call get_bill_details and return
				if (tax == 0 && response.length > 0) {
					var firstEntry = response[0]; // Get the first response entry
					var billID = CDType === "salecd" ? firstEntry.TransID : firstEntry.BillID;
					get_bill_details(billID, firstEntry.sum_total, firstEntry.igst, firstEntry.cgst, firstEntry.sgst);
					return;
				}
				
				// Otherwise, proceed with showing the popup
				var $itemModal = $('#sales_item_modal');
				var html = '<thead>';
				...
				$.each(response, function (index, value) {
					if (CDType == "salecd") {
						...
						var gst = (value.igst == "0.00" || value.igst == null) ? (parseFloat(value.cgst) + parseFloat(value.sgst)) : value.igst;
						html += '<td align="right">' + parseFloat(gst).toFixed(2) + '</td>';
						html += '<td align="right">' + value.sum_total + '</td>';
						html += '<td align="right">0</td>';
						html += '<td align="right">0</td>';
