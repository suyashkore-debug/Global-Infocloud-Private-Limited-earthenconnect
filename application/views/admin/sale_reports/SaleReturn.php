					...
					<div class="panel-body">
						<nav aria-label="breadcrumb">
            				<ol class="breadcrumb custombreadcrumb" style="background-color:#fff !important; margin-Bottom:0px !important;">
            					...
            					<li class="breadcrumb-item active" aria-current="page"><b>Sales Return - Report</b></li>
							</ol>
						</nav>
                        <hr class="hr_style">
						<div class="_buttons">
							<div class="row">
								...
								?>
								<div class="col-md-2" >
									<label for="from_date" class="control-label">Account Name</label>
								</div>
							...
							<div class="row">
								...
								<div class="col-md-2" style="padding-left:1px;">
									<div class="form-group">
										<input type="text" name="AccountCity" id="AccountCity" class="form-control" readonly>
									</div>
								</div>
								<div class="col-md-8">
								...
								<div class="col-md-4">
									<input type="text" id="myInput" onkeyup="myFunction1()" placeholder="Search for names.." class="form-control" title="Type in a name" style="float: right;">
											...
											<div class="col-md-12">
												<div class="table_accountlist">
													<table class="tree table table-striped table-bordered table_accountlist" id="table_accountlist" width="100%">
														<thead>
															<tr>
																<th class="sortablePop" style="text-align:center;">Account ID</th>
						...
						<div class="fixTableHead SaleRtn_report">
...
<?php init_tail(); ?>
<style>
    .SaleRtn_report { overflow: auto;max-height: 60vh;width:100%;position:relative;top: 0px; }
    .SaleRtn_report thead th { position: sticky; top: 0; z-index: 1; }
    .SaleRtn_report tbody th { position: sticky; left: 0; }
    
    /* Just common table stuff. Really. */
    .SaleRtn_report table  { border-collapse: collapse; width: 100%; }
    th, td { padding: 1px 5px !important; white-space: nowrap; border:1px solid !important;font-size:11px; line-height:1.42857143!important;vertical-align: middle !important;}
    .SaleRtn_report th     { background: #50607b;color: #fff !important; }
...
<script type="text/javascript" language="javascript" >
	$(document).ready(function(){
		...
		$( "#AccountID" ).autocomplete({
			...
			select: function (event, ui) {
				...
				$('#AccountCity').val(ui.item.CityName); // display the selected text
				return false;      
			}
		});
		...
		$('.GetAccountID').on('click',function(){ 
			...
			$.ajax({
				...
				success:function(data){
					...
					$('#AccountCity').val(data.city_name); // display the selected text
					$('#search_data').focus();
				}
			});
		});
		...
		$('#search_data').on('click',function(){
			...
			var AccountAddress2 = $("#AccountAddress2").val();
			var AccountCity = $("#AccountCity").val();
			
			$.ajax({
				url:"<?php echo admin_url(); ?>sale_reports/GetSaleRtnReport",
				dataType:"JSON",
				method:"POST",
				cache: false,
				data:{from_date:from_date, to_date:to_date, AccountID:AccountID,AccountName:AccountName},
				beforeSend: function () {
					$('#searchh').css('display','block');
					$('.SaleRtn_report').css('display','none');
				},
				complete: function () {
					$('.SaleRtn_report').css('display','');
					$('#searchh').css('display','none');
				},
				success:function(data){
					$('.SaleRtn_report').html(data);
				}
			});
		});
	...
	$("#caexcel").click(function(){
		...
		var AccountCity = $("#AccountCity").val();*/
	    $.ajax({
			url:"<?php echo admin_url(); ?>sale_reports/ExportSaleRtnReport",
			method:"POST",
			data:{from_date:from_date, to_date:to_date, AccountID:AccountID,AccountName:AccountName},
			beforeSend: function () {
...
<script type="text/javascript">
	function printPage(){
		...
		var filter = 'Account Name : '+AccountName + ' , Report Date : '+from_date+' To '+to_date;
		...
		heading_data += '<tbody><tr><td style="text-align:center;" ><?php echo $company_detail->company_name; ?></td></tr>';
		heading_data += '<tr><td style="text-align:center;" ><?php echo $company_detail->address; ?></td></tr>';    
		heading_data += '<tr><td style="text-align:left;">'+filter+'</td></tr>';
