					...
					<div class="panel-body">
						<nav aria-label="breadcrumb">
							<ol class="breadcrumb custombreadcrumb" style="background-color:#fff !important; margin-Bottom:0px !important;">
								...
								<li class="breadcrumb-item active" aria-current="page"><b>Stock Alert</b></li>
							</ol>
						</nav>
						<hr class="hr_style">
							...
							<table class="tree table table-striped table-bordered table-daily_report" id="table-daily_report" width="100%">
								...
								<thead>
									...
									<tr>
										...
										<th class="sortable" style="text-align:left;">Item ID</th>
										<th class="sortable" style="text-align:left;">Item Name</th>
										<th class="sortable" style="text-align:left;">Minimum Required Quantity</th>
										<th class="sortable" style="text-align:left;">Maximum Required Quantity</th>
										<th class="sortable" style="text-align:left;">Stock Available</th>
									</tr>
								</thead>
								<tbody>
									<?php
										$i = 1;
										foreach($ItemListFG as $each)
										{
											$PQty = 0;
											$PRQty = 0;
											$IQty = 0;
											...
											foreach ($each['itemStocks'] as $stock) {
											...
											{
												$style = '';
												if ($each['min_qty'] > 0 && $stockQtyInCase <= $each['min_qty']){
													$style ="background-color:red; color:#fff;";
												}
												if ($each['min_qty'] > 0 && $stockQtyInCase >= $each['max_qty']){
													$style ="background-color:green;color:#fff;";
												}
											?>
											<tr style="<?= $style;?>">
												<td style="text-align:left;"><?= $i ?></td>
												<td style="text-align:left;"><?= $each['item_code']?></td>
												<td style="text-align:left;"><?= $each['description']?></td>
												<td style="text-align:left;"><?= $each['min_qty']?></td>
												<td style="text-align:left;"><?= $each['max_qty']?></td>
												<td style="text-align:left;"><?= number_format($stockQtyInCase, 2, '.', ''); ?></td>
											</tr>
											<?php
												$i++;
											}
...
<script type="text/javascript" language="javascript" >
	...
	$("#caexcel").click(function(){
	    ...
	    $.ajax({
            url:"<?php echo admin_url(); ?>sale_reports/export_Stock_alert",
            beforeSend: function () {
...
<script type="text/javascript">
	function printPage(){
		...
		heading_data += '<tr>';
		heading_data += '<td style="text-align:center;"colspan="9">Stock Alert Report</td>';
		heading_data += '</tr>';
