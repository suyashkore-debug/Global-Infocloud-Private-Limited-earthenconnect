					...
					<div class="panel-body">
						...
						<nav aria-label="breadcrumb">
            				<ol class="breadcrumb custombreadcrumb" style="background-color:#fff !important; margin-Bottom:0px !important;">
            					...
            					<li class="breadcrumb-item active" aria-current="page"><b>Vehicle Loaded Capacity Report</b></li>
							</ol>
						</nav>
                        <hr class="hr_style">
						<div class="_buttons">
							...
							<table class="tree table table-striped table-bordered table-daily_report" id="table-daily_report" width="100%">
								...
								<thead>
									<tr>
										...
										<th class="sortable" style="text-align:left;">Gatepass Time</th>
										<th class="sortable" style="text-align:left;">Vehicle Capacity</th>
										<th class="sortable" style="text-align:left;">Vehicle Capacity Of Loaded</th>
									</tr>
								</thead>
								<tbody>
								</tbody>
...
<script type="text/javascript" language="javascript" >
	$(document).ready(function(){
		...
		function load_data(from_date,to_date,vehicle,Route)
		{
			$.ajax({
				url:"<?php echo admin_url(); ?>VehRtn/GetVehicleLoadedReport",
				dataType:"JSON",
				method:"POST",
				data:{from_date:from_date, to_date:to_date,vehicle:vehicle,Route:Route},
				beforeSend: function () {
		...
		$('#search_data').on('click',function(){
			...
			var Route = $("#Route").val();
			load_data(from_date,to_date,vehicle,Route);
...
<script type="text/javascript">
	function printPage(){
		...
		heading_data += '<tr>';
		heading_data += '<td style="text-align:center;"colspan="9">Vehicle Loaded Capacity Report From : '+from_date+' To '+to_date+'</td>';
		heading_data += '</tr>';
	...
	$("#caexcel").click(function(){
		...
		var Route = $("#Route").val();
		$.ajax({
			url:"<?php echo admin_url(); ?>VehRtn/export_VehicleLoadedReport",
			method:"POST",
			data:{from_date:from_date, to_date:to_date,vehicle:vehicle,Route:Route},
			beforeSend: function () {
