				...
				<div class="panel_s">
					...
					<nav aria-label="breadcrumb">
						...
						<div class="row">
							...
							<input type="hidden" name="group_codehidden" id="group_codehidden" class="form-control" value="<?php echo $NextAccountGroupID;?>">
							<div class="col-md-4">
								<div class="form-group">
									<label for="group_code">Group Code</label>
									<input type="text" name="group_code" id="group_code" class="form-control" value="<?php echo $NextAccountGroupID;?>" onkeypress="return isNumber(event)">
						...
						<div class="row"> 
							...
							<div class="col-md-12">
								<?php if (has_permission_new('account_main_groups', '', 'create')) {
								?>
								<button type="button" class="btn btn-info saveBtn disabled" style="margin-right: 25px;">Save</button>
								<?php
									}else{
								?>
								...
								<?php if (has_permission('account_main_groups', '', 'edit')) {
								?>
								<button type="button" class="btn btn-info updateBtn disabled" style="margin-right: 25px;">Update</button>
								<?php
									}else{
								?>
									...
									<div class="modal-body" style="padding:0px 5px !important">
										...
										<div class="table-AccountGroup tableFixHead2">
											<table class="tree table table-striped table-bordered table-AccountGroup tableFixHead2" id="table_AccountGroup" width="100%">
												<thead>
													<tr>
														<th class="sortablePop" style="text-align:left;">AccountGroup </th>
														<th class="sortablePop" style="text-align:left;">AccountDescription</th>
														<th class="sortablePop" style="text-align:left;">GroupType</th>
...
<script type="text/javascript" language="javascript" >
	$(document).ready(function(){
		...
		$('#group_code').on('focus',function(){
			var NextGroupNo =  $('#group_codehidden').val();
			$('#group_code').val(NextGroupNo);
			$('#group_name').val('');
		...
		$( "#group_code" ).autocomplete({
			source: function( request, response ) {
				// Fetch data
				$.ajax({
					url: "<?=base_url()?>admin/accounts_master/get_accounts_main_group",
					type: 'post',
					dataType: "json",
					data: {
						search: request.term
            ...
            $.ajax({
                ...
                success:function(data){
					if(data == true){
						...
						$.ajax({
							url:"<?php echo admin_url(); ?>accounts_master/NextAccountGroupID",
							dataType:"JSON",
							cache: false,
							success:function(data){
								$('.saveBtn').show();
