								...
								<div class="col-md-12">
									<nav aria-label="breadcrumb">
										<ol class="breadcrumb custombreadcrumb" style="background-color:#fff !important; margin-Bottom:0px !important;">
											...
											<li class="breadcrumb-item active" aria-current="page"><b>Weekly/Monthly Pest Control Log</b></li>
										</ol>
									</nav>
									<hr class="hr_style">							
								</div>
							</div>	
							<form action="" method="post" id="PestControlLogForm">
								...
								<div class="row">
									...
									<div class="col-md-6">
										<label for="serviceName" class="control-label" >Pest Control Service Used (if any)</label>
										<div class="form-group">
											<textarea id="serviceName" name="serviceName" class="form-control"></textarea>
										</div>
									</div>
									<div class="col-md-6">
									...
									<div class="col-md-12">
										<div class="table-daily_report tableFixHead2">
											<table class="tree table table-striped table-bordered table-daily_report tableFixHead2" id="table-daily_report" width="100%">
												<thead>
													<tr>
														<th class="sortable">Trap Location</th>
														<th class="sortable">Trap Condition</th>
														<th class="sortable">Activity Found(Y/N)</th>
												...
												<tbody>
													...
													<tr>
														...
														<td>
															<div class="form-group">
																<textarea class="form-control" name="CornersInitial" id="CornersInitial"></textarea>
															</div>
														</td>
													</tr>
													<tr>
  ...
  <div class="modal-dialog" role="document" style="width: 80vw;">
    <div class="modal-content">
      <div class="modal-header" style="padding:5px 10px;">
        ...
        <h4 class="modal-title">Weekly/Monthly Pest Control Log List</h4>
      </div>
          ...
          <table class="tree table table-striped table-bordered table-ListModal tableFixHead2" id="table_ListModal" width="100%">
            <thead>
              <tr>
                ...
                <th class="sortable">Door</th>
                <th class="sortable">Corners</th>
            ...
            <tbody>
							...
							if(isset($log_list) && !empty($log_list)){
								foreach($log_list as $key => $value) {
									echo '<tr class="get_Details" data-id="'.$value['PestLogID'].'" onclick="getDetails(\''.$value['PestLogID'].'\', this)">
										<td>'.explode(' ', _d($value['TransDate']))[0].'</td>
										<td>'.$value['WarehouseName'].'</td>
										<td>'.$value['LocationID'].'</td>
										<td>'.$value['ServiceName'].'</td>
										<td>'.$value['DoorCondition'].'</td>
										<td>'.$value['CornersCondition'].'</td>
										<td>'.$value['AreaCondition'].'</td>
									</tr>';
								}
							}
							?>
...
<script type="text/javascript">
	...
	$('#PestControlLogForm').on('submit', function(e) {
		...
		let urlLink = '<?= admin_url('StorageAuditLog/savePestControlLog'); ?>';
		if($('#update_id').val() != ''){
			urlLink = '<?= admin_url('StorageAuditLog/updatePestControlLog'); ?>';
		}
		$.ajax({
			url: urlLink,
			method: 'POST',
	...
	async function getDetails(PestLogID, row) {
		let $row = $(row);
		...
		try {
			const response = await $.ajax({
				url: "<?= admin_url('StorageAuditLog/getPestControlLog'); ?>",
				method: "POST",
				dataType: "json",
				data: { PestLogID }
			});
			...
			if (d.details && d.details.length > 0) {
				d.details.forEach(item => {
					switch (item.TrapLocation) {
						case 'Door':
							$('#DoorCondition').val(item.TrapCondition);
							$('#DoorActFound').val(item.ActivityFound);
							$('#DoorAction').val(item.ActionTaken);
							$('#DoorInitial').val(item.Initials);
							break;
						case 'Corners':
							$('#CornersCondition').val(item.TrapCondition);
							$('#CornersActFound').val(item.ActivityFound);
							$('#CornersAction').val(item.ActionTaken);
							$('#CornersInitial').val(item.Initials);
							break;
						case 'Area':
							$('#AreaCondition').val(item.TrapCondition);
							$('#AreaActFound').val(item.ActivityFound);
							$('#AreaAction').val(item.ActionTaken);
							$('#AreaInitial').val(item.Initials);
							break;
						default:
							console.warn('Unknown parameter:', item.TrapLocation);
					}
				});
			}
...
<script>
  ...
  $('#filter_list_form').submit(function(e) {
    ...
    function fetchChunk() {
      ...
      $.ajax({
        url: "<?= admin_url('StorageAuditLog/getPestControlLogList') ?>",
        type: "POST",
        data: form_data,
        processData: false,
        contentType: false,
        success: function(res) {
          ...
          if (!json.success) {
            ...
            if (offset === 0) {
              $('#table_ListModal tbody').html('<tr><td colspan="7" class="text-center">No Data Found</td></tr>');
            }
            return;
          }
  ...
  function appendRows(rows) {
    ...
    rows.forEach(function(row) {
      html += `<tr class="get_Details" data-id="${row.PestLogID}" onclick="getDetails('${row.PestLogID}', this)">
        <td>${moment(row.TransDate).format('DD/MM/YYYY')}</td>
				<td>${row.WarehouseName || ''}</td>
				<td>${row.LocationID || ''}</td>
				<td>${row.ServiceName || ''}</td>
				<td>${row.DoorCondition || ''}</td>
				<td>${row.CornersCondition || ''}</td>
				<td>${row.AreaCondition || ''}</td>
      </tr>`;
    });
