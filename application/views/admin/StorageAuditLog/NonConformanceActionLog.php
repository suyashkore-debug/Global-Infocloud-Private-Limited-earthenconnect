								...
								<div class="col-md-12">
									<nav aria-label="breadcrumb">
										<ol class="breadcrumb custombreadcrumb" style="background-color:#fff !important; margin-Bottom:0px !important;">
											...
											<li class="breadcrumb-item active" aria-current="page"><b>Non-Conformance & Corrective Action Log</b></li>
										</ol>
									</nav>
									<hr class="hr_style">							
								</div>
							</div>	
							<form action="" method="post" id="NonConformanceActionLogForm">
								...
								<div class="row">
									...
									<div class="col-md-12">
										<label for="Type" class="control-label" ><small class="req text-danger">* </small>Type</label><br>
										<?php
											if(isset($NCTypeList) && !empty($NCTypeList)){
												foreach($NCTypeList as $key => $value) {
													echo '<label for="type'.$value->TypeID.'"><input type="checkbox" name="type[]" id="type'.$value->TypeID.'" value="'.$value->TypeID.'">&nbsp;'.$value->TypeName.' &emsp;</label>';
												}
											}
										?>
									</div>
									
									<div class="col-md-6" id = "other_input_div" style="display:none;">
										<label for="other_input" class="control-label" ><small class="req text-danger">* </small>Others</label>
										<div class="form-group">
											<textarea id="other_input" name="other_input" class="form-control"></textarea>
										</div>
									</div>
									
									<div class="col-md-6">
										<label for="Issue" class="control-label" ><small class="req text-danger">* </small>Issue Identified</label>
										<div class="form-group">
									...
									<div class="col-md-6">
										<label for="ActionTaken" class="control-label" ><small class="req text-danger">* </small>Corrective Action Taken</label>
										<div class="form-group">
											<textarea id="ActionTaken" name="ActionTaken" class="form-control"></textarea>
										</div>
									</div>
									
									
									<div class="col-md-6">
										<label for="PreAction" class="control-label" ><small class="req text-danger">* </small>Preventive Action (future)</label>
										<div class="form-group">
											<textarea id="PreAction" name="PreAction" class="form-control"></textarea>
										</div>
									</div>
  ...
  <div class="modal-dialog" role="document" style="width: 80vw;">
    <div class="modal-content">
      <div class="modal-header" style="padding:5px 10px;">
        ...
        <h4 class="modal-title">Non-Conformance & Corrective Action Log List</h4>
      </div>
          ...
          <table class="tree table table-striped table-bordered table-ListModal tableFixHead2" id="table_ListModal" width="100%">
            <thead>
              <tr>
                ...
                <th class="sortable">Corrective Action</th>
                <th class="sortable">Preventive Action</th>
              </tr>
            </thead>
            <tbody>
							<?php
							...
							if(isset($log_list) && !empty($log_list)){
								foreach($log_list as $key => $value) {
									echo '<tr class="get_Details" data-id="'.$value['NCID'].'" onclick="getDetails(\''.$value['NCID'].'\', this)">
										<td>'.explode(' ', _d($value['TransDate']))[0].'</td>
										<td>'.$value['WarehouseName'].'</td>
										<td>'.$value['LocationID'].'</td>
										<td>'.$value['Issue'].'</td>
										<td>'.$value['CorrectiveAction'].'</td>
										<td>'.$value['PreventiveAction'].'</td>
									</tr>';
								}
							}
							?>
...
<script type="text/javascript">
	...
	function ResetForm() {
		$('#update_id').val('');
		$('#NonConformanceActionLogForm')[0].reset();
	...
	$('#NonConformanceActionLogForm').on('submit', function(e) {
		e.preventDefault();
		var formData = new FormData(this);
		let urlLink = '<?= admin_url('StorageAuditLog/saveNonConformanceActionLog'); ?>';
		if($('#update_id').val() != ''){
			urlLink = '<?= admin_url('StorageAuditLog/updateNonConformanceActionLog'); ?>';
		}
		$.ajax({
			url: urlLink,
			method: 'POST',
	...
	async function getDetails(NCID, row) {
		let $row = $(row);
		...
		try {
			const response = await $.ajax({
				url: "<?= admin_url('StorageAuditLog/getNonConformanceActionLog'); ?>",
				method: "POST",
				dataType: "json",
				data: { NCID }
			});
			...
			if (d.details && d.details.length > 0) {
				d.details.forEach(item => {
					$(`#type${item.TypeID}`).prop('checked', true);
					if (item.TypeID == 5) {
						$('#other_input_div').show().focus();
					}
				});
...
<script>
  ...
  $('#filter_list_form').submit(function(e) {
    ...
    function fetchChunk() {
      ...
      $.ajax({
        url: "<?= admin_url('StorageAuditLog/getNonConformanceActionLogList') ?>",
        type: "POST",
        data: form_data,
        processData: false,
        contentType: false,
        success: function(res) {
          ...
          if (!json.success) {
            ...
            if (offset === 0) {
              $('#table_ListModal tbody').html('<tr><td colspan="6" class="text-center">No Data Found</td></tr>');
            }
            return;
          }
  ...
  function appendRows(rows) {
    ...
    rows.forEach(function(row) {
      html += `<tr class="get_Details" data-id="${row.NCID}" onclick="getDetails('${row.NCID}', this)">
        <td>${moment(row.TransDate).format('DD/MM/YYYY')}</td>
				<td>${row.WarehouseName || ''}</td>
				<td>${row.LocationID || ''}</td>
				<td>${row.Issue || ''}</td>
				<td>${row.CorrectiveAction || ''}</td>
				<td>${row.PreventiveAction || ''}</td>
      </tr>`;
    });
