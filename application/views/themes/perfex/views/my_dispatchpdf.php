<?php
	...
	$Customerlist = get_order_list_groupByCustomer($invoice->ChallanID);
	// echo count($Customerlist);
	// echo "<pre>";print_r($get_order_list);
	$count = 0;
	...
	foreach ($Customerlist as $key => $order_detail) {
		//$client_detail = get_client_detail($order_detail["AccountID"]);
		$html .= '<td colspan="2" align="center"><b>'.$order_detail["company"].'</b></td>';
	}
	...
	foreach ($Customerlist as $key => $order_detail) {
		//$client_detail = get_client_detail($order_detail["AccountID"]);
		$html .= '<td colspan="2" align="center"><b>'.$order_detail["StationName"].'</b></td>';
	}
	$html .= '<td align="center"><b></b></td>';
	$html .= '</tr>';
	...
	$html .= '<td align="center" colspan="4" width="'.$width1.'%"><b>Delivery Sequence</b></td>';
	foreach ($Customerlist as $key => $order_detail) {
		//$client_detail = get_client_detail($order_detail["AccountID"]);
		$html .= '<td colspan="2" align="center"><b>'.$order_detail["DeliveryPoint"].'</b></td>';
	}
	...
	// $html .= '<td colspan="2" align="center"> <b>'.round($order_detail["BillAmt"],2).'</b></td>';
	...
	foreach ($Customerlist as $key => $order_detail) {
		$html .= '<td align="center"><b>Pkt Qty</b></td>';
		$html .= '<td align="center"><b>CS/CR.</b></td>';
	}
	$html .= '<td align="center"><b>Total CS/CR</b></td>';
	$html .= '</tr>';
	$AllItemList = array();
	$AllItemListU = array();
	...
	// echo "<pre>";print_r($ItemDetailsNew);
	$i = 1;
	...
	foreach ($AllItemList as $key1 => $value1) {
		...
		foreach ($Customerlist as $key => $order_detail) {
			...
			$billedQty = '';
			foreach ($ItemDetailsNew as $key2 => $value2) {
				if($order_detail['AccountID'] == $value2["AccountID"] && $value1['ItemID'] == $value2["ItemID"]){
					$Qty += (int) round($value2["caseqty"]);
					$billedQty += (int) $value2["qty"] ;
					$rowTotal += $value2["caseqty"];
				}
			}
			
			$html .= '<td align="center;"><b>';
			$html .= !empty($billedQty) ? $billedQty : '';
			$html .= '</b></td>';
		...
		$html .= '<td align="center;"><b>'.round($rowTotal).'</b></td>';
		$html .= '</tr>';
	}
	...
	foreach ($Customerlist as $key => $order_detail) {
		...
		$AllQty = '';
		foreach ($ItemDetailsNew as $key2 => $value2) {
			if($order_detail['AccountID'] == $value2["AccountID"]){
				$billedQty += (int) $value2["qty"] ;
				$AllQty += (int) $value2["caseqty"] ;
			}
		}
		// $AllQty = $order_detail["Cases"] + $order_detail["Crates"];
		$html .= '<td colspan="" align="center"><b>'.$billedQty.'</b></td>';
		$html .= '<td colspan="" align="center"><b>'.$AllQty.'</b></td>';
		$SumQty += $AllQty;
	}
	...
	foreach ($Customerlist as $key => $order_detail) {
		$Newcases = '';
		foreach ($get_order_list as $key => $order_detail2) {
			if($order_detail2["AccountID"] == $order_detail["AccountID"]){
				$Newcases += $order_detail2["Cases"];
			}
		}
		$html .= '<td colspan="2" align="center"><b>'.$Newcases.'</b></td>';
		$AllCases += $Newcases;
	}
	...
	$NewCrates = '';
		foreach ($get_order_list as $key => $order_detail2) {
			if($order_detail2["AccountID"] == $order_detail["AccountID"]){
				$NewCrates += $order_detail2["Crates"];
			}
		}
        $html .= '<td colspan="2" align="center"><b>'.$NewCrates.'</b></td>';
        $AllCrates += $NewCrates;
	}
	...
	<td width="16%"><b>AccountName</b></td>
	<td width="12%"><b>Order ID</b></td>
	<td width="15%"><b>Order Date</b></td>
	<td width="12%"><b>Sales ID</b></td>
	<td width="15%"><b>Sales Date</b></td>
	<td width="10%"><b>Bill Amt</b></td>
	<td width="7.5%"><b>Credit</b></td>
	<td width="7.5%"><b>Fresh Return</b></td>
	</tr>';
	$sr = 1;
	foreach ($get_order_list as $key => $order_detail) {
		$empty_height = $empty_height - 35;
		...
		<td><b>'.round($order_detail["BillAmt"],2).'</b></td>
		<td><b></b></td>
		<td><b></b></td>
		</tr>';
		$sr++;
