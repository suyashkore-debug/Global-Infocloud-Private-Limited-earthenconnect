<?php
	...
	$html .= '<p style="text-align:center;padding-bottom:0px;margin-bottom:0px;"><b>PURCHASE ORDER</b></p>';
	...
	<th colspan="2" style="border-right: 1px solid black;" width="40%"><p style="text-align:center;font-size:10px;"><span>Invoice To</span><br/><b style="font-size:16px;">' . $PlantDetail->FIRMNAME . '</b><br>' . $PlantDetail->ADDRESS1 . ' ' . $PlantDetail->ADDRESS2 . '-'.$PlantDetail->ADDRESS3.'<br>PAN/IEC : ' . $PlantDetail->FLNO1 . '<br>GSTIN : ' . $PlantDetail->GSTNO . ', State Name : Uttar Pradesh, State Code : 09<br> E-Mail : Info@mygm.in </p></th>
	<th style="border-bottom: 1px solid #333;border-top: 1px solid #333;border-left: 1px solid #333; height:100px;" width="24%"><b>PO No : </b><b> ' . $order_detail["PurchID"] . '</b></th>
	
	
	<th width="26%" style="border-bottom: 1px solid #333;border-top: 1px solid #333;border-left: 1px solid #333;height:100px;"><b> Date : </b> <b>'._d(substr($order_detail["Transdate"], 0, 10)).'</b></th>
	</tr>';
    if (!empty($order_detail["ShipToAccoountName"])) {
    // --- Use order_detail values ---
    $consignee_name    = $order_detail["ShipToAccoountName"];
    $consignee_address = $order_detail["ShipToAddressFormatted"];
    $consignee_pan     = $order_detail["ShipToPanNumber"];
    $consignee_gstin   = $order_detail["ShipToGST"];
    $consignee_state   = $order_detail["ShipToStateName"] . ', State Code : 09';
    $consignee_email   = $order_detail["ShipToEmail"];
} else {
    // --- Use PlantDetail fallback ---
    $consignee_name    = $PlantDetail->FIRMNAME;
    $consignee_address = $PlantDetail->ADDRESS1 . ' ' . $PlantDetail->ADDRESS2 . ' - ' . $PlantDetail->ADDRESS3;
    $consignee_pan     = $PlantDetail->FLNO1;
    $consignee_gstin   = $PlantDetail->GSTNO;
    $consignee_state   = 'Uttar Pradesh, State Code : 09';
    $consignee_email   = 'Info@mygm.in';
}

$html .= '<tr>
    <th colspan="2" style="border-right: 1px solid black; border-top: 1px solid black; word-wrap: break-word;" width="50%">
			<p style="text-align:left; font-size:12px;"><span style="text-align:left;">Consignee (Ship to)</span><br/>
            <b style="font-size:16px;">' . $consignee_name . '</b><br>' . 
            $consignee_address . 
            '<br>PAN/IEC : ' . $consignee_pan . 
            '<br>GSTIN : ' . $consignee_gstin . ', State Name : ' . $consignee_state . 
            '<br>E-Mail : ' . $consignee_email . 
        '</p>
    </th>
    <th style="border-top: 1px solid #333; border-left: 1px solid #333;" width="50%"><b>Terms of Delivery</b> : 
		' . $order_detail["DeliveryTerms"] . '
    </th>
</tr>';
	...
	<th width="31%"><b>Name of Product</b></th>
	<th width="8%" ><b>Quantity</b></th>
	<th width="9%" ><b>Cases</b></th>
	<th  width="9%"><b>Unit</b></th>
	
	<th width="6%"  style="text-align:center;"><b>Rate</b></th>
	<th width="10%" style="text-align:center;"><b>Amount</b></th>
	<th width="9%"  style="text-align:center;"><b>GST</b></th>
	<th width="13%"  style="text-align:center;"><b>Total Amt.</b></th>';
    ...
    foreach ($inv_item as $item) {
		...
		$order_total += $amount;
		$NetAmt += $item['NetOrderAmt'];
        $html .= '<tr>';
        $html .= '<td width="5%" style="text-align:center;">' . $i . '</td>';
        $html .= '<td width="31%" class="description" align="left;"><b>' . $item['description'] . '</b></td>';
        $html .= '<td width="8%" style="text-align:right;" colspan="2"><b>' . $item['BilledQty'] . '</b></td>';
        $html .= '<td width="9%" style="text-align:right;" colspan="2"><b>' . $item['Cases'] . '</b></td>';
        $html .= '<td width="9%" style="text-align:right;"><b>' . $item['unit'] . '</b></td>';
		$html .= '<td width="6%" style="text-align:right;"><b>' . $item['PurchRate'] . '</b></td>';
        $html .= '<td width="10%" style="text-align:right;"><b>' . number_format($amount, 2, '.', '') . '</b></td>';
        $html .= '<td width="9%" style="text-align:right;"><b>' . $item['gst'] . '%</b></td>';
        $html .= '<td width="13%" style="text-align:right;"><b>' . $item['NetOrderAmt'] . '</b></td>';
    ...
    if (!empty($inv_item)) {
        $html .= '<tr>';
        $html .= '<td colspan="8" style="text-align:center;"><b>Total</b></td>';
    ...
    // $html .= '<tr>';
    // $html .= '<td colspan="6"><b>Company\'s PAN : ' . $PlantDetail->FLNO1 . '<br></b><b>Contact Person : </b><br><b>Contact Mobile : </b></td>';
    // $html .= '<td style="text-align:right;" colspan="7">For<b> ' . $PlantDetail->FIRMNAME . '<br><br><br></b>Authorized Signatory</td>';
    // $html .= '</tr>';
	$html .= '<tr>';
	$html .= '<td colspan="6"><b>Company\'s PAN : ' . $PlantDetail->FLNO1 . '<br></b><b>Contact Person : ' . $order_detail["ContactPersonName"] . ' </b><br><b>Contact Mobile : ' . $order_detail["ContactMobileNo"] . ' </b></td>';
	$html .= '<td style="text-align:right;" colspan="7">For<b> ' . $PlantDetail->FIRMNAME . '<br><br><br></b>Authorized Signatory</td>';
	$html .= '</tr>';
