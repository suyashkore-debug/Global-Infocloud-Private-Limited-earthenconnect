...
class Einvoice_model extends App_Model
{
    public function __construct()
    {
        parent::__construct();
    }
    public function sales_model_table($data){
          ...
          $to_date = '20'.$year_new.'-03-31';*/
          
          $challanIDS = array();
          $this->db->select('*');
          $this->db->join('tblclients', 'tblsalesmaster.AccountID = tblclients.AccountID AND tblsalesmaster.PlantID = tblclients.PlantID', 'left');
          $this->db->where('tblsalesmaster.PlantID', $selected_company);
          $this->db->where('tblsalesmaster.FY', $year);
          $this->db->where('tblsalesmaster.Transdate  BETWEEN "'. $from_date. ' 00:00:00" and "'. $to_date.' 23:59:59"');
          $this->db->where('tblsalesmaster.BT !=', 'B');
          $this->db->order_by('tblsalesmaster.SalesID ', 'DESC');
          $SalesList = $this->db->get('tblsalesmaster')->result_array();
          return $SalesList;
    ...
    public function get_json($data = '')
    {
	    ...
	    $this->db->join(db_prefix() . 'xx_citylist', db_prefix() . 'clients.city = '.db_prefix() . 'xx_citylist.id');
		...
		$newItemList = array();
		 foreach ($ItemList as $value) {
            # code...
            $newItemList[$i]['SlNo'] = $SlNo;
            $newItemList[$i]['PrdDesc'] = $value["hsn_code"];;
            $newItemList[$i]['IsServc'] = 'N';
            $newItemList[$i]['HsnCd'] = $value["hsn_code"];
            $newItemList[$i]['Barcde'] = null;
            $newItemList[$i]['Qty'] = $value["BilledQty"];
            $newItemList[$i]['FreeQty'] = 0;
            $newItemList[$i]['Unit'] = $value["unit"];
            $newItemList[$i]['UnitPrice'] = $value["BasicRate"];
            $newItemList[$i]['TotAmt'] = $value["ChallanAmt"];
            $newItemList[$i]['Discount'] = $value["DiscAmt"];
            $newItemList[$i]['PreTaxVal'] = 0.00;
            $newItemList[$i]['AssAmt'] = $value["ChallanAmt"];
            if($value["igst"] == NULL){
            ...
            $newItemList[$i]['TotItemVal'] = $value["NetChallanAmt"];
        ...
        $RndOffAmt = number_format(abs($rnd),2);
        ...
        $file_name = 'E-INV_V1_JSON_'.$party_name.'_'.$InvNo.'_'.$ChallanID;
        $m = date('m');
        ...
        $PercDocDtls_new = array();
        array_push($PercDocDtls_new, $PercDocDtls);*/
       /* $DocPerdDtls = array(
            "InvStDt"=>$InvDate,
            "InvEndDt"=>$InvDate
            );*/
        ...
        $ContrDtls_new = array();
        array_push($ContrDtls_new, $ContrDtls);*/
        /*$RefDtls = array(
            "InvRm"=>"Test remark",
            "DocPerdDtls"=>$DocPerdDtls,
            "PercDocDtls"=>$PercDocDtls_new,
            "ContrDtls"=>$ContrDtls_new
            );*/
        /*$AddlDocDtls = array(
            "Url"=>null,
            "Docs"=>null,
        ...
        $AddlDocDtls_new = array();
        array_push($AddlDocDtls_new, $AddlDocDtls);*/
    ...
    public function get_json_table($data = '')
    {
        $fy = $this->session->userdata('finacial_year');
        $selected_company = $this->session->userdata('root_company');
        ...
        $selected_company = $this->session->userdata('root_company');
         $year = $this->session->userdata('finacial_year');
            ...
            $data = $this->db->get('tblvehiclereturn')->result_array();
             ...
             $this->db->select('crate_data.AccountID as act_id,crate_data.Qty as crate_data_qty');
            ...
            foreach($data as $key=>$value){
                foreach($data_next as $value_data){
                    if($value['act_id'] ==$value_data['act_id']){
