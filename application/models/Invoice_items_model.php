	...
	class Invoice_items_model extends App_Model
	{
		...
		*/
		public function get($id = '')
		{
            ...
            description,long_description,item_code,BotanicalName,TraceStageImg,BannerTraceImg,NutritionalImg,IsTraceability,MainGrpID,SubGrpID1,SubGrpID2,brand_name,local_supply_in,outst_supply_in,crate_qty,case_qty,bowl_qty,min_qty,max_qty,weight,mrp,product_range,
            case_weight,min_day,monitorstock,hsn_code,rack_id,subrack_id,isactive,' . db_prefix() . 'items_groups.name as group_name,' . db_prefix() . 'items_sub_groups.name as subgroup_name,unit,image');
			...
			$this->db->join(db_prefix() . 'items_sub_group2', '' . db_prefix() . 'items_sub_group2.id = ' . db_prefix() . 'items.SubGrpID2', 'left');
			...
			if ($id) {
				...
				if($data){
					...
					$data->EditParameterList = $this->QcParameterByItemID($data->item_code);
				}
				return $data;
			}
		...
		public function QcParameterByItemID($ItemID)
		{
			$this->db->select('tblItemQcParameter.*');
			$this->db->from(db_prefix() .'ItemQcParameter');
			$this->db->where(db_prefix() .'ItemQcParameter.ItemID', $ItemID);
			$this->db->where(db_prefix() . 'ItemQcParameter.ItemID is NOT NULL', NULL, FALSE);
			return $this->db->get()->result_array();
		}
		...
		*/
		public function get2($id = '')
		{
            ...
            description,long_description,item_code,group_id,SubGrpID2,' . db_prefix() . 'items_groups.name as group_name,' . db_prefix() . 'items_sub_groups.name as subgroup_name,unit');
			$this->db->from(db_prefix() . 'items');
			$this->db->join('' . db_prefix() . 'taxes t1', 't1.id = ' . db_prefix() . 'items.tax', 'left');
			$this->db->join('' . db_prefix() . 'taxes t2', 't2.id = ' . db_prefix() . 'items.tax2', 'left');
			$this->db->join(db_prefix() . 'items_groups', '' . db_prefix() . 'items_groups.id = ' . db_prefix() . 'items.group_id', 'left');
			$this->db->join(db_prefix() . 'items_sub_groups', '' . db_prefix() . 'items_sub_groups.id = ' . db_prefix() . 'items.SubGrpID2', 'left');
			$this->db->order_by('description', 'asc');
			return $this->db->get()->result_array();
		}
		...
		// Add New ItemID
		public function SaveItemID($data,$StockQty,$ItemStatus_new,$parameters)
		{
			$selected_company = $this->session->userdata('root_company');
			if($selected_company == "1"){
				$GodownID = 'CSPL';
            ...
            foreach ($company_data as $key => $value) {
                ...
                if($INSERT > 0){
					...
					if (!empty($data['item_code']) && !empty($parameters)) 
                    {
            		    //insert new
            		    foreach($parameters as $paramID) 
            		    {
                            ...
                            $this->db->insert(db_prefix() . 'ItemQcParameter',$insertnew);
                        }
                    }
				}
			}
		...
		{
			$this->db->insert(db_prefix() . 'items_sub_group2', $data);
			$INSERT = $this->db->affected_rows();
			if($INSERT > 0){
				return true;    
				}else{
		...
		public function UpdateItemID($data,$StockQty,$item_code,$ItemStatus_new,$parameters)
		{
			$selected_company = $this->session->userdata('root_company');
			if($selected_company == "1"){
				$GodownID = 'CSPL';
            ...
            foreach ($company_data as $key => $value) 
            {
                ...
                if(!empty($item_code) && !empty($parameters)) 
                {
                    //Delete Item Parameters
                    $this->db->where('ItemID', $item_code);
            		$this->db->delete(db_prefix() . 'ItemQcParameter');
		...
		{
			$this->db->where('id', $itemGroupID);
			$this->db->update(db_prefix() . 'items_sub_group2', $data);
			$UPDATE = $this->db->affected_rows();        
			if($UPDATE > 0){
				return true;
				}else{
		...
		*/
		...
		public function get_table_data(){
            ...
            case_weight,min_day,monitorstock,hsn_code,rack_id,subrack_id,isactive,' . db_prefix() . 'items_groups.name as group_name,' . db_prefix() . 'items_main_groups.name as MainGrpName,' . db_prefix() . 'items_sub_groups.name as subgroup_name,' . db_prefix() . 'items_sub_group2.name as subgroup2_name,unit,image');
			$this->db->from(db_prefix() . 'items');
			$this->db->join('' . db_prefix() . 'taxes t1', 't1.id = ' . db_prefix() . 'items.tax', 'left');
			$this->db->join(db_prefix() . 'items_groups', '' . db_prefix() . 'items_groups.id = ' . db_prefix() . 'items.group_id', 'left');
		...
		public function get_last_recordItemGroup2(){
			$this->db->select('*');
			$this->db->from('tblitems_sub_group2');
			$this->db->order_by('id', 'DESC'); // 'created_at' is the column name of the date on which the record has stored in the database.
			$ItemGroupRecord =  $this->db->get()->row();
			return $ItemGroupRecord->id;
		}
		...
		public function getItemSubGroup2Details($ItemGroup2ID){
			...
			$this->db->from(db_prefix() . 'items_sub_group2');
			$this->db->where(db_prefix() . 'items_sub_group2.id', $ItemGroup2ID);
		...
		public function GetSubgroup1DataNew($filterdata ="")
		{
			...
			$this->db->where(db_prefix() . 'items_sub_groups.main_group_id', $MainItemGroup);
			$this->db->order_by(db_prefix() . 'items_sub_groups.name', 'ASC');
		...
		{
			$this->db->select(db_prefix() . 'items_sub_group2.*');
			$this->db->where(db_prefix() . 'items_sub_group2.main_group_id', $MainGroup);
		...
		{
			$this->db->select(db_prefix() . 'items_sub_group2.*');
			$this->db->where(db_prefix() . 'items_sub_group2.sub_group_id1', $SubGroup1);
			$this->db->order_by(db_prefix() . 'items_sub_group2.name', 'ASC');
			return $this->db->get('tblitems_sub_group2')->result_array();
		}
		public function GetSubgroup2DataNew($filterdata ="")
		{
			...
			$this->db->select(db_prefix() . 'items_sub_group2.*');
			$this->db->from(db_prefix() . 'items');
			$this->db->join(db_prefix() . 'items_sub_group2', 'items_sub_group2.id = tblitems.SubGrpID2','INNER');
			if(!empty($ItemType)){
				if($ItemType == 'NonTaxable'){
					$this->db->where('tblitems.tax', '1');
				}
		...
		public function GetSubGroup2ByGroupID($SubGroup1)
		{
			if(!empty($SubGroup1)){
				$this->db->select(db_prefix() . 'items_sub_group2.*');
				$this->db->where_in(db_prefix() . 'items_sub_group2.sub_group_id1', $SubGroup1);
				$this->db->order_by(db_prefix() . 'items_sub_group2.name', 'ASC');
				return $this->db->get('tblitems_sub_group2')->result_array();
				}else{
				return array();
		...
		public function GetItemList($filterdata)
		{
			...
			if($filterdata["SubGroup"]){
			    ...
			    $this->db->select(db_prefix().'items_sub_groups.id as item_code,'.db_prefix().'items_sub_groups.name as description_name');
			}
			if($filterdata["SubGroup"]){
		...
		public function GetOQtyItemList($filterdata)
		{
			...
			if($filterdata["SubGroup"]){
			    ...
			    $this->db->select('tblitems_sub_groups.id as ItemID,'.db_prefix().'items_sub_groups.name as description_name,SUM(tblstockmaster.OQty) AS OQty');
			}
		...
		public function get_item_open_qty($filterdata)
		{
			...
			if($SubGroup){
			    ...
			    $this->db->select(db_prefix().'items_sub_groups.id as ItemID,tblhistory.TType,tblhistory.TType2, SUM(BilledQty) as billsum,'.db_prefix().'items_sub_groups.name as description_name');
			}
			$this->db->join(db_prefix() . 'items', db_prefix() . 'items.item_code = ' . db_prefix() . 'history.ItemID AND '.db_prefix() . 'items.PlantID = ' . db_prefix() . 'history.PlantID');
		...
		public function GetStockData($filterdata)
		{
			...
			if($SubGroup){
			    ...
			    $this->db->select(db_prefix().'items_sub_groups.id as ItemID,,tblhistory.TType,tblhistory.TType2,tblhistory.BilledQty,'.db_prefix().'items_sub_groups.name as description_name');
			}
			$this->db->join(db_prefix() . 'items', db_prefix() . 'items.item_code = ' . db_prefix() . 'history.ItemID AND '.db_prefix() . 'items.PlantID = ' . db_prefix() . 'history.PlantID');
		...
		public function GetTopStockLevelChart($filterdata)
		{
			...
			if(!empty($MainItemGroup) && empty($SubGroup1)){
				$this->db->select('tblitems_sub_groups.id as ItemID,tblitems_sub_groups.name as ItemName,AVG(tblitems.case_qty) AS CaseQty');
			...
			if(!empty($MainItemGroup) && empty($SubGroup1)){
				$this->db->select('tblitems_sub_groups.id as ItemID,tblitems_sub_groups.name as ItemName,SUM(OQty) AS OQty');
				}elseif(!empty($SubGroup1) && empty($SubGroup2)){
				$this->db->select('tblitems_sub_group2.id as ItemID,tblitems_sub_group2.name as ItemName,SUM(OQty) AS OQty');
				}else{
			...
			if(!empty($MainItemGroup) && empty($SubGroup1)){
				$this->db->select('tblitems_sub_groups.id as ItemID,tblitems_sub_groups.name as ItemName,SUM(tblhistory.BilledQty) as BilledQty,tblhistory.TType,tblhistory.TType2');
				}elseif(!empty($SubGroup1) && empty($SubGroup2)){
				$this->db->select('tblitems_sub_group2.id as ItemID,tblitems_sub_group2.name as ItemName,SUM(tblhistory.BilledQty) as BilledQty,tblhistory.TType,tblhistory.TType2');
				}else{
		...
		public function GetTopMonthlyStockLevelChart($filterdata)
		{
			...
			if(!empty($MainItemGroup) && empty($SubGroup1)){
				$this->db->select('DATE_FORMAT(tblhistory.TransDate2, "%b-%Y") as month,tblitems_sub_groups.id as ItemID,tblitems_sub_groups.name as ItemName,SUM(tblhistory.BilledQty) as BilledQty,tblhistory.TType,tblhistory.TType2');
				}elseif(!empty($SubGroup1) && empty($SubGroup2)){
				$this->db->select('DATE_FORMAT(tblhistory.TransDate2, "%b-%Y") as month,tblitems_sub_group2.id as ItemID,tblitems_sub_group2.name as ItemName,SUM(tblhistory.BilledQty) as BilledQty,tblhistory.TType,tblhistory.TType2');
				}else{
		...
		public function GetTopDayWiseStockLevelChart($filterdata)
		{
			...
			* ------------------------ */
			if (!empty($MainItemGroup) && empty($SubGroup1)) {
				$this->db->select('tblitems_sub_groups.id as ItemID,SUM(OQty) AS OQty');
				} elseif (!empty($SubGroup1) && empty($SubGroup2)) {
				$this->db->select('tblitems_sub_group2.id as ItemID,SUM(OQty) AS OQty');
				} else {
			...
			* ------------------------ */
			if (!empty($MainItemGroup) && empty($SubGroup1)) {
				$this->db->select('DATE_FORMAT(tblhistory.TransDate2, "%d-%b-%Y") as day,tblitems_sub_groups.id as ItemID,SUM(tblhistory.BilledQty) as BilledQty,tblhistory.TType,tblhistory.TType2');
				} elseif (!empty($SubGroup1) && empty($SubGroup2)) {
				$this->db->select('DATE_FORMAT(tblhistory.TransDate2, "%d-%b-%Y") as day,tblitems_sub_group2.id as ItemID,SUM(tblhistory.BilledQty) as BilledQty,tblhistory.TType,tblhistory.TType2');
				} else {
		...
		/*public function GetStockValuesByMainGroupWiseOld($filterdata)
			{
			$fy = $this->session->userdata('finacial_year');
			$selected_company = $this->session->userdata('root_company');
		...
		public function GetStockValuesByMainGroupWise($filterdata)
		{
			...
			if (empty($MainItemGroup)) {
				$SubGroup1 = '';
				$filterdata['SubGroup1'] = '';
			}

			$groupIds = empty($MainItemGroup) ? ['1', '2', '3'] : [$MainItemGroup];
			...
			foreach ($groupIds as $grpId) {
				...
				if (!empty($GodownID)) {
					$this->db->select('tblitems.PlantID,
						tblitems.item_code,
						tblitems.crate_qty,
						tblitems.description,
						tblitems.case_qty,
						tblitems.unit,
						tblstockmaster.ItemID,
						tblstockmaster.OQty AS OQty,
						tblstockmaster.BasicRate AS StockRate');
					$this->db->from('tblitems');
					$this->db->join('tblstockmaster', 'tblstockmaster.ItemID = tblitems.item_code AND tblstockmaster.PlantID = tblitems.PlantID', 'inner');
					$this->db->where('tblitems.PlantID', $selected_company);
					$this->db->where('tblstockmaster.PlantID', $selected_company);
					$this->db->where('tblstockmaster.FY', $fy);
					$this->db->where('tblstockmaster.cnfid', '1');
					$this->db->where('tblstockmaster.GodownID', $GodownID);
					$this->db->where('tblitems.MainGrpID', $grpId);
					if (!empty($SubGroup1)) {
						$this->db->where('tblitems.SubGrpID1', $SubGroup1);
					}
				} else {
					$this->db->select('tblitems.PlantID,
						...
						tblitems.case_qty,
						tblitems.unit,
						(SELECT SUM(tblstockmaster.OQty) AS OQty
							FROM tblstockmaster
							WHERE tblstockmaster.ItemID = tblitems.item_code
								AND tblstockmaster.PlantID = ' . $selected_company . '
								AND tblstockmaster.FY = "' . $fy . '"
								AND tblstockmaster.cnfid = "1"
							GROUP BY tblstockmaster.ItemID, tblstockmaster.PlantID, tblstockmaster.FY) AS OQty,
						(SELECT ROUND(AVG(tblstockmaster.BasicRate), 2) AS StockRate
							FROM tblstockmaster
							WHERE tblstockmaster.ItemID = tblitems.item_code
								AND tblstockmaster.PlantID = ' . $selected_company . '
								AND tblstockmaster.FY = "' . $fy . '"
								AND tblstockmaster.cnfid = "1"
							GROUP BY tblstockmaster.ItemID, tblstockmaster.PlantID, tblstockmaster.FY) AS StockRate');
					$this->db->from('tblitems');
					$this->db->where('tblitems.PlantID', $selected_company);
					$this->db->where('tblitems.MainGrpID', $grpId);
					if (!empty($SubGroup1)) {
				...
				if (!empty($ItemType)) {
					if ($ItemType == 'NonTaxable') {
						$this->db->where('tblitems.tax', '1');
					} elseif ($ItemType == 'Taxable') {
						$this->db->where('tblitems.tax !=', '1');
					}
				}
				$this->db->order_by('tblitems.SubGrpID1', 'ASC');
				$AllItemList = $this->db->get()->result_array();

				if (empty($AllItemList)) {
					continue;
				}

				$this->db->select('tblhistory.*,tblitems.item_code');
				$this->db->from('tblhistory');
				...
				if ($from_date != $from_date_value) {
					if (!empty($GodownID)) {
						$sql = 'SELECT tblhistory.TType,
										tblhistory.TType2,
										tblhistory.ItemID,
										tblstockmaster.OQty,
										SUM(tblhistory.BilledQty) AS billsum
								FROM `tblhistory`
								INNER JOIN tblitems
									ON tblitems.item_code = tblhistory.ItemID
									AND tblitems.PlantID = tblhistory.PlantID
								LEFT JOIN tblstockmaster
									ON tblstockmaster.ItemID = tblhistory.ItemID
									AND tblstockmaster.PlantID = tblhistory.PlantID
									AND tblstockmaster.FY = tblhistory.FY
									AND tblstockmaster.cnfid = "1"
								WHERE tblhistory.PlantID = ' . $selected_company . '
									AND tblhistory.FY = "' . $fy . '"
									AND tblhistory.TransDate BETWEEN "' . $first_date . ' 00:00:00" AND "' . $day_before . ' 23:59:59"
									AND tblitems.MainGrpID = "' . $grpId . '"';
						if (!empty($SubGroup1)) {
							$sql .= ' AND tblitems.SubGrpID1 = "' . $SubGroup1 . '"';
						}
						if (!empty($ItemType)) {
							if ($ItemType == 'NonTaxable') {
								$sql .= ' AND tblitems.tax = "1"';
							} elseif ($ItemType == 'Taxable') {
								$sql .= ' AND tblitems.tax != "1"';
							}
						}
						$sql .= ' AND tblhistory.BillID IS NOT NULL';
						if (!empty($GodownID)) {
						...
						$sql = 'SELECT tblhistory.TType,
										tblhistory.TType2,
										tblhistory.ItemID,
										(SELECT SUM(tblstockmaster.OQty)
											FROM tblstockmaster
											WHERE tblstockmaster.ItemID = tblhistory.ItemID
												AND tblstockmaster.PlantID = ' . $selected_company . '
												AND tblstockmaster.FY = "' . $fy . '"
												AND tblstockmaster.cnfid = "1"
											GROUP BY tblstockmaster.ItemID, tblstockmaster.PlantID, tblstockmaster.FY) AS OQty,
					...
					$StockOQtyData = $this->db->query($sql)->result_array();
				}

				$groupStockValue = 0.0;

				foreach ($AllItemList as $value) {
					$rate = 0;
					...
					foreach ($StockData as $row) {
						if (trim(strtoupper($value['item_code'])) != trim(strtoupper($row['ItemID']))) {
							continue;
						}

						if ($row['TType'] == 'P' && $row['TType2'] == 'Purchase') {
							$PurchQty += $row['BilledQty'];
							if ($row['SaleRate'] != '' && $row['SaleRate'] != null && $row['SaleRate'] > 0) {
								$rate = $row['SaleRate'];
							}
						}
						if ($row['TType'] == 'I' && $row['TType2'] == 'Inward') {
							$InwardQty += $row['BilledQty'];
							if ($row['SaleRate'] != '' && $row['SaleRate'] != null && $row['SaleRate'] > 0) {
								$rate = $row['SaleRate'];
							}
						}
						if ($row['TType'] == 'N' && $row['TType2'] == 'PurchaseReturn') {
							$PurchRtnQty += $row['BilledQty'];
							if ($row['SaleRate'] != '' && $row['SaleRate'] != null && $row['SaleRate'] > 0) {
								$rate = $row['SaleRate'];
							}
						}
						if ($row['TType'] == 'A' && $row['TType2'] == 'Issue') {
							$IssueQty += $row['BilledQty'];
							if ($row['SaleRate'] != '' && $row['SaleRate'] != null && $row['SaleRate'] > 0) {
								$rate = $row['SaleRate'];
							}
						}
						if ($row['TType'] == 'B' && $row['TType2'] == 'Production') {
							$PRDQty += $row['BilledQty'];
							if ($row['SaleRate'] != '' && $row['SaleRate'] != null && $row['SaleRate'] > 0) {
								$rate = $row['SaleRate'];
							}
						}
						if ($row['TType'] == 'O' && $row['TType2'] == 'Order') {
							$SalesQty += $row['BilledQty'];
							if ($row['SaleRate'] != '' && $row['SaleRate'] != null && $row['SaleRate'] > 0) {
								$rate = $row['SaleRate'];
							}
						}
						if ($row['TType'] == 'R' && $row['TType2'] == 'Fresh') {
							$SalesRtnQty += $row['BilledQty'];
							if ($row['SaleRate'] != '' && $row['SaleRate'] != null && $row['SaleRate'] > 0) {
								$rate = $row['SaleRate'];
							}
						}
						if ($row['TType'] == 'X' && (
							$row['TType2'] == 'Free Distribution' ||
							$row['TType2'] == 'Free distribution' ||
							$row['TType2'] == 'Promotional Activity' ||
							$row['TType2'] == 'Stock Adjustment' ||
							$row['TType2'] == 'IssueAgainstReturn'
						)) {
							$AdjQty += $row['BilledQty'];
							if ($row['SaleRate'] != '' && $row['SaleRate'] != null && $row['SaleRate'] > 0) {
								$rate = $row['SaleRate'];
							}
						}
						if ($row['TType'] == 'T' && $row['TType2'] == 'Out') {
							$GOQty += $row['BilledQty'];
							if ($row['SaleRate'] != '' && $row['SaleRate'] != null && $row['SaleRate'] > 0) {
								$rate = $row['SaleRate'];
							}
						}
						if ($row['TType'] == 'T' && $row['TType2'] == 'In') {
							$GIQty += $row['BilledQty'];
							if ($row['SaleRate'] != '' && $row['SaleRate'] != null && $row['SaleRate'] > 0) {
								$rate = $row['SaleRate'];
							}
					...
					$CaseQty = ($value['case_qty'] == '0' || $value['case_qty'] == '') ? 1 : $value['case_qty'];

					// Opening quantity in cases – same as Unit Wise Stock Position
					if ($from_date == $from_date_value) {
						$OQTYCases = !empty($value['OQty']) ? (float)$value['OQty'] : 0.0;
					} else {
						$OQtySum = !empty($value['OQty']) ? (float)$value['OQty'] : 0.0;
						foreach ($StockOQtyData as $valueOQty) {
							if (trim(strtoupper($valueOQty['ItemID'])) != trim(strtoupper($value['item_code']))) {
								continue;
							}
							...
							if ($valueOQty['TType'] == 'I' && $valueOQty['TType2'] == 'Inward') {
								$OQtySum += $valueOQty['billsum'];
							}
							if ($valueOQty['TType'] == 'N') {
								$OQtySum -= $valueOQty['billsum'];
							}
							if ($valueOQty['TType'] == 'A' && $valueOQty['TType2'] == 'Issue') {
								$OQtySum -= $valueOQty['billsum'];
							}
							if ($valueOQty['TType'] == 'B') {
								$OQtySum += $valueOQty['billsum'];
							}
						...
						$OQTYCases = (float)$OQtySum;
					}

					$PurchQtyCases    = (float)$PurchQty;
					$InwardQtyCases   = (float)$InwardQty;
					$PurchRtnQtyCases = (float)$PurchRtnQty;
					$IssueQtyCases    = (float)$IssueQty;
					$PRDCases         = (float)$PRDQty;
					$SalesCases       = (float)$SalesQty;
					$SalesRtnCases    = (float)$SalesRtnQty;
					$AdjCases         = (float)$AdjQty;
					$GOCases          = (float)$GOQty;
					$GICases          = (float)$GIQty;
					...
					// skip pure-zero rows like UWSP
					if (
						$OQTYCases == 0.0 && $PurchQtyCases == 0.0 && $InwardQtyCases == 0.0 &&
						$PurchRtnQtyCases == 0.0 && $IssueQtyCases == 0.0 && $PRDCases == 0.0 &&
						$SalesCases == 0.0 && $SalesRtnCases == 0.0 && $AdjCases == 0.0 &&
						$GOCases == 0.0 && $GICases == 0.0
					) {
						continue;
					}

					// If no SaleRate found in the period, fall back to StockRate like Unit Wise Stock Position
					if ($rate <= 0 && isset($value['StockRate']) && $value['StockRate'] > 0) {
						$rate = $value['StockRate'];
					}

					// Valuation in cases (not multiplied by case size) – same as report
					if ($CaseQty == 0 || $CaseQty == '' || $CaseQty == '0') {
						$stockqty = round($BQty) * 1;
					} else {
						$stockqty = round($BQty);
					}
					...
					$groupStockValue += $stockValue;
				}

				// Attach this group's value to the right bucket
				if ($grpId == '1') {
					$FGValue += $groupStockValue;
				} elseif ($grpId == '2') {
					$RMValue += $groupStockValue;
				} elseif ($grpId == '3') {
					$PMValue += $groupStockValue;
				}
			}
		...
		public function get_all_parameter_list()
		{
		    return $this->db
            ->select('*')
            ->from('tblQcMaster')
