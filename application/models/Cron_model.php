<?php
	...
	class Cron_model extends App_Model
	{
		...
		private function deliveryOverdue()
		{
			// echo "ok"die;
			// User events
			...
			$this->db->select('tblordermaster.*,tblchallanmaster.Gatepassuserid,tblchallanmaster.GetPassTime');
			$this->db->join(db_prefix() . 'challanmaster', db_prefix() . 'challanmaster.ChallanID = '.db_prefix() . 'ordermaster.ChallanID ');
			$this->db->where('tblordermaster.Dispatchdate <',$Currant_datetime);
			$this->db->where('tblchallanmaster.Gatepassuserid IS NULL');
			$this->db->where('tblordermaster.Dispatchdate IS NOT NULL');
			$this->db->where('tblordermaster.OrderID','ORD25115278');
			// $this->db->where('tblordermaster.PlantID',$selected_company);
			// $this->db->where('tblordermaster.FY',$FY);
			$OrderList = $this->db->get(db_prefix() . 'ordermaster')->result_array();
			...
			$this->db->where(db_prefix() . 'hr_job_position.department_id', '11');
			$TransportStaff = $this->db->get(db_prefix() . 'staff')->result_array();
			// echo "<pre>";print_r($TransportStaff);die;
			
			foreach($OrderList as $key=>$val){
				if(($val["Gatepassuserid"] == NULL || $val["Gatepassuserid"] == "")){
					/*echo $sr." => ".$val["OrderID"];*/
					array_push($challanlist,$val["ChallanID"]);
					/*echo "<br>";
					$sr++;*/
					
					
					if($val["notification_count"] <= 5){
						foreach($TransportStaff as $Staff){
							// echo "ok";die;
							$Notification_msg = "Dispatch delayed for Order ".$val["OrderID"]." (Expected Dispatch Date - ".substr(_d($val["Dispatchdate"]),0,16).") of Challan ".$val["ChallanID"]." by ".time_delay($val["Dispatchdate"]);
							$notification_data = [
							'description' => $Notification_msg,
							'touserid'    => $Staff['staffid'],
							'fromcompany'     => 1,
							'fromuserid'      => 0,
							'link'        => 'order/order/'.$val["OrderID"],
							]; 
							$notification_data['additional_data'] = serialize([
							'Order No. '.$val["OrderID"],
							]);
						...
						'notification_count' => $val["notification_count"] + 1,
						]);
					}
				}
			}
			
			$challanlist_unique = array_unique($challanlist);
			//echo count($challanlist_unique);
			...
			foreach($AdminStaff as $admin){
				// echo "ok";die;
				$Notification_msg = "Total Delivery Overdue Challan Count is (".count($challanlist_unique);
				$notification_data = [
				'description' => $Notification_msg,
				'touserid'    => $admin['staffid'],
				'fromcompany'     => 1,
				'fromuserid'      => 0,
				'link'        => 'challan/view_gatepass?status=NotGenerated',
				]; 
				$notification_data['additional_data'] = serialize([
				'Purch No. '.$id,
				]);
		...
		private function events()
		{
			...
			$whereStaff = 'tblstaff.active=1 AND tblstaff.is_not_staff=0';
			if (count($notified_users) > 0) {
				$whereStaff .= ' AND tblstaff.staffid NOT IN (' . implode(',', $notified_users) . ')';
			}
