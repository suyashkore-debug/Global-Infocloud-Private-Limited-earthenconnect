<?php
	...
	class Accounts_master_model extends App_Model
	{
		...
		{
			$this->db->select(db_prefix() . 'accountgroupssub1.*,tblaccountgroups.ActGroupName');
			$this->db->join(db_prefix() . 'accountgroups', '' . db_prefix() . 'accountgroups.ActGroupID = ' . db_prefix() . 'accountgroupssub1.ActGroupID');
			if($main_group){
			    $this->db->where(db_prefix() . 'accountgroupssub1.ActGroupID',$main_group);
			}
		...
		{
			$this->db->select(db_prefix() . 'accountgroupssub2.*,tblaccountgroups.ActGroupName,tblaccountgroupssub1.SubActGroupName AS SubActGroupName1');
			$this->db->join(db_prefix() . 'accountgroupssub1', '' . db_prefix() . 'accountgroupssub1.SubActGroupID1 = ' . db_prefix() . 'accountgroupssub2.SubActGroupID1');
			$this->db->join(db_prefix() . 'accountgroups', '' . db_prefix() . 'accountgroups.ActGroupID = ' . db_prefix() . 'accountgroupssub1.ActGroupID');
			if($main_group){
			    $this->db->where(db_prefix() . 'accountgroupssub1.ActGroupID',$main_group);
			}
			if($subgroup1){
			    $this->db->where(db_prefix() . 'accountgroupssub1.SubActGroupID1',$subgroup1);
			}
		...
		public function get_accounts_list($data = '')
		{
			...
			$sql = 'SELECT tblclients.*,tblaccountgroupssub.SubActGroupName AS SubActGroupName2,tblaccountgroups.ActGroupName As MainGroupName,tblaccountgroupssub1.SubActGroupName AS SubActGroupName1 FROM tblclients 
			LEFT JOIN tblaccountgroups ON tblaccountgroups.ActGroupID = tblclients.ActGroupID
			LEFT JOIN tblaccountgroupssub1 ON tblaccountgroupssub1.SubActGroupID1 = tblclients.SubActGroupID1
		...
		{                          
			...
			if ($id) {
				...
				if($Data){
					$Data->SubGroupData1 = $this->db->query("select * from tblaccountgroupssub1 where ActGroupID = '".$Data->ActGroupID."'")->result();
		...
		{  
			$sql ='SELECT '.db_prefix().'accountgroups.ActGroupID
			FROM '.db_prefix().'accountgroups ORDER BY ActGroupID DESC LIMIT 1';
			$result = $this->db->query($sql)->row();
		...
		public function GetAccountSubGroupID3($InitialAccountSubGroupID1)
		{
			...
			$sql = "SELECT `tblaccountgroupssub2`.*, `tblaccountgroupssub1`.`SubActGroupID1` as `SubActID1`, `tblaccountgroupssub1`.`SubActGroupName` as `SubActGroupName1`, `tblaccountgroups`.`ActGroupID` as `ActGroupIDMain`, `tblaccountgroups`.`ActGroupName` as `ActGroupNameMain`
			FROM `tblaccountgroupssub2`
			LEFT JOIN `tblaccountgroupssub1` ON `tblaccountgroupssub1`.`SubActGroupID1` = `tblaccountgroupssub2`.`SubActGroupID1`
			LEFT JOIN `tblaccountgroups` ON `tblaccountgroups`.`ActGroupID` = `tblaccountgroupssub1`.`ActGroupID`
		...
		public function GetAllAccountSubGroupID2_List($main_group = '',$subgroup1 = '')
		{
			...
			$sql ='SELECT '.db_prefix().'accountgroupssub.*,tblaccountgroupssub1.SubActGroupName AS SubGroup1,tblaccountgroups.ActGroupName AS MainGroup
			FROM '.db_prefix().'accountgroupssub
			LEFT JOIN tblaccountgroupssub1 ON tblaccountgroupssub1.SubActGroupID1 = tblaccountgroupssub.SubActGroupID1
			LEFT JOIN tblaccountgroups ON tblaccountgroups.ActGroupID = tblaccountgroupssub1.ActGroupID
		...
		{  
			$sql ='SELECT '.db_prefix().'accountgroupssub.*,'.db_prefix().'accountgroupssub1.SubActGroupID1,'.db_prefix().'accountgroupssub1.SubActGroupName AS SubActGroupName1,'.db_prefix().'accountgroups.ActGroupID,'.db_prefix().'accountgroups.ActGroupName AS ActGroupNameMain
			FROM '.db_prefix().'accountgroupssub
			INNER JOIN '.db_prefix().'accountgroupssub1 ON '.db_prefix().'accountgroupssub1.SubActGroupID1 = '.db_prefix().'accountgroupssub.SubActGroupID1
		...
		public function GetSubGroupOneByMainGroupId($Account_Group){
			...
			$ss = "SELECT * FROM tblaccountgroupssub1 WHERE ActGroupID=".$Account_Group."";
			$result_data = $this->db->query($ss)->result_array();
			return $result_data;
		}
