	...
	class Godown_model extends App_Model
	{
		...
		function itemlist($postData){
			...
			if(isset($postData['search']) ){
				...
				$where_items .= '(description LIKE "%' . $q . '%" ESCAPE \'!\' OR item_code LIKE "%' . $q. '%")';
		...
		function GetTransDetails($TransID){
			...
			$FY = $this->session->userdata('finacial_year');
			$html = "";
			$this->db->select('tblTransferMaster.TransID,tblTransferMaster.Transdate,tblTransferMaster.Transdate2,tblTransferMaster.TransFrom,tblTransferMaster.TransTo,tblgodownmaster.AccountName AS FromName,G2.AccountName AS ToName');
			...
			if($records){
				...
				foreach($ItemDetails as $value){
							...
							$OQty = $value["OQty"];
							foreach ($Stocks as $stock) {
								if($value['ItemID']== $stock['ItemID'] && $stock['TType'] == 'P' && $stock['TType2'] == 'Purchase'){
									$PQty = $stock['BilledQty'];
									}elseif($value['ItemID']== $stock['ItemID'] && $stock['TType'] == 'N'){
									$PRQty = $stock['BilledQty'];
									}elseif($value['ItemID']== $stock['ItemID'] && $stock['TType'] == 'A'){
									$IQty = $stock['BilledQty'];
									}elseif($value['ItemID']== $stock['ItemID'] && $stock['TType'] == 'B'){
									$PRDQty = $stock['BilledQty'];
									}elseif($value['ItemID']== $stock['ItemID'] && $stock['TType'] == 'O' && $stock['TType2'] == 'Order'){
									$SQty = $stock['BilledQty'];
									}elseif($value['ItemID']== $stock['ItemID'] && $stock['TType'] == 'R' && $stock['TType2'] == 'Fresh'){
									$SRTQty = $stock['BilledQty'];
									}elseif($value['ItemID']== $stock['ItemID'] && $stock['TType'] == 'X'){
									$AQty = $stock['BilledQty'];
									}elseif($value['ItemID']== $stock['ItemID'] && $stock['TType'] == 'T'  && $stock['TType2'] == 'In'){
									$GIQty = $stock['BilledQty'];
									}elseif($value['ItemID']== $stock['ItemID'] && $stock['TType'] == 'T'   && $stock['TType2'] == 'Out'){
									$GOQty = $stock['BilledQty'];
								}
							}
							$stockQty = $OQty + $PQty - $PRQty - $IQty + $PRDQty - $SQty + $SRTQty - $AQty + $GIQty - $GOQty;
		...
		public function TransItemDetails($TransID,$AccountID)
		{
			...
			$this->db->from(db_prefix() .'history');
			$this->db->join('tblstockmaster ', 'tblstockmaster.ItemID = tblhistory.ItemID AND tblstockmaster.PlantID = tblhistory.PlantID AND tblstockmaster.FY = tblhistory.FY AND tblstockmaster.GodownID = "'.$AccountID.'"','LEFT');
