	...
	class Challan_model extends App_Model
	{
		...
		*/
		...
		public function getNew($ChallanID){
			...
			$this->db->select(db_prefix() . 'ordermaster.*,'.db_prefix() . 'salesmaster.Transdate AS TransDate,'.db_prefix() . 'ordermaster.SalesID,'.db_prefix() . 'clients.company,'.db_prefix() . 'clients.state,'.db_prefix() . 'clients.DistributorType,'.db_prefix() . 'clients.MaxCrdAmt,'.db_prefix() . 'contacts.istcs,'.db_prefix() . 'salesmaster.irn,'.db_prefix() . 'salesmaster.DeliveryPoint,tblaccountroutes.RouteID,tblroute.name As RouteName');
			$this->db->join(db_prefix() . 'ordermaster', '' . db_prefix() . 'ordermaster.ChallanID = ' . db_prefix() . 'challanmaster.ChallanID');
		...
		*/
		...
		public function UpdateCratesCasesAfterGatepass($data,$ChallanID)
		{
			$CrateData = $data['CrateData'];
			$CaseData = $data['CaseData'];
			
			$TotalChlCrates = 0;
			$TotalChlCases = 0;
			foreach($CrateData as $OrderID => $CrateValue){
				if(empty($CrateValue)){
					$CrateValue = 0;
				}
				$TotalChlCrates += $CrateValue;
				$crate_update_array = [
				'Crates' => $CrateValue,
				]; 
				$this->db->where('OrderID', $OrderID);
				$this->db->where('ChallanID', $ChallanID);
				$this->db->update(db_prefix() . 'ordermaster', $crate_update_array);
			}
			foreach($CaseData as $OrderID => $CaseValue){
				if(empty($CaseValue)){
					$CaseValue = 0;
				}
				$TotalChlCases += $CaseValue;
				$case_update_array = [
				'Cases' => $CaseValue,
				]; 
				$this->db->where('OrderID', $OrderID);
				$this->db->where('ChallanID', $ChallanID);
				$this->db->update(db_prefix() . 'ordermaster', $case_update_array);
			}
			...
			foreach ($order_ids as $orderid1) {
				...
				if($order_data1->credit_apply == 'Y'){
					if($FinalAccountWiseOrders[$order_data1->AccountID]['OrderAbleAmt'] >= $ChkNetOrderAmt){
						...
						$this->db->select(db_prefix() . 'staff.*');
						$this->db->join(db_prefix() . 'hr_job_position', db_prefix() . 'hr_job_position.position_id = '.db_prefix() . 'staff.job_position');
						$this->db->where(db_prefix() . 'staff.PlantID', $selected_company);
						$this->db->where(db_prefix() . 'hr_job_position.department_id', '4');
						$AccountStaff = $this->db->get(db_prefix() . 'staff')->result_array();
						foreach($AccountStaff as $Account){
							
							$Notification_msg = "Credit Limit Reached! Challan can not be generated for order ".$orderid1;
							$notification_data = [
							'description' => $Notification_msg,
							'touserid'    => $Account['staffid'],
							'link'        => 'order/LimitExceededOrders',
							]; 
							$notification_data['additional_data'] = serialize([
							'Order ID. '.$orderid1,
							...
							if (add_notification($notification_data)) {
								pusher_trigger_notification($Account['staffid']);
							}
						}
					}
				}
						...
						if($order_data){
							...
							foreach($GetMultipleScheme as $each){
								if($each["TotalBilledQty"] >= $each['SlabQty'] &&  $each['SlabQty'] > 0){
									...
									$ItemRate =  $this->get_rate_master_data_by_id2($each['FreeItemID'],$disttype,$state);
							...
							foreach($SingleFreeItem as $ItemID => $FreeQty){
								...
								$ItemRate =  $this->get_rate_master_data_by_id2($ItemID,$disttype,$state);
								$Rate = $ItemRate->assigned_rate;
		...
		*/
		...
		public function get_order_entry_details_PO($id)
		{
			...
			$this->db->select('tblpurchaseordermaster.*,tblpurchaseordermaster.AccountID As Vendor,tblclients.*,tblxx_statelist.*,tblaccountbalances.*,tblxx_citylist.city_name');
		...
		public function get_detailed_shortage_list($from_date = null, $to_date = null, $AccountID = null, $VehicleID = null, $DriverID = null)
		{
			$prefix = db_prefix();
			...
			if ($DriverID) {
				$this->db->where('cm.DriverID ', $DriverID);
			}
		...
		public function PartyListBySales(){
			$selected_company = $this->session->userdata('root_company');
			$fy = $this->session->userdata('finacial_year');
			
			$this->db->select('tblclients.AccountID,tblclients.company');
			$this->db->from(db_prefix() . 'salesmaster');
			$this->db->join(db_prefix() . 'clients', 'tblclients.AccountID = tblsalesmaster.AccountID');
			$this->db->group_by(db_prefix() . 'salesmaster.AccountID');
			return $this->db->get()->result_array();
			
		}
		
		public function DriverListByChallan(){
			$selected_company = $this->session->userdata('root_company');
			$fy = $this->session->userdata('finacial_year');
			
			$this->db->select('tblstaff.AccountID,CONCAT_WS(" ", tblstaff.firstname, tblstaff.lastname) as DriverName');
			$this->db->from(db_prefix() . 'challanmaster');
			$this->db->join(db_prefix() . 'staff', 'tblstaff.AccountID = tblchallanmaster.DriverID');
			$this->db->group_by(db_prefix() . 'challanmaster.DriverID');
			return $this->db->get()->result_array();
			
		}
		public function VehicleListByChallan(){
			$selected_company = $this->session->userdata('root_company');
			$fy = $this->session->userdata('finacial_year');
