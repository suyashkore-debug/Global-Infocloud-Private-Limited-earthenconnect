...
//======================= Get Item Group List By Main GroupID ==================
	...
	public function GetItemGroupByMainGroupID($id = '')
	...
	{	
		...
		public function GetItemListBySubgroup($data)

		{	

			$this->db->select('tblitems.*,' . db_prefix() . 'items_sub_groups.name as subgroup_name');

			$this->db->join(db_prefix() . 'items_sub_groups', '' . db_prefix() . 'items_sub_groups.id = ' . db_prefix() . 'items.SubGrpID1', 'left');

			if($data['Subgroup'] != ''){

				$this->db->where('SubGrpID2',$data['Subgroup']);

			}

			$this->db->where('MainGrpID','1');
		...
		public function GetItemListBySubgroupNew($data)
		...
		{	
			...
			if(!empty($data['Subgroup'])){
			    ...
			    $this->db->select(db_prefix().'items_sub_groups.id as item_code,'.db_prefix().'items_sub_groups.name as description');
		...
		{  
			...
			$this->db->select(db_prefix() .'rate_master.item_id,'.db_prefix() .'rate_master.distributor_id,'.db_prefix() .'rate_master.SaleRate,'.db_prefix() .'rate_master.assigned_rate,'.db_prefix() .'rate_master.effective_date,'.db_prefix() . 'items.description,'.db_prefix() . 'items.case_qty,'.db_prefix() . 'items.mrp,'.db_prefix() . 'items.crate_qty ,'.db_prefix() .'taxes.taxrate');
			...
			if($data['Subgroup'] != ''){
				...
				$this->db->where(db_prefix() .'items.SubGrpID2',$data['Subgroup']);

			}

			

			$this->db->where(db_prefix() .'rate_master.effective_date <=', $curDate);

			$this->db->where(db_prefix() .'items.MainGrpID','1');

			$this->db->group_by('tblrate_master.distributor_id,tblrate_master.item_id');

			$this->db->order_by(db_prefix() .'rate_master.effective_date', 'DESC');
		...
		//======================= Get Staff Wise Party List ============================

		public function GetStaffWisePartyList($data)
		...
		{	
			...
			$this->db->join(db_prefix() . 'clients', db_prefix() . 'clients.AccountID = ' . db_prefix() . 'customer_admins.customer_id');
		...
		//================== Get Staff Wise Party Wise Target ==========================

		public function GetStaffWisePartyWiseTargate($data)
		...
		{	
			...
			$this->db->select('tblstaff_target.*');

			$this->db->from('tblstaff_target');

			$this->db->where('tblstaff_target.PlantID', $selected_company);

			$this->db->where('tblstaff_target.FY', $year);

			if($data['staff_d'] != ''){

				$this->db->where('tblstaff_target.Staff_AccountID', $data['staff_d']);

			}

			$this->db->where('tblstaff_target.MonthID', $data['month_data']);

			$this->db->order_by('tblstaff_target.AccountID');
		...
		public function GetItemWiseTarget($data)
		...
		{	
			...
			$this->db->select('tblItemWiseSaleTarget.*');

			$this->db->from('tblItemWiseSaleTarget');

			$this->db->where('tblItemWiseSaleTarget.PlantID', $selected_company);

			$this->db->where('tblItemWiseSaleTarget.FY', $year);

			$this->db->where('tblItemWiseSaleTarget.MonthID', $data['month_data']);

			$this->db->order_by('tblItemWiseSaleTarget.ItemID');
		...
		public function GetItemWiseTargetNew($data)
		...
		{	
			...
			if(!empty($data['Subgroup'])){
			    ...
			    $this->db->select(db_prefix().'ItemWiseSaleTarget.*');

				}else{

			    $this->db->select(db_prefix().'items_sub_groups.id as ItemID,tblItemWiseSaleTarget.*');

			}

			$this->db->from('tblItemWiseSaleTarget');

			$this->db->join(db_prefix() . 'items', '' . db_prefix() . 'items.item_code = ' . db_prefix() . 'ItemWiseSaleTarget.ItemID', 'left');
		...
		public function StaffWiseSaleAchievment($data)
		...
		{	
			...
			$sql = "SELECT tblitems.SubGrpID1,tblhistory.AccountID,SUM(tblhistory.NetChallanAmt) AS Amount FROM `tblhistory` INNER JOIN tblcustomer_admins ON tblcustomer_admins.customer_id = tblhistory.AccountID INNER JOIN tblitems ON tblitems.item_code = tblhistory.ItemID WHERE MONTH(tblhistory.TransDate) = '".$data['month_data']."' AND tblhistory.PlantID = '".$selected_company."' AND tblhistory.FY = '".$year."' AND tblhistory.TType = 'O' AND tblhistory.TType2 = 'Order' AND tblcustomer_admins.staff_id='".$data['staff_d']."' GROUP BY tblitems.SubGrpID1,tblhistory.AccountID";
		...
		public function SaleAchievment($data)
		...
		{	
			...
			$sql = "SELECT tblhistory.ItemID,tblitems.SubGrpID1,tblhistory.AccountID,SUM(tblhistory.BilledQty) AS SaleQty,SUM(tblhistory.NetChallanAmt) AS SaleAmount FROM `tblhistory` 

			INNER JOIN tblitems ON tblitems.item_code = tblhistory.ItemID WHERE MONTH(tblhistory.TransDate) = '".$data['month_data']."' AND tblhistory.PlantID = '".$selected_company."' AND tblhistory.FY = '".$year."' AND tblhistory.TType = 'O' AND tblhistory.TType2 = 'Order' AND tblhistory.TransID IS NOT NULL  GROUP BY tblhistory.ItemID";
		...
		public function SaleAchievmentNew($data)
		...
		{	
			...
			if(!empty($data['Subgroup'])){
			    ...
			    $this->db->select('tblhistory.ItemID,tblitems.SubGrpID1,tblhistory.AccountID,SUM(tblhistory.BilledQty) AS SaleQty,SUM(tblhistory.NetChallanAmt) AS SaleAmount');

			}else{

			    $this->db->select(db_prefix().'items_sub_groups.id as ItemID,tblhistory.AccountID,SUM(tblhistory.BilledQty) AS SaleQty,SUM(tblhistory.NetChallanAmt) AS SaleAmount');
			...
			$this->db->where('MONTH(tblhistory.TransDate)', $data['month_data']);
		...
		//=========================== Add Staff Wise Target ============================

		public function AddStaffWiseSaleTarget($data)
		...
		{	
			...
			// Delete Existing Staff Target

			$this->db->where('tblstaff_target.Staff_AccountID',$data['Staff_AccountID']);

			$this->db->delete('tblstaff_target');
			...
			foreach($data["Target"] as $key=>$val){
				...
				foreach($val as $ItemGrp=>$TargetVal){
					...
					if($TargetVal > 0){
    					...
    					'Staff_AccountID' => $data['Staff_AccountID'],

    					'AccountID'=>$key,

    					'MonthID' => $month,

    					'ItemSubGroupID' => $ItemGrp,
						...
						if($this->db->insert('tblstaff_target',$TargetArray)){
		...
		public function AddItemWiseSaleTarget($data)
		...
		{	
			...
			foreach($data["ItemID"] as $key=>$val){
				...
				$Qty = $data["TargetQty"][$key];

				$Amt = $data["TargetAmt"][$key];
				...
				$this->db->where('tblItemWiseSaleTarget.ItemID',$val);

				$this->db->where('tblItemWiseSaleTarget.FY',$FY);

				$this->db->where('tblItemWiseSaleTarget.MonthID',$month);

				$this->db->delete('tblItemWiseSaleTarget');
				...
				if($this->db->insert('tblItemWiseSaleTarget',$TargetArray)){
...

		{
			...
			if($GodownID !==''){
				...
				WHERE tblhistory.PlantID = '.$selected_company.' AND tblhistory.FY = "'.$fy.'" AND tblhistory.TransDate BETWEEN "'.$first_date.' 00:00:00" AND "'.$day_before.' 23:59:59" AND tblitems.SubGrpID1 IN('.$item_group.') AND tblhistory.BillID IS NOT NULL ';
...

		{
			...
			WHERE tblitems.PlantID = '.$selected_company.' AND tblitems.SubGrpID1 IN('.$item_group.')';
...

		{
			...
			WHERE tblhistory.PlantID = '.$selected_company.' AND tblhistory.FY = "'.$fy.'" AND tblhistory.TransDate BETWEEN "'.$from_date.' 00:00:00" AND "'.$to_date.' 23:59:59" AND tblitems.SubGrpID1 IN('.$item_group.') AND tblhistory.BillID IS NOT NULL ';
...

		{
			...
			if($GodownID !==''){
				...
				$sql = 'SELECT tblitems.PlantID,tblitems.item_code,tblitems.crate_qty,tblitems.description,tblitems.case_qty,tblitems.unit,tblstockmaster.ItemID,tblstockmaster.OQty AS OQty,tblstockmaster.BasicRate AS StockRate,tblrate_master.assigned_rate
				...
				AND tblitems.SubGrpID1 IN('.$item_group.')';
				...
				$sql = 'SELECT  tblitems.PlantID,tblitems.item_code,tblitems.crate_qty,tblitems.description,tblitems.case_qty,tblitems.unit,tblrate_master.assigned_rate,
				...
				(SELECT ROUND(AVG(tblstockmaster.BasicRate),2) AS StockRate FROM tblstockmaster WHERE tblstockmaster.ItemID=tblitems.item_code AND tblstockmaster.PlantID = '.$selected_company.' AND tblstockmaster.FY = "'.$fy.'" AND tblstockmaster.cnfid = "1" GROUP BY tblstockmaster.ItemID,tblstockmaster.PlantID,tblstockmaster.FY) AS StockRate
...

		{
			...
			WHERE tblhistory.PlantID = '.$selected_company.' AND tblhistory.FY = "'.$fy.'" AND tblhistory.TransDate BETWEEN "'.$from_date.'" AND "'.$to_date.'" AND

			tblitems.SubGrpID1 IN('.$item_group.') AND tblhistory.BillID IS NOT NULL';
		...
		{ 
			...
			WHERE tblhistory.PlantID = '.$selected_company.' AND tblhistory.FY = "'.$fy.'" AND tblhistory.TransDate BETWEEN "'.$from_date.' 00:00:00" AND "'.$to_date.' 23:59:59" AND tblitems.subgroup_id IN('.$item_group.') AND tblhistory.BillID IS NOT NULL';
...

		{
			...
			$sql .= ' WHERE tblhistory.PlantID = '.$selected_company.' AND tblhistory.FY = "'.$fy.'" AND tblhistory.TransDate BETWEEN "'.$from_date.' 00:00:00" AND "'.$to_date.' 23:59:59" AND tblitems.subgroup_id IN('.$item_group.') AND tblhistory.BillID IS NOT NULL AND tblhistory.NetChallanAmt !=0.00';
		...
		function getaccounts($postData){
			...
			if(isset($postData['search']) ){
				...
				$this->db->where_in(db_prefix() . 'staff.SubActGroupID', ['10022004','1000190']);
		...
		public function AccountList_table(){
			...
			$this->db->select('tblclients.*,tblStationMaster.StationName,tblcustomers_groups.name as DistType');
			...
			$this->db->where('tblclients.SubActGroupID ', '1000012');

			$this->db->where('tblclients.Blockyn ', 'N');
		...
		public function GetProductionWiseBakingData($filterdata)
		...
		{ 
			...
			$sql = ' SELECT tblproduction.pro_order_id,tblproduction.recipeID,SUM(tblproduction_stage.Qty) AS TotalBakingQty FROM '.db_prefix() . 'production

				INNER JOIN tblproduction_stage ON tblproduction_stage.ProductionID = tblproduction.pro_order_id 

				WHERE '.db_prefix() . 'production.FY = '.$fy.'  AND tblproduction.production_status = "Completed" 

				AND tblproduction.PlantID = '.$selected_company.' AND tblproduction.TransDate BETWEEN "'.$from_date.' 00:00:00" AND "'.$to_date.' 23:59:59" 

				AND tblproduction_stage.Stage = "Baking"';

				if($report_type == 1 && empty($ItemID) && empty($accountID)){

				    $sql .= 'GROUP BY tblproduction.pro_order_id ';

				}else if($report_type == 2 && empty($ItemID) && empty($accountID)){

				    $sql .= 'GROUP BY tblproduction.recipeID ';
				...
				$sql .= 'ORDER BY tblproduction.pro_order_id';
		...
		{ 
			...
			if($report_type == 1 && empty($ItemID) && empty($accountID)){
				...
				$sql = ' SELECT '.db_prefix() . 'production.*,'.db_prefix() . 'clients.*,tblitems.description,tblstaff.firstname, tblrecipe.qty FROM '.db_prefix() . 'production 
				...
				LEFT JOIN tblrecipe ON tblrecipe.item_code=tblproduction.recipeID AND tblrecipe.status="Y" AND tblrecipe.PlantID = tblproduction.PlantID 
				...
				WHERE tblproduction.FY = '.$fy.' AND tblproduction.PlantID = '.$selected_company.' AND tblproduction.production_status = "Completed" AND tblproduction.TransDate BETWEEN "'.$from_date.' 00:00:00" AND "'.$to_date.' 23:59:59" 
				...
				if($source == "con"){
					...
					$sql = ' SELECT '.db_prefix() . 'production.*,'.db_prefix() . 'clients.*,tblitems.description, tblrecipe.qty FROM '.db_prefix() . 'production 

					INNER JOIN tblclients ON tblclients.AccountID=tblproduction.contractor_name AND tblclients.PlantID = tblproduction.PlantID 

					LEFT JOIN tblitems ON tblitems.item_code=tblproduction.recipeID AND tblitems.PlantID = tblproduction.PlantID 
					...
					$sql = ' SELECT '.db_prefix() . 'production.*,'.db_prefix() . 'staff.*,tblitems.description, tblrecipe.qty FROM '.db_prefix() . 'production 
		...
		{  
			...
			$this->db->select(db_prefix() .'rate_master.item_id,'.db_prefix() .'rate_master.assigned_rate,'.db_prefix() .'rate_master.dis_per,'.db_prefix() .'rate_master.effective_date,'.db_prefix() .'rate_master.TransDate,

			tblstaff.firstname,tblstaff.lastname,CONCAT(tblstaff2.firstname, " ", tblstaff2.lastname) AS UpdatedBy,'.db_prefix() . 'items.description,

			'.db_prefix() . 'items.case_qty,'.db_prefix() . 'items.mrp,'.db_prefix() . 'items.crate_qty ,'.db_prefix() .'taxes.taxrate,tblrate_master.distributor_id,tblrate_master.state_id');
			...
			$this->db->join(db_prefix() .'staff', db_prefix() .'staff.AccountID = '.db_prefix() .'rate_master.UserId',"LEFT");

			$this->db->join(db_prefix() .'staff as tblstaff2', db_prefix() .'staff2.AccountID = '.db_prefix() .'rate_master.UserID2','LEFT');
			...
			if($data['item_group'] !=''){
				...
				$this->db->where_in(db_prefix() .'items.SubGrpID1', $item_group);

			}

			

			$this->db->group_by(db_prefix() .'items.item_code,tblrate_master.TransDate,tblrate_master.assigned_rate');

			$this->db->order_by('tblrate_master.TransDate DESC, tblrate_master.id DESC');

			// $this->db->order_by(db_prefix() .'items.SubGrpID1', 'ASC');
		...
		public function sum_get_achievement_division($data){
			...
			$this->db->where(db_prefix() . 'history.TransDate >=', $start_date.' 00:00:00');

			$this->db->where(db_prefix() . 'history.TransDate <=',$end_date.' 23:59:59');
		...
		public function market_outstanding_data($filterdata){
			...
			$sql .= 'SELECT tblclients.StationName,tblclients.CtrlAccountID,tblclients.AccountID,tblclients.company,tblclients.state
			...
			$sql .= ' WHERE tblclients.PlantID = '.$selected_company. ' AND tblclients.SubActGroupID = "1000012" ';
		...
		public function market_outstanding_opn_bal_data($filterdata)

		{
			...
			$sql .= 'SELECT tblaccountbalances.BAL1 AS opn_Amt,AccountID

			FROM tblaccountbalances 

			WHERE PlantID = '.$selected_company.' AND FY = "'.$fy.'"   

			GROUP BY tblaccountbalances.AccountID';
		...
		public function market_outstanding_last_billDate($filterdata){
			...
			$sql .= 'SELECT max(TransDate) AS TransDate,AccountID FROM tblhistory 

			WHERE PlantID = '.$selected_company.' AND FY = "'.$fy.'" AND TType = "O" AND TType2 = "Order" AND tblhistory.TransDate BETWEEN "'.$from_date.' 00:00:00" AND "'.$to_date.' 23:59:59"  
		...
		public function market_outstanding_currDaySale($filterdata){
			...
			WHERE PlantID = '.$selected_company.' AND FY = "'.$fy.'" AND TType = "O" AND TType2 = "Order" AND tblhistory.TransDate BETWEEN "'.$to_date.' 00:00:00" AND "'.$to_date.' 23:59:59"  
...

		{
			...
			// $this->db->where_in(db_prefix() . 'clients.SubActGroupID', ['50003002','60001004','1000012']);

			$this->db->where_in(db_prefix() . 'clients.SubActGroupID1', ['100056']);
		...
		public function getCratesRcvdVehicle($from_date,$to_date,$driver=""){
			...
			if(!empty($driver)){
				...
				$con .= " AND tblchallanmaster.DriverID = '".$driver."'";

			}

			

			$sql = 'SELECT tblvehiclereturn.*,tblroute.name,tblchallanmaster.Crates AS ChlCrates,tblchallanmaster.VehicleID,tblstaff.firstname,tblstaff.lastname FROM tblvehiclereturn 
			...
			WHERE tblvehiclereturn.FY = '.$fy.'  AND tblvehiclereturn.PlantID = '.$selected_company.' AND  tblvehiclereturn.Transdate BETWEEN "'.$from_date.' 00:00:00" AND "'.$to_date.' 23:59:59" '.$con.' 
			...
			foreach ($result_all as $key => $value) {
				...
				WHERE tblaccountcrates.FY = '.$fy.'  AND tblaccountcrates.PlantID = '.$selected_company.' AND tblaccountcrates.VoucherID = "'.$value['ReturnID'].'" AND tblaccountcrates.TType = "C" 
		...
		public function get_Crates_for_body_data($filterdata){
			...
			if($accountId != ''){
				...
				/*$sql4 .= 'SELECT max(TransDate) AS lastBill,AccountID FROM tblhistory 

                    WHERE PlantID = '.$selected_company.' AND FY = "'.$fy.'" AND TType = "O" AND TType2 = "Order" AND tblhistory.TransDate BETWEEN "'.$Billing_start_Date.' 00:00:00" AND "'.$Billing_end_Date.' 23:59:59"  
		...
		public function GetOpenCrates($filterdata)

		{
			...
			$sql = 'SELECT tblaccountcrates.TType,SUM(tblaccountcrates.Qty) AS OpnQty FROM tblaccountcrates 

			WHERE tblaccountcrates.FY = '.$fy.'  AND tblaccountcrates.PlantID = '.$selected_company.' AND 

			tblaccountcrates.PassedFrom = "OPENCRATES" AND tblaccountcrates.AccountID = "'.$accountId.'" 

			GROUP BY tblaccountcrates.TType';
		...
		public function GetOpeningTransactionCrates($filterdata)

		{
			...
			$FirstDateOfFY = "20".$fy."-04-01";

			$OpnTransQty = 0;

			if($FirstDateOfFY == $from_date){

				

				}else{

				$FromDate = $FirstDateOfFY;

				$ToDate = date('Y-m-d', strtotime('-1 day', strtotime($from_date)));

				$sql = 'SELECT TType,SUM(Qty) AS OpnQty FROM tblaccountcrates 

				WHERE tblaccountcrates.FY = '.$fy.'  AND tblaccountcrates.PlantID = '.$selected_company.' AND 

				tblaccountcrates.PassedFrom != "OPENCRATES" AND  tblaccountcrates.Transdate BETWEEN "'.$FromDate.' 00:00:00" AND "'.$ToDate.' 23:59:59" 

				AND tblaccountcrates.AccountID = "'.$accountId.'" GROUP BY TType';

				$OpenTransactionCrate = $this->db->query($sql)->result_array();
				...
				foreach($OpenTransactionCrate as $val){
		...
		public function GetTransactionCrates($filterdata)

		{
			...
			$sql1 = 'SELECT tblaccountcrates.*, tblchallanmaster.VehicleID,CONCAT(tblstaff.firstname, " ", tblstaff.lastname) AS DriverName,

			tblclients.company FROM tblaccountcrates

			LEFT JOIN tblchallanmaster ON tblchallanmaster.ChallanID = tblaccountcrates.ChallanID

			LEFT JOIN tblclients ON tblclients.AccountID = tblaccountcrates.AccountID AND tblclients.PlantID = tblaccountcrates.PlantID

			LEFT JOIN tblstaff ON tblstaff.AccountID = tblchallanmaster.DriverID

			WHERE tblaccountcrates.FY = '.$fy.'  AND tblaccountcrates.PlantID = '.$selected_company.' AND tblaccountcrates.PassedFrom != "OPENCRATES"

			AND  tblaccountcrates.Transdate BETWEEN "'.$from_date.' 00:00:00" AND "'.$to_date.' 23:59:59" ';

			if($driver){

				$sql1 .= " AND tblchallanmaster.DriverID = '".$driver."'";

			}

			if($accountId){

				$sql1 .= " AND tblaccountcrates.AccountID = '".$accountId."'";

			}

			$sql1 .= ' ORDER BY tblaccountcrates.Transdate ASC';

			$CrateTransData = $this->db->query($sql1)->result_array();

			return $CrateTransData;

		}

		public function GetCrateLedger_New($filterdata)

		{
			...
			if($accountId != ''){
				...
				WHEN TType = "C" THEN -Qty 

				WHEN TType = "D" THEN Qty 

				ELSE 0 

				END) AS OQty FROM tblaccountcrates 
				...
				if($from_date == $FirstDate){
					...
					$sql1 = 'SELECT tblaccountcrates.*, tblchallanmaster.VehicleID,CONCAT(tblstaff.firstname, " ", tblstaff.lastname) AS DriverName,tblclients.company FROM tblaccountcrates
					...
					AND tblaccountcrates.AccountID = "'.$accountId.'" '.$con.' ORDER BY tblaccountcrates.Transdate ASC';
					...
					$sql = 'SELECT tblaccountcrates.*, tblchallanmaster.VehicleID,CONCAT(tblstaff.firstname, " ", tblstaff.lastname) AS DriverName,tblclients.company FROM tblaccountcrates 
		...
		public function GetAllCrateLedger($filterdata)

		{
            ...
            if($from_date == $FirstDate){
                ...
                $ToDate = to_sql_date($filterdata["from_date"]);
			...
			$sql = 'SELECT AccountID,CustomerName,CrateLimit,SUM(Closing) Opening,SUM(Debit)Debit,SUM(Credit)Credit,

			SUM(Closing)+SUM(Debit)-SUM(Credit) Balance from

			(SELECT AccountID,CustomerName,CrateLimit,0 Opening,0 Debit,0 Credit,SUM(Opening)+SUM(Debit)-SUM(Credit) Closing from (SELECT AccountID,CustomerName,CrateLimit,TType,PassedFrom, case when TType="C" and PassedFrom="OPENCRATES" THEN -Quantity ELSE case when TType="D" and PassedFrom="OPENCRATES" THEN Quantity ELSE 0 end end Opening, case when PassedFrom="CHALLAN" THEN Quantity ELSE 0 end Debit, case when PassedFrom="VEHRTNCRATES" THEN Quantity ELSE 0 end Credit from (SELECT a.AccountID,a.company CustomerName,a.crate_limit CrateLimit,b.TType,b.PassedFrom, SUM(Qty) Quantity from tblclients as a left join tblaccountcrates as b on a.PlantID=b.PlantID and a.AccountID=b.AccountID WHERE a.PlantID="'.$selected_company.'" and b.FY="'.$fy.'" AND cast(b.TransDate as date) BETWEEN "'.$FirstDate.'" AND "'.$ToDate.'" GROUP BY a.AccountID,a.company,b.TType,b.PassedFrom)x GROUP BY AccountID,CustomerName,TType,PassedFrom)Y GROUP BY AccountID,CustomerName

			UNION

			SELECT AccountID,CustomerName,CrateLimit,0 Opening,SUM(Debit)Debit,SUM(Credit)Credit,0 Closing from (SELECT AccountID,CustomerName,CrateLimit,TType,PassedFrom, case when PassedFrom="CHALLAN" THEN Quantity ELSE 0 end Debit, case when PassedFrom="VEHRTNCRATES" THEN Quantity ELSE 0 end Credit from (SELECT a.AccountID,a.company CustomerName,a.crate_limit CrateLimit,b.TType,b.PassedFrom, SUM(Qty) Quantity from tblclients as a left join tblaccountcrates as b on a.PlantID=b.PlantID and a.AccountID=b.AccountID WHERE a.PlantID="'.$selected_company.'" and b.FY="'.$fy.'" AND cast(b.TransDate as date) BETWEEN "'.$from_date.'" AND "'.$to_date.'" AND PassedFrom in ("CHALLAN","VEHRTNCRATES") GROUP BY a.AccountID,a.company,b.TType,b.PassedFrom)x GROUP BY AccountID,CustomerName,TType,PassedFrom)Y GROUP BY AccountID,CustomerName)z

			GROUP BY AccountID,CustomerName
		...
		public function get_ItemSubGroup2_data(){
			...
			$this->db->select(db_prefix() . 'items_sub_group2.*,'.db_prefix() . 'items_sub_groups.name AS SubGroup1Name,'.db_prefix() . 'items_main_groups.name AS MainGroupName');

			$this->db->from(db_prefix() . 'items_sub_group2');

			$this->db->join(db_prefix() . 'items_sub_groups', '' . db_prefix() . 'items_sub_groups.id = ' . db_prefix() . 'items_sub_group2.sub_group_id1');
			...
			$this->db->order_by(db_prefix() . 'items_sub_group2.id', 'ASC');
		...
		public function get_distwise_rate_table_data($data)
		...
		{  
			...
			$this->db->select('tblrate_master.item_id,tblrate_master.assigned_rate,tblrate_master.effective_date,

			tblrate_master.TransDate,tblrate_master.distributor_id,

			tblitems.description,tblitems.case_qty,tblitems.mrp,tblitems.crate_qty ,tbltaxes.taxrate');

			$this->db->from('tblrate_master');

			$this->db->join('tblitems', 'tblitems.item_code = tblrate_master.item_id AND tblitems.PlantID = tblrate_master.PlantID');
			...
			if($data['distributor_id'] !=''){
				...
				$this->db->where_in(db_prefix() .'rate_master.distributor_id', $data['distributor_id']);
		...
		public function GetItemsBySubgroup($item_group)
		...
		public function GetDistTypeList($DistType)
		...
		{  
			...
			if($DistType){
			    ...
			    $this->db->where_in(db_prefix() .'customers_groups.id', $DistType);

			}

			$this->db->order_by(db_prefix() .'customers_groups.name', 'ASC');
