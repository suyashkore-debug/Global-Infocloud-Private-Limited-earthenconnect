<?php
	...
	class SchemeMaster_model extends App_Model
	{
		...
		public function GetItemDetailByItemID($ItemID,$State,$DistType,$FromDate,$ToDate,$SchemeID)
		{
			$selected_company = $this->session->userdata('root_company');
			$FY = $this->session->userdata('finacial_year');
			$FromDate = to_sql_date($FromDate)." 00:00:00";
			$ToDate = to_sql_date($ToDate)." 23:59:59";
			// echo $ToDate;die;
			...
			$this->db->where_in(db_prefix() . 'schemedetails.DistributorType', $DistType);
			// $this->db->where(db_prefix() . 'schemedetails.StartDate >=',$FromDate);
			
			// $this->db->where(db_prefix() . 'schemedetails.EndDate <=',$ToDate);
			
			$this->db->where('"' . $ToDate . '" BETWEEN ' . db_prefix() . 'schemedetails.StartDate AND ' . db_prefix() . 'schemedetails.EndDate', null, false);
			$this->db->where(db_prefix() . 'schemedetails.SchemeID !=',$SchemeID);
			...
			if($data){
                ...
                $this->db->join(db_prefix() . 'rate_master', '' . db_prefix() . 'rate_master.item_id = ' . db_prefix() . 'items.item_code AND '. db_prefix() . 'rate_master.PlantID = ' . db_prefix() . 'items.PlantID AND '. db_prefix() . 'rate_master.state_id ="'.$State.'" AND FIND_IN_SET(' . db_prefix() . 'rate_master.distributor_id, "' . $distTypeStr . '")','LEFT');
		...
		public function GetSchemeDetailByID($id)
		{
			...
			$result = $this->db->get()->row();
			if($result){
				$this->db->select(db_prefix() . 'schemedetails.*');
				$this->db->where(db_prefix() . 'schemedetails.PlantID', $selected_company);
				$this->db->where(db_prefix() . 'schemedetails.FY', $FY);
				$this->db->from(db_prefix() . 'schemedetails');
		...
		public function ItemPartyWiseSaleExport($allItem,$FromDate,$ToDate,$states,$client_type,$Item_UnitDisc)
		{
			...
			$this->db->where(db_prefix() . 'history.TransDate2 <=',$ToDate);
			$this->db->where(db_prefix() . 'clients.state', $states);
			$this->db->where_in(db_prefix() . 'clients.DistributorType', $client_type);
			$this->db->from(db_prefix() . 'history');
			$this->db->where_in(db_prefix() . 'history.ItemID', $allItem);
			$this->db->group_by(db_prefix() . 'history.AccountID,'.db_prefix() . 'history.ItemID');
			$result = $this->db->get()->result_array();
