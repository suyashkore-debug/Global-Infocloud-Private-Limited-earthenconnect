<?php
	...
	class Staff_model extends App_Model
	{
		...
		*/
		...
		{
			$select_str = 'tblstaff.*,tblhr_job_position.*,CONCAT(tblstaff.firstname,\' \',tblstaff.lastname) as full_name, firstname,lastname,'.db_prefix() . 'staff_departments.departmentid';
			$selected_company = $this->session->userdata('root_company');
			...
			if (is_staff_logged_in() && $id != '' && $id == get_staff_user_id()) {
				$select_str .= ',(SELECT COUNT(*) FROM ' . db_prefix() . 'notifications WHERE touserid=' . get_staff_user_id() . ' and isread=0) as total_unread_notifications, (SELECT COUNT(*) FROM ' . db_prefix() . 'todos WHERE finished=0 AND tblstaff.staffid=' . get_staff_user_id() . ') as total_unfinished_todos';
			}
		...
		public function getAssignStaff($id = '', $where = [])
		{
			$selected_company = $this->session->userdata('root_company');
			$select_str = '*,CONCAT(firstname,\' \',lastname) as full_name, firstname,lastname';
			
			$this->db->select($select_str);
			$this->db->where($where);
			/*if($selected_company == "3"){
				$this->db->where('SubActGroupID', "30001002");
			}*/
		...
		*/
		...
		public function GetStaffByDepartment($departmentid = "")
		{
			...
			$selected_company = $this->session->userdata('root_company');
			$select_str = 'tblstaff.*,CONCAT(firstname,\' \',lastname) as full_name,'.db_prefix() . 'staff_departments.departmentid';
			$this->db->select($select_str);
			
			$this->db->join(db_prefix() . 'staff_departments', db_prefix() . 'staff_departments.staffid = '.db_prefix() . 'staff.staffid ','LEFT');
			if(!empty($departmentid)){
				$this->db->where('tblstaff_departments.departmentid', $departmentid);
			}
			$this->db->where('tblstaff.active', 1);
			$this->db->where('tblstaff.PlantID', $selected_company);
			$this->db->order_by('tblstaff.firstname', 'ASC');
