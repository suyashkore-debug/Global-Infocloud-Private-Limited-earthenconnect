<?php
...

	{
		...
		public function GetCalenderMonthlySaleReturnDataNew($filter = "")

		{
			...
			$DayWiseSale  = $this->GetDayWiseSaleReturnReportNew($filter);
...

		{
			...
			$filter["TType2"] = array('Damage',"Fresh");
...

		{
			...
			// echo "<pre>";print_r($MonthWiseSale);die;
		...
		public function GetDayWiseSaleReturnReportNew($filterdata = "")

		{
			...
			$this->db->where_in('tblhistory.TType2', array('Fresh','Damage'));
...

		{
			...
			$this->db->select('tblhistory.FY,tblhistory.TType2,Year(tblhistory.TransDate) as year,MONTHNAME(tblhistory.TransDate) as month,MONTH(tblhistory.TransDate) as MonthNumber,ROUND(COALESCE(SUM(tblhistory.NetChallanAmt),0),2) as AmtSum');
			...
			$this->db->where_in('tblhistory.TType2', $filterdata["TType2"]);
			...
			$this->db->order_by("YEAR(tblhistory.TransDate)", "ASC");

			$this->db->order_by("MONTH(tblhistory.TransDate)", "ASC");
...

		{
			...
			$SubGroup1 = $filterdata["SubGroup"];

			$SubGroup2 = $filterdata["SubGroup2"];

			$ItemID = $filterdata["Items"];
			...
			if(!empty($ItemID)){
				...
				$this->db->where_in('tblhistory.ItemID', $ItemID);
...
//==================== Total Order Count =======================================
...

		{
			...
			$this->db->select('COUNT(tblordermaster.OrderID) as Total,tblordermaster.OrderStatus');
			...
			$this->db->group_by('tblordermaster.OrderStatus');

			$Transaction = $this->db->get('tblordermaster')->result_array();
			...
			foreach($Transaction as $val){
			    ...
			    if($val["OrderStatus"] == "O"){

			        $OpenOrder = $val["Total"];

			    }elseif($val["OrderStatus"] == "C"){

			        $CancelOrder = $val["Total"];

			    }

			}

			$response->AllOrder = $OpenOrder + $CancelOrder;

			$response->CancelOrder = $CancelOrder;
...
//======================== Total Pending Order Count ===========================
    ...
    public function TotalPendingOrderCount($filterdata)

	{
		...
		$this->db->select('COUNT(tblordermaster.OrderID) as PendingOrderCount');
		...
		return $Transaction->PendingOrderCount;

	}

//=========================== Total Invoice Order Count ========================
...
//======================= New Party Registered =================================
	...
	public function NewParties($filterdata = "")
...
//============================ Top Order Item ==================================
    ...
    public function TopOrderdItem($filterdata = "")
...
//================== Total GST Collectiom Amt ==================================

    public function TotalGSTAmt($filterdata)

	{
		...
		$this->db->select('ROUND(COALESCE(SUM(tblhistory.cgstamt + tblhistory.sgstamt + tblhistory.igstamt),0),0) as TotalGSTAmt');
		...
		return $Transaction->TotalGSTAmt;
...

		{
			...
			foreach ($Report as $r) {
				...
				if($r['rate'] > 0){
					...
					'name' => $r['ItemName'] . " (Return : " . round($r['rate'],2) . "%)", 
