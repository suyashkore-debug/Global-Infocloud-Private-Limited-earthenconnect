...
function initDataTable(e, t, a, i, n, s) {
    ...
    function newexportaction(e, dt, button, config) {
     ...
     $('input[name=export_option][value=all]').attr('checked', 'checked');
     $(".custom_button").attr("style", "display:none");
     
     $('input:radio[name=export_option]').change(function () {
            if ($("input[name='export_option']:checked").val() == 'all') {
                //alert("all export...");
                 $(".custom_button").attr("style", "display:none")
                 $(".dt-buttons").removeAttr("style")
            }
            if ($("input[name='export_option']:checked").val() == 'itemwisesale') {
                //alert("custom export");
                $(".dt-buttons").attr("style", "display:none")
                $(".custom_button").removeAttr("style")
                //$('#custom_button').removeCss('display');
            }
        });
...
function init_order(e) {
    load_small_table_item(e, "#invoice", "invoiceid", "order/get_order_data_ajax", ".table-order")
}
...
function crate_invoice(e) {
    void 0 !== e && "" !== e && $("#invoice").load(admin_url + "order/crate_invoice_by_ajax/" + e)
}
...
function add_item_to_preview1(e,l,t,s,disc) {
    requestGetJSON("invoice_items/get_item_by_id/" + e + "/" + t + "/" + s).done(function(e) {
		// Get dis Per val
		// alert(e);
		// alert(l);
		// alert(t);
    ...
    // console.log("Dist Type ID:", t);
    // console.log("Dist State ID:", s);
    // console.log("Client ID:", disc);
		// alert(disc);
		// dis = $('.main input[name="dis"]').val(),
		// dis = disc,
		dis = e.dis_per,
        clear_item_preview_values1(),  $('.main input[name="item_code"]').val(e.item_code), $('.main input[name="hsn_code"]').val(e.hsn_code),$('.main input[name="autouser"]').val(e.description), $('.main input[name="dis"]').val(dis), $('.main input[name="dis_amt"]').val(0), $('.main input[name="taxrate1"]').val(e.taxrate), $('.main textarea[name="description"]').val(e.description), $('.main input[name="quantity"]').val("");
        var t = [];
        e.taxname && e.taxrate && t.push(e.taxname + "|" + e.taxrate), e.taxname_2 && e.taxrate_2 && t.push(e.taxname_2 + "|" + e.taxrate_2), $(".main select.tax").selectpicker("val", t), $('.main input[name="unit"]').val(e.unit);
        var a = $("body").find('.accounting-template select[name="currency"]'),
            i = a.attr("data-base"),
       ...
       // str.indexOf("welcome")
        if(e.taxrate == '0.00' || e.taxrate == null || e.taxrate == ''){
            $('input[name="tax1"]').val("NonTaxItems");
            $('input[name="taxes1"]').val("NonTaxItems");
            $('#item_added_msg').text("We can add only Non Taxable items..");
        }else {
            $('input[name="tax1"]').val('TaxItems');
            $('input[name="taxes1"]').val("TaxItems");
            $('#item_added_msg').text("We can add only Taxable items..");
        }
        
        var aa = $('.main input[name="items_case_qty"]'),
            bb = '',
            cc = $('.main input[name="pack_qty"]');
        if (s !== 'UP') { 
            aa.val(e.outst_supply_in); bb = e.outst_supply_in; }
        if (s == 'UP') { aa.val(e.local_supply_in); bb = e.local_supply_in; }
        ...
        { 
            cc.val(e.case_qty);
            var PactQty = e.case_qty;
        }
        else {
            cc.val(e.crate_qty);
            var PactQty = e.case_qty;
        }
        var stock = parseFloat(e.OQty) + parseFloat(e.PQty) - parseFloat(e.PRQty) - parseFloat(e.IQty) + parseFloat(e.PRDQty)  - parseFloat(e.SQty) + parseFloat(e.SRQty) - parseFloat(e.ADJQTY); 
        var stockInCase = stock / PactQty
        $('.main input[name="stockqty"]').val(parseFloat(stockInCase).toFixed(2));
        $('.main input[name="stockqty1"]').val(parseFloat(stockInCase).toFixed(2));
        $(document).trigger({
            type: "item-added-to-preview",
            item: e,
            item_type: "item"
...
function clear_item_preview_values1(e) {
    var t = $("table.items tbody").find("tr:last-child").find("select").selectpicker("val"),
        a = $(".main");
    a.find("textarea").val(""), a.find('td.custom_field input[type="checkbox"]').prop("checked", !1), a.find("td.custom_field input:not(:checkbox):not(:hidden)").val(""), a.find("td.custom_field select").selectpicker("val", ""), a.find('input[name="quantity"]').val(" "), a.find("select.taxname").selectpicker("val", ""), a.find('input[name="rate"]').val(""), a.find('input[name="items_case_qty"]').val(""), a.find('input[name="pack_qty"]').val(""), a.find('input[name="stockqty"]').val(""), a.find('input[name="stockqty1"]').val(""), a.find('input[name="item_code"]').val(""), a.find('input[name="dis"]').val(""), a.find('input[name="dis_amt"]').val(""),  a.find('input[name="taxrate1"]').val(""), $('input[name="task_id"]').val(""), $('input[name="expense_id"]').val("")
}
...
function add_item_to_table1(n, s, e, t) {
    if ("" !== (n = void 0 === n || "undefined" == n ? get_item_preview_values1() : n).description || "" !== n.long_description || "" !== n.rate) {
        if ("0" !== n.pack_qty) {
            var numbers = /^[0-9]+$/;
        if ("" !== n.rate && "0.00" !== n.rate && "0.000" !== n.rate) {
            if(n.qty.match(numbers) && "0" !== n.qty){
            var dis_per = n.dis;
            var dis_amt = 0;
            var cscr_qty = 0;
        var p = "",
            _ = $("body").find("tbody .item").length + 1;
        p += '<tr class="sortable item" data-merge-invoice="' + e + '" data-bill-expense="' + t + '">', p += '<td class="dragger">', isNaN(n.qty) && (n.qty = 1), ("" === n.rate || isNaN(n.rate)) && (n.rate = 0);
        var o = n.rate * n.qty,// * n.pack_qty Add This If Needed
        
			
        cscr_qty = (n.qty / n.pack_qty);
        dis_amt = (o * dis_per / 100);
            a = "newitems[" + _ + "][taxname][]";
        $("body").append('<div class="dt-loader"></div>');
        var l = /<br[^>]*>/gi;
        return get_taxes_dropdown_template(a, n.taxname).done(function(e) {
            p += '<input type="hidden" class="order" name="newitems[' + _ + '][order]">', p += '<td class="item_code_td"><input type="text" placeholder="' + app.lang.item_code + '" name="newitems[' + _ + '][item_code]" class="form-control" value="' + n.item_code + '" disabled><input type="hidden" name="newitems[' + _ + '][item_code1]" value="' + n.item_code + '"><input type="hidden" name="newitems[' + _ + '][hsn_code]" value="' + n.hsn_code + '" >', p += "</td>", p += '<td><input type="text" placeholder="' + app.lang.description + '" name="newitems[' + _ + '][description]" class="form-control" value="' + n.description + '">', p += "</td>", p += '<td class="cs_cr"><input type="text" placeholder="' + app.lang.items_case_qty + '" name="newitems[' + _ + '][items_cs_cr1]" class="form-control" value="' + n.items_case_qty + '" disabled> <input type="hidden" name="newitems[' + _ + '][items_cs_cr]" value="' + n.items_case_qty + '">', p += "</td>", p += '<td class="pack_qty"><input type="text" placeholder="' + app.lang.pack_qty + '" name="newitems[' + _ + '][pack_qty1]" class="form-control" value="' + n.pack_qty + '" disabled> <input type="hidden" name="newitems[' + _ + '][pack_qty]" value="' + n.pack_qty + '">', p += "</td>";
            var t = $("tr.main td.custom_field"),
                c = !1;
            0 < t.length && $.each(t, function() {
                var a = $(this).clone(),
                ...
                if (t.is(":checkbox")) {
                    ...
                    d.append(r), $.each(o, function(e, t) {
                        d.find('select option[value="' + t + '"]').attr("selected", !0)
                        d.find('select option[value="' + t + '"]').attr("disabled", 'disabled')
                    }), e = d.html()
                }
                p += '<td class="custom_field">' + e + "</td>"
            }), p += '<td class="order_qty"><input type="text" min="0" onblur="calculate_total();" onchange="calculate_total();" data-quantity name="newitems[' + _ + '][qty]" value="' + n.qty + '" class="form-control">', p += "</td>", p += '<td class="qty_cs_cr"><input type="number" name="newitems[' + _ + '][qty_cs_cr]" value="' + cscr_qty.toFixed(1) + '" class="form-control" disabled></td>', p += '<td class="rate"><input type="number" name="newitems[' + _ + '][rate1]" value="' + n.rate + '" class="form-control" onblur="calculate_total();" onchange="calculate_total();"><input type="hidden" name="newitems[' + _ + '][rate]" value="' + n.rate + '"></td>', p += '<td class="dis"><input type="number" name="newitems[' + _ + '][dis]" value="' + n.dis + '" class="form-control" onblur="calculate_total();" onchange="calculate_total();"></td>', p += '<td class="dis_amt"><input type="number" name="newitems[' + _ + '][dis_amt]" value="' + dis_amt + '" class="form-control" disabled><input type="hidden" name="newitems[' + _ + '][dis_amt1]" value="' + dis_amt + '" class="form-control"></td>', p += '<td class="taxrate"><input type="hidden" name="newitems[' + _ + '][taxrate1]" value="' + n.taxrate1 + '" class="form-control">' + e + "</td>", p += '<td class="amount" align="right">'  + format_money(o, !0) + "</td>", p += '<td><a href="#" class="btn btn-danger pull-left" onclick="delete_item(this,' + s + '); return false;"><i class="fa fa-trash"></i></a></td>', p += "</tr>", $("table.items tbody").append(p), $(document).trigger({
                type: "item-added-to-table",
                data: n,
                row: p
            }), setTimeout(function() {
            ...
            return "" !== a && void 0 !== a && (billed_tasks = a.split(","), $.each(billed_tasks, function(e, t) {
                ...
                $("#billed-expenses").append(hidden_input("billed_expenses[" + _ + "][]", t))
            })), $("#item_select").hasClass("ajax-search") && "" !== $("#item_select").selectpicker("val") && $("#item_select").prepend("<option></option>"), init_selectpicker(), init_datepicker(), init_color_pickers(),dis = $('.main input[name="dis"]').val(), clear_item_preview_values1(), reorder_items(), $("body").find("#items-warning").remove(), $("body").find(".dt-loader").remove(), $("#item_select").selectpicker("val", ""), $("#taxname").selectpicker("val", ""), $("#autouser").val(""), $("#item_code").focus(), c && $(".invoice-form").length ? validate_invoice_form() : c && $(".estimate-form").length ? validate_estimate_form() : c && $(".proposal-form").length ? validate_proposal_form() : c && $(".credit-note-form").length && validate_credit_note_form(), !0
        }), !1
            }else {
        ...
        alert ("Please assign Package Quntity...it should be not equal to 0");
    }
    
}
}

function add_item_to_table2(n, s, e, t) {
    if ("" !== (n = void 0 === n || "undefined" == n ? get_item_preview_values1() : n).description || "" !== n.long_description || "" !== n.rate) {
        ...
        if ("" !== n.rate && "0.00" !== n.rate && "0.000" !== n.rate) {
            ...
            if(parseFloat(n.stockqty) >= parseFloat(n.qty) || PlantID == "1"){
            
        var p = "",
            _ = $("body").find("tbody .item").length + 1;
        p += '<tr class="sortable item" data-merge-invoice="' + e + '" data-bill-expense="' + t + '">', p += '<td class="dragger">', isNaN(n.qty) && (n.qty = 1), ("" === n.rate || isNaN(n.rate)) && (n.rate = 0);
        var o = n.rate * n.qty * n.pack_qty,
        ...
        return get_taxes_dropdown_template(a, n.taxname).done(function(e) {
            ...
            0 < t.length && $.each(t, function() {
                ...
                p += '<td class="custom_field">' + e + "</td>"
            }), p += '<td><input type="text" onblur="calculate_total();" onchange="calculate_total2();" name="newitems[' + _ + '][oldqty]" value="" class="form-control" disabled>', p += "</td>", p += '<td class="order_qty"><input type="text" min="0" onblur="calculate_total();" onchange="calculate_total2();" data-quantity name="newitems[' + _ + '][qty]" value="' + n.qty + '" class="form-control">', p += "</td>", p += "</td>" , p += '<td class="stock_qty"><input type="text" name="newitems[' + _ + '][stockqty]" value="' + n.stockqty + '" class="form-control" disabled><input type="hidden" name="newitems[' + _ + '][stockqty1]" value="' + n.stockqty + '" class="form-control">', p += "</td>", p += '<td><input type="text" name="newitems[' + _ + '][ereason]" value="" class="form-control" disabled>', p += "</td>", p += '<td class="rate"><input type="number" name="newitems[' + _ + '][rate1]" value="' + n.rate + '" class="form-control" disabled><input type="hidden" name="newitems[' + _ + '][rate]" value="' + n.rate + '" class="form-control"></td>', p += '<td class="dis"><input type="number" name="newitems[' + _ + '][dis]" value="' + n.dis + '" class="form-control" readonly></td>', p += '<td class="dis_amt"><input type="number" name="newitems[' + _ + '][dis_amt]" value="' + n.dis_amt + '" class="form-control" readonly><input type="hidden" name="newitems[' + _ + '][dis_amt1]" value="' + n.dis_amt + '" class="form-control"></td>', p += '<td class="taxrate"><input type="hidden" name="newitems[' + _ + '][taxrate1]" value="' + n.taxrate1 + '" class="form-control">' + e + "</td>", p += '<td class="amount" align="right">'  + format_money(o, !0) + "</td>", p += '<td><a href="#" class="btn btn-danger pull-left" onclick="delete_item(this,' + s + '); return false;"><i class="fa fa-trash"></i></a></td>', p += "</tr>", $("table.items tbody").append(p), $(document).trigger({
                type: "item-added-to-table",
                data: n,
                row: p
            }), setTimeout(function() {
            ...
            return "" !== a && void 0 !== a && (billed_tasks = a.split(","), $.each(billed_tasks, function(e, t) {
                ...
                $("#billed-expenses").append(hidden_input("billed_expenses[" + _ + "][]", t))
            })), $("#item_select").hasClass("ajax-search") && "" !== $("#item_select").selectpicker("val") && $("#item_select").prepend("<option></option>"), init_selectpicker(), init_datepicker(), init_color_pickers(), clear_item_preview_values1(), reorder_items(), $("body").find("#items-warning").remove(), $("body").find(".dt-loader").remove(), $("#item_select").selectpicker("val", ""), $("#taxname").selectpicker("val", ""), $("#autouser").val(""), $("#autouser").focus(), c && $(".invoice-form").length ? validate_invoice_form() : c && $(".estimate-form").length ? validate_estimate_form() : c && $(".proposal-form").length ? validate_proposal_form() : c && $(".credit-note-form").length && validate_credit_note_form(), !0
        }), !1
            }else{
...
function get_item_preview_values1() {
    var e = {};
    return e.item_code = $('.main input[name="item_code"]').val(), e.hsn_code = $('.main input[name="hsn_code"]').val(), e.description = $('.main input[name="autouser"]').val(), e.items_case_qty = $('.main input[name="items_case_qty"]').val(), e.pack_qty = $('.main input[name="pack_qty"]').val(), e.dis = $('.main input[name="dis"]').val(), e.dis_amt = $('.main input[name="dis_amt"]').val(), e.taxrate1 = $('.main input[name="taxrate1"]').val(), e.long_description = $('.main textarea[name="long_description"]').val(), e.qty = $('.main input[name="quantity"]').val(), e.stockqty = $('.main input[name="stockqty"]').val(), e.taxname = $(".main select.tax").selectpicker("val"), e.rate = $('.main input[name="rate"]').val(), e.unit = $('.main input[name="unit"]').val(), e
}
...
function calculate_total() {
    ...
    var a, i, e, n, s, o, l = {},
        ...
        location = $('input[name="customer_state_id"]').val();
        istcs = $('input[name="istcs"]').val(),
        istcsper = $('input[name="istcsper"]').val(),
        Y = 0;
        
    $(".tax-area").remove(), $.each(p, function() {
        if($(this).find("[data-quantity]").val() <=0){
            //alert("Please enter a value greater than or equal to 1.");
        }else{
          
          "" === (t = $(this).find("[data-quantity]").val()) && (t = 1, $(this).find("[data-quantity]").val(1)), s = accounting.toFixed($(this).find("td.rate input").val() * t , app.options.decimal_places), cs_cr = $(this).find("td.order_qty input").val(),total_pieces += parseFloat(cs_cr),  cs_cr_new = cs_cr / $(this).find("td.pack_qty input").val(), cs_cr_value = $(this).find("td.cs_cr input").val(), n = $(this), dis_per = $(this).find("td.dis input").val(), dis_amt = (s * dis_per / 100),total_dis_amt +=dis_amt, tax = (s-dis_amt) / 100 * $(this).find("td.taxrate input").val(), s = parseFloat(s), ss_new = s-dis_amt, ss = s, s -= dis_amt, s += tax, $(this).find("td.qty_cs_cr input").val(cs_cr_new.toFixed(1)), $(this).find("td.dis_amt input").val(dis_amt), $(this).find("td.amount").html(format_money(s, !0)), d += ss, (e = $(this).find("select.tax").selectpicker("val")) && $.each(e, function(e, t) {
            if(cs_cr_value == "CS"){ total_cases += cs_cr_new } else { total_crates += cs_cr_new }
            
            i = n.find('select.tax [value="' + t + '"]').data("taxrate"), a = ss_new / 100 * i, l.hasOwnProperty(t) ? l[t] = l[t] += a : 0 != i && (o = t.split("|"),/* tax_row = '<tr class="tax-area"><td>' + o[0] + "(" + i + '%)</td><td id="tax_id_' + slugify(t) + '"></td></tr>', $(_).after(tax_row), l[t] = a,*/ tgst +=a)
        })  
        }
    ...
    }), r += d, r +=tgst, "" !== u && 0 != u && "after_tax" == v && h.hasClass("discount-type-percent") ? c = r * u / 100 : "" !== f && 0 != f && "after_tax" == v && h.hasClass("discount-type-fixed") && (c = f), r -= c, m = parseFloat(m), isNaN(m) || (r += m);
    ...
    if(r == "0.00" || r == "0.000"){
        ...
        $('input[name="tax1"]').val(" ");
        $('input[name="taxes1"]').val("");
        $('#item_added_msg').text(" ");
    }
    r = r - total_dis_amt;
    if(istcs == 1){
        istcsper_new = istcsper + "%";
        $(".istcsper").html(istcsper_new);
        p = istcsper/100;
        Y = p * r;
        r = r + Y;
    }
    var rr = Math.round(r);
     if (location == 'UP') { sgst = tgst / 2, cgst += sgst, $(".igsttotaltr").hide(), $(".cgsttotaltr").show(), $(".sgsttotaltr").show() } else { igst += tgst, $(".cgsttotaltr").hide(), $(".sgsttotaltr").hide() }
    $('input[name="discount_total"]').val(accounting.toFixed(total_dis_amt, app.options.decimal_places)),$('input[name="total_cases"]').val(parseFloat(Math.ceil(total_cases))),$('input[name="total_pieces"]').val(parseFloat(Math.ceil(total_pieces))),$('input[name="total_crates"]').val(parseFloat(Math.ceil(total_crates))), $('input[name="total_tax"]').val(tgst), $(".discount-total").html(accounting.toFixed(total_dis_amt, app.options.decimal_places)), $(".adjustment").html(format_money(m)), $(".igsttotal").html(format_money(igst)), $(".sgsttotal").html(format_money(sgst)), $(".cgsttotal").html(format_money(cgst)), $(".cases").html(parseFloat(Math.ceil(total_cases)).toFixed(2)), $(".pieces").html(parseFloat(Math.ceil(total_pieces)).toFixed(2)), $(".crates").html(parseFloat(Math.ceil(total_crates)).toFixed(2)), $(".subtotal").html(format_money(d) + hidden_input("subtotal", accounting.toFixed(d, app.options.decimal_places))), $(".tcstotal").html(format_money(Y) + hidden_input("tcstotal", accounting.toFixed(Y, app.options.decimal_places))), $(".total").html(format_money(rr) + hidden_input("total", accounting.toFixed(rr, app.options.decimal_places))), $(document).trigger("sales-total-calculated")
}
...
function calculate_total2() {
    ...
    $(".tax-area").remove(), $.each(p, function() {
        ...
        var oldqty = $(this).find("td.order_qty1 input").val();
        var newqty = $(this).find("[data-quantity]").val();
        var StkQty = $(this).find("td.stock_qty input").val();
       
        if(parseFloat(oldqty) == parseFloat(newqty)){
            $($(this).find("td.reason_td input")).attr("required", false);
        }else{
            
            $($(this).find("td.reason_td input")).attr("required", true);
            //alert(oldqty + newqty)
        }
        var OQty = $(this).find("td.order_qty input").val();
        $($(this).find("td.order_qty input")).next('p').remove();
       /* //var PlantID = <?php $this->session->userdata('root_company'); ?>;
        var PlantID = <?php echo $_SESSION['root_company']; ?>;
        alert(PlantID);*/
        if(PlantID !== '1'){
            if(parseFloat(StkQty) < parseFloat(OQty) && OQty !== '0'){
                $($(this).find("td.order_qty input")).after('<p style="color:red;font-size:10px; line-height:11px;margin: 0px;">Stock Not Available</p>');
            }
        }
		"" === (t = $(this).find("[data-quantity]").val()) && (t = 1, $(this).find("[data-quantity]").val(1)), s = accounting.toFixed($(this).find("td.rate input").val() * t , app.options.decimal_places), cs_cr = $(this).find("td.order_qty input").val() ,total_pieces += parseFloat(cs_cr),  cs_cr_new = cs_cr / $(this).find("td.pack_qty input").val(), cs_cr_value = $(this).find("td.cs_cr input").val(), n = $(this), dis_per = $(this).find("td.dis input").val(), dis_amt = (s * dis_per / 100),total_dis_amt +=dis_amt, $(this).find("td.dis_amt input").val(dis_amt), tax = (s-dis_amt) / 100 * $(this).find("td.taxrate input").val(), s = parseFloat(s), ss_new = s-dis_amt, ss = s, s -= dis_amt, s += tax, $(this).find("td.amount").html(format_money(s, !0)), d += ss, (e = $(this).find("select.tax").selectpicker("val")) && $.each(e, function(e, t) {
    ...
    var rr = Math.round(r);
     if (location == 'UP') { sgst = tgst / 2, cgst += sgst, $(".igsttotaltr").hide(), $(".cgsttotaltr").show(), $(".sgsttotaltr").show() } else { igst += tgst, $(".cgsttotaltr").hide(), $(".sgsttotaltr").hide() }
    $('input[name="discount_total"]').val(accounting.toFixed(total_dis_amt, app.options.decimal_places)),$('input[name="total_cases"]').val(parseFloat(Math.ceil(total_cases))),$('input[name="total_pieces"]').val(total_pieces),$('input[name="total_crates"]').val(parseFloat(Math.ceil(total_crates))), $('input[name="total_tax"]').val(tgst), $(".discount-total").html(b),$(".discount-total").html(total_dis_amt), $(".adjustment").html(format_money(m)), $(".igsttotal").html(format_money(igst)), $(".sgsttotal").html(format_money(sgst)), $(".cgsttotal").html(format_money(cgst)), $(".cases").html(parseFloat(Math.ceil(total_cases)).toFixed(2)), $(".pieces").html(total_pieces), $(".crates").html(parseFloat(Math.ceil(total_crates)).toFixed(2)), $(".subtotal").html(format_money(d) + hidden_input("subtotal", accounting.toFixed(d, app.options.decimal_places))), $(".total").html(format_money(rr) + hidden_input("total", accounting.toFixed(rr, app.options.decimal_places))), $(document).trigger("sales-total-calculated")
}
...
function validate_invoice_form(e) {
    e = void 0 === e ? "#invoice-form" : e, appValidateForm($(e), {
        ...
        date1: {
				remote: {
					url: site_url + "admin/misc/checkorder_val",
					type: 'post',
					data: {
						order_date: function() {
							return $('input[name="date1"]').val();
						},
						OrderID: function() {
...
function merge_field_format_url(e, t, a, i) {
    return -1 < e.indexOf("%7B") && -1 < e.indexOf("%7D") && (e = e.replace("%7B", "{").replace("%7D", "}")), e
}
...
$("body").on("loaded.bs.select change", "select.ajax-search", function(e) {
    ...
    if (table_invoices = $("table.table-invoices"), table_estimates = $("table.table-estimates"), 0 < table_invoices.length || 0 < table_estimates.length) {
        ...
        }), table_invoices.length && initDataTable(table_invoices, admin_url + "invoices/table" + ($("body").hasClass("recurring") ? "?recurring=1" : ""), "undefined", "undefined", u, $("body").hasClass("recurring") ? [table_invoices.find("th.next-recurring-date").index(), "asc"] : [
            [3, "desc"],
            [0, "desc"]
    ...
    if (table_order = $("table.table-order")) {
        var u = {},
            f = $("._hidden_inputs._filters input");
        $.each(f, function() {
            u[$(this).attr("name")] = '[name="' + $(this).attr("name") + '"]'
        }), table_order.length && initDataTable(table_order, admin_url + "order/table" + ($("body").hasClass("recurring") ? "?recurring=1" : ""), "undefined", "undefined", u, $("body").hasClass("recurring") ? [table_order.find("th.next-recurring-date").index(), "desc"] : [
            [0, "desc"],
            [6, "desc"]
        ])
    }
    if (table_order = $("table.table-pending_order")) {
        var u = {},
            f = $("._hidden_inputs._filters input");
        $.each(f, function() {
            u[$(this).attr("name")] = '[name="' + $(this).attr("name") + '"]'
        }), table_order.length && initDataTable(table_order, admin_url + "order/pending_table" + ($("body").hasClass("recurring") ? "?recurring=1" : ""), "undefined", "undefined", u, $("body").hasClass("recurring") ? [table_order.find("th.next-recurring-date").index(), "desc"] : [
            [1, "desc"],
            [0, "desc"]
        ])
    }
    ...
    $("body").on("submit", "._transaction_form", function() {
    ...
    }), $("body").on("change", ".f_client_id1 #clientid", function() {
        var e = $(this).val(),
        ...
        requestGetJSON("order/client_change_data/" + e + "/" + (a = void 0 === a ? "" : a)).done(function(e) {
            tvalue = "";
            /*if(e.client_details['pending_order'] == false){
                alert("Please dispatch or cancel your existing order.");
                $('input[name="customer_group"]').val('');
                $('input[name="customer_group_id"]').val('');
                $('input[name="customer_state_id"]').val('');
                $('input[name="acct_bal"]').val('');
                $('input[name="dist_street"]').val('');
                $('input[name="istcs"]').val('');
                $('input[name="dist_route"]').val('');
                $('input[name="dist_comp"]').val('');
                $('input[name="dist_sale_agent"]').val('');
                $('input[name="location_type"]').val("");
                $('input[name="location_typevalue"]').val("");
                $('input[name="item_divistion"]').val('');
            }else{
                var tvalue = '';
                if(e.client_details['pending_order'] == "NonTaxItems"){
                    alert("We can add only NonTaxable Order.");
                    tvalue = 'NonTaxItems';
                    $('#item_added_msg').text("We can add only Non Taxable items..");
                }else if(e.client_details['pending_order'] == "TaxItems"){
                    alert("We can add only Taxable Order.");
                    tvalue = 'TaxItems';
                    $('#item_added_msg').text("We can add only Taxable items..");
                }*/
            ...
            //$('select[name="acct_bal"]').selectpicker("val", "madhav");
            if(e.customer_groups_name){
                var ss = e.customer_groups_name['name'];
                $('input[name="customer_group"]').val(ss);
                $('input[name="customer_group_id"]').val(e.customer_groups_name['id']);
                $('input[name="customer_state_id"]').val(e.client_details['state']);
            }
            
           // Account CLosing Balance
            var accbal = e.client_actbal;
            if(accbal > 0){
                newacbal = accbal.toFixed(2) + " Dr";
            }else{
                newacbal = accbal.toFixed(2) + " Cr";
            }
            // Account Last Bill
            if(e.client_last_bill == null){
                var last_bill_date_new = '';
                var last_bill_amt = '';
            }else{
                var last_bill_date = e.client_last_bill['Transdate'].substring(0, 10);
                var last_bill_date_new = last_bill_date.split("-").reverse().join("/");
                var last_bill_amt = e.client_last_bill['BillAmt'];
            }
            // Account LAst Deposite
            if(e.client_last_deposit == null){
                var last_deposit_date_new = "";
                var last_deposit_amt = "";
            }else{
                var last_deposit_date = e.client_last_deposit['Transdate'].substring(0, 10);
                var last_deposit_date_new = last_deposit_date.split("-").reverse().join("/");
                var last_deposit_amt = e.client_last_deposit['Amount'];
            }
            
			$('input[name="MaxCrdAmt"]').val(e.client_details['MaxCrdAmt']);
            $('input[name="acct_bal"]').val(newacbal);
            $('input[name="last_bill_date"]').val(last_bill_date_new);
            $('input[name="last_bill_amt"]').val(last_bill_amt);
            $('input[name="last_dep_date"]').val(last_deposit_date_new);
            $('input[name="last_dep_amt"]').val(last_deposit_amt);
            $('input[name="dist_street"]').val(e.client_details['address']);
            $('input[name="istcs"]').val(e.client_details['istcs']);
            $('input[name="act_gst"]').val(e.client_details['vat']);
            $('#PartyGST').val(e.client_details['vat']);
            $('#ship_to_act_gst').val(e.client_details['vat']);
            $('#billing_state').val(e.client_details['state_name']);
            $('#billing_city').val(e.client_details['city_name']);
            $('input[name="dist_route"]').val(e.client_details['routes']);
            $('input[name="dist_comp"]').val(e.client_details['company_assigned']);
            $('input[name="dist_sale_agent"]').val(e.client_details['company_assigned_staff']);
            $('#ShipToParty').val(e.client_details['AccountID']);
            $('#ShipToParty').selectpicker('refresh');
            
            let ShippingAddress = e.shipping_address;
			...
			$('input[name="shipping_city"]').val(ShippingAddress[0]['city_name']);
			$('input[name="shipping_state"]').val(ShippingAddress[0]['state_name']);
            // Location Type
            if(e.client_details['location_type']=="1"){
                $('input[name="location_type"]').val("Local");
                $('input[name="location_typevalue"]').val("Local");
            }
            if(e.client_details['location_type']=="2"){
                $('input[name="location_type"]').val("OutStation");
                $('input[name="location_typevalue"]').val("OutStation");
            }
            if(e.client_details['location_type']==" "){
                $('input[name="location_type"]').val("NotDefined");
                $('input[name="location_typevalue"]').val("NotDefined");
            }
            
            $('input[name="item_divistion"]').val(e.client_details['itemdivision']);
            $('input[name="tax1"]').val(tvalue);
            $('input[name="taxes1"]').val(tvalue);
            //init_billing_and_shipping_details();
        ...
        requestGetJSON("invoices/client_change_data/" + e + "/" + (a = void 0 === a ? "" : a)).done(function(e) {
            $("#merge").html(e.merge_info);
            var t = $("#expenses_to_bill");
            for (var a in 0 === t.length ? e.expenses_bill_info = "" : t.html(e.expenses_bill_info), "" !== e.merge_info || "" !== e.expenses_bill_info ? $("#invoice_top_info").removeClass("hide") : $("#invoice_top_info").addClass("hide"), billingAndShippingFields) - 1 < billingAndShippingFields[a].indexOf("billing") && (-1 < billingAndShippingFields[a].indexOf("country") ? $('select[name="' + billingAndShippingFields[a] + '"]').selectpicker("val", e.billing_shipping[0][billingAndShippingFields[a]]) : -1 < billingAndShippingFields[a].indexOf("billing_street") ? $('input[name="' + billingAndShippingFields[a] + '"]').val(e.billing_shipping[0][billingAndShippingFields[a]]) : $('input[name="' + billingAndShippingFields[a] + '"]').val(e.billing_shipping[0][billingAndShippingFields[a]]));
            for (var i in empty(e.billing_shipping[0].shipping_street) || $('input[name="include_shipping"]').prop("checked", !0).change(), billingAndShippingFields) - 1 < billingAndShippingFields[i].indexOf("shipping") && (-1 < billingAndShippingFields[i].indexOf("country") ? $('select[name="' + billingAndShippingFields[i] + '"]').selectpicker("val", e.billing_shipping[0][billingAndShippingFields[i]]) : -1 < billingAndShippingFields[i].indexOf("shipping_street") ? $('textarea[name="' + billingAndShippingFields[i] + '"]').val(e.billing_shipping[0][billingAndShippingFields[i]]) : $('input[name="' + billingAndShippingFields[i] + '"]').val(e.billing_shipping[0][billingAndShippingFields[i]]));
            //init_billing_and_shipping_details();
            var n = e.client_currency,
                s = $("body").find('.accounting-template select[name="currency"]');
...
$(document).ready(function() {
  $("#ordnumber").inputFilter(function(value) {
