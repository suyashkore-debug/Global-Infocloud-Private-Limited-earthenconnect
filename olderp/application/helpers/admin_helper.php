...
function staff_can_new($capability, $feature = null, $staff_id = '')
{
    $staff_id = empty($staff_id) ? get_staff_user_id() : $staff_id;
    ...
    if (!$permissions) {
        ...
        $permissions = $CI->staff_model->get_staff_permissions_new_for_chk_per($staff_id);
    }
    ...
    foreach ($permissions as $permission) {
        if ($feature == $permission['feature']
            && $capability == $permission['capability'] && $selected_company == $permission['plant_id'] && $selected_year == $permission['year']) {
            return hooks()->apply_filters('staff_can', true, $capability, $feature, $staff_id);
        }
    }
...
function has_permission_new($permission, $staffid = '', $can = '')
{
    return staff_can_new($can, $permission, $staffid);
}
/**
 * @since  1.0.0
