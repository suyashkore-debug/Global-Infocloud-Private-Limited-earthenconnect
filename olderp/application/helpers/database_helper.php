...
function get_staff_full_name1($userid = '')
{
    ...
    if ($userid == '' || $userid == $tmpStaffUserId) {
        if (isset($GLOBALS['current_user'])) {
            return $GLOBALS['current_user']->firstname . ' ' . $GLOBALS['current_user']->lastname . '(' .$GLOBALS['current_user']->email .')';
        }
    ...
    if (!$staff) {
        ...
        $staff = $CI->db->select('firstname,lastname,email')->from('tblstaff')->get()->row();
        //$CI->object_cache->add('staff-full-name-data-' . $userid, $staff);
    }

    return $staff ? $staff->firstname . ' ' . $staff->lastname . '(' .$staff->email .')' : '' ;
