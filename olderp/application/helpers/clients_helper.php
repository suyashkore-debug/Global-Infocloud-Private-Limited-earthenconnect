...
function get_staff_name($id = '')
{
    ...
    $contact = $CI->db->get(db_prefix() . 'staff')->row();
    return $contact;
}

function get_opening_bal($account_id = '')
{
    $fy = date("y");
    $CI = &get_instance();
    ...
    $CI->db->where('FY', $fy);
    $CI->db->where('AccountID', $account_id);
    $opening_bal = $CI->db->get(db_prefix() . 'accountbalances')->row();

    return $opening_bal;
}

function get_bal_on_bill($account_id = '')
{
    
    $CI = &get_instance();
    $selected_company = $CI->session->userdata("root_company");
    $CI->db->select('BalancesYN');
    $CI->db->where('PlantID', $selected_company);
    
    $CI->db->where('AccountID', $account_id);
    $bal_on_bill = $CI->db->get(db_prefix() . 'contacts')->row();
...
function get_state_name($id = '')
{
    ...
    $CI->db->where('id', $id);
    $state_name = $CI->db->get(db_prefix() . 'xx_statelist')->row();
...
{
    $CI = &get_instance();
    $short_name = $CI->db->select('short_name')
        ->where('id', $id)
...
function get_city_name($id = '')
{
    ...
    $CI->db->where('id', $id);
    $city_name = $CI->db->get(db_prefix() . 'xx_citylist')->row();

    return $city_name;
    
}

function get_account_name($acc_id = '',$selected_company)
{
...
function get_subgroup_name($SubActGroupID = '',$selected_company)
{
    
    $CI = &get_instance();

    $CI->db->select('SubActGroupName,ActGroupID');
    $CI->db->where('SubActGroupID', $SubActGroupID);
   // $CI->db->where('PlantID', $selected_company);
    $SubActGroup_name = $CI->db->get(db_prefix() . 'accountgroupssub')->row();

    return $SubActGroup_name;
    
}

function get_group_name($ActGroupID = '')
{
    
    $CI = &get_instance();

    $CI->db->select('ActGroupName');
    $CI->db->where('ActGroupID', $ActGroupID);
   // $CI->db->where('PlantID', $selected_company);
    $ActGroup_name = $CI->db->get(db_prefix() . 'accountgroups')->row();

    return $ActGroup_name;
    
}

function get_station_name($acc_id = '',$selected_company)
{
    
    $CI = &get_instance();

    $CI->db->select('StationName');
    $CI->db->where('AccountID', $acc_id);
    $CI->db->where('PlantID', $selected_company);
    $StationName = $CI->db->get(db_prefix() . 'clients')->row();

    return $StationName;
    
}

function get_state_name_by_acc_id($acc_id = '',$selected_company)
{
    ...
    $CI->db->where('AccountID', $acc_id);
    $CI->db->where('PlantID', $selected_company);
    $state = $CI->db->get(db_prefix() . 'clients')->row();
    //$short_name = get_state_code($state->state);
...
function get_dist_name_by_acc_id($acc_id = '',$selected_company)
{
    
    $CI = &get_instance();

    $CI->db->select('DistributorType');
    $CI->db->where('AccountID', $acc_id);
    $CI->db->where('PlantID', $selected_company);
    $acc_detail = $CI->db->get(db_prefix() . 'clients')->row();
    //$short_name = get_state_code($state->state);
    
    $name = $CI->db->select('name')
        ->where('id', $acc_detail->DistributorType)
...
function get_bal_by_acc_id($acc_id = '',$selected_company)
{
    ...
    $CI->db->where('PlantID', $selected_company);
    $CI->db->where('FY', $fy);
    $actBal = $CI->db->get(db_prefix() . 'accountbalances')->row();

    return $actBal;
    
}

function get_bill_amt($challan_id = '',$selected_company)
{
    ...
    $CI->db->where('PlantID', $selected_company);
    $CI->db->where('FY', $fy);
    $billBal = $CI->db->get(db_prefix() . 'challanmaster')->row();
