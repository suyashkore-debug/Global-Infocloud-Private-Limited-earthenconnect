...
class SplDisc_model extends App_Model
{
    ...
    public function ShowResultForExport($data)
     {
        ...
        $this->db->join(db_prefix() . 'accountlocations', '' . db_prefix() . 'accountlocations.AccountID = ' . db_prefix() . 'history.AccountID AND ' . db_prefix() . 'accountlocations.PlantID = ' . db_prefix() . 'history.PlantID');
        ...
        if($loc_type){
            $this->db->where(db_prefix() . 'accountlocations.LocationTypeID',$loc_type);
        }
        ...
        $this->db->where_in(db_prefix() . 'items.subgroup_id',$_ItemGroupSerializedArr);
    ...
    public function SaveResult($data)
     {
        ...
        if($checkDescID){
            ...
            $mmOld = "BAL".$mOld;
            foreach ($checkDescID as $key => $value) {
                $AccountBal = $this->GetAccountBal($value["AccountID"]);
                $current_bal = $AccountBal->$mmOld;
                
                if($value["TType"] == "C"){
                    $current_amt = $current_bal + $value["Amount"];
                }else{
                    $current_amt = $current_bal - $value["Amount"];
                }
        ...
        $affectedRow = false;
            foreach ($AccountIDList as $key => $value) {
                ...
                if($match == "1"){
                    ...
                    // Balance Update
                        $AccountBal = $this->GetAccountBal($value["AccountID"]);
                        $current_bal = $AccountBal->$mm;
                        
                        $current_amt = $current_bal - $DiscAmt;
                        $this->db->where('PlantID', $selected_company);
                        $this->db->LIKE('FY', $FY);
            ...
            // Balance Update
                $AccountBal = $this->GetAccountBal('CLAIM');
                $current_bal = $AccountBal->$mm;
                        
                $current_amt = $current_bal + $totalDebit;
                $this->db->where('PlantID', $selected_company);
                $this->db->LIKE('FY', $FY);
    ...
    public function SaleDetails($discItemGroupID,$TransdateFrom,$TransdateTo,$StateID,$LocationTypeID)
     {  
        ...
        if($LocationTypeID){
            $this->db->join(db_prefix() . 'accountlocations', '' . db_prefix() . 'accountlocations.AccountID = ' . db_prefix() . 'history.AccountID AND ' . db_prefix() . 'accountlocations.PlantID = ' . db_prefix() . 'history.PlantID');
            $this->db->where(db_prefix() . 'accountlocations.LocationTypeID',$LocationTypeID);
        }
        $this->db->join(db_prefix() . 'items', '' . db_prefix() . 'items.item_code = ' . db_prefix() . 'history.ItemID AND ' . db_prefix() . 'items.PlantID = ' . db_prefix() . 'history.PlantID');
        $this->db->where(db_prefix() . 'history.PlantID',$selected_company);
        $this->db->where(db_prefix() . 'history.FY',$FY);
        $this->db->where_in(db_prefix() . 'items.subgroup_id',$discItemGroupID);
