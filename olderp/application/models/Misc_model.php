...
class Misc_model extends App_Model
{
    ...
    public function get_taxes_dropdown_template($name, $taxname, $type = '', $item_id = '', $is_edit = false, $manual = false)
    {
        ...
        $select = '<select class="selectpicker display-block  tax" data-width="100%" name="' . $name . '" multiple disabled data-none-selected-text="' . _l('no_tax') . '">';
    ...
    public function perform_search($q)
    {
        ...
        if ($have_assigned_customers || $have_permission_customers_view) {
            ...
            // Clients
            $this->db->select(db_prefix() . 'clients.AccountID,'.db_prefix() . 'clients.company,'.db_prefix() . 'clients.StationName');
            ...
            $this->db->where('(company LIKE "%' . $this->db->escape_like_str($q) . '%" ESCAPE \'!\'
                OR AccountID LIKE "%' . $this->db->escape_like_str($q) . '%" ESCAPE \'!\'
                )');
        ...
        $Vendor_search = $this->_search_Vendor($q, $limit);
        if (count($Vendor_search['result']) > 0) {
            $result[] = $Vendor_search;
        }
        $AccountHead_search = $this->_search_AccountHead($q, $limit);
        if (count($AccountHead_search['result']) > 0) {
            $result[] = $AccountHead_search;
        }
        return $result;
    }
    public function perform_search11($q)
    {
        $q = trim($q);
        $this->load->model('staff_model');
        $is_admin                       = is_admin();
    ...
    public function _search_Vendor($q, $limit = 0)
    {
        $result = [
            'result'         => [],
            'type'           => 'Vendor',
        ...
        if (has_permission('staff', '', 'view')) {
            ...
            $this->db->from(db_prefix() . 'clients');
            $this->db->where(db_prefix() . 'clients.PlantID',$selected_company);
            $this->db->where(db_prefix() . 'clients.SubActGroupID','50003002');
            $this->db->where('(company LIKE "%' . $this->db->escape_like_str($q) . '%" ESCAPE \'!\'
                OR AccountID LIKE "%' . $this->db->escape_like_str($q) . '%" ESCAPE \'!\'
                )');
    ...
    public function _search_AccountHead($q, $limit = 0)
    {
        $result = [
            'result'         => [],
            'type'           => 'Head',
            'search_heading' => 'Account Head',
        ];
        $selected_company = $this->session->userdata('root_company');
        
        if (has_permission('staff', '', 'view')) {
            // Staff
            $SUBID = array('50003002','60001004');
            $this->db->select(db_prefix() . 'clients.AccountID,'.db_prefix() . 'clients.company,'.db_prefix() . 'clients.StationName');
            $this->db->from(db_prefix() . 'clients');
            $this->db->where(db_prefix() . 'clients.PlantID',$selected_company);
            $this->db->where_not_in(db_prefix() . 'clients.SubActGroupID',$SUBID);
            $this->db->where('(company LIKE "%' . $this->db->escape_like_str($q) . '%" ESCAPE \'!\'
                OR AccountID LIKE "%' . $this->db->escape_like_str($q) . '%" ESCAPE \'!\'
                )');
