...
class Clients_model extends App_Model
{
    ...
    public function SaveAccountDetails($Data)
    {
        ...
        $ItemDiv = $Data["ItemDivSerializedArr"];
        $CompAssign = $Data["CompSerializedArr"];
        
        $ItemDivArray = json_decode($ItemDiv, true);
        $CompAssignArray = json_decode($CompAssign, true);
        
        $ItemDivArraylen = count($ItemDivArray);
        ...
        $ClientArray = array(
            ...
            'CtrlAccountID' =>strtoupper($Data["CtrlAccountID"]),
        ...
        if($LastId){
            ...
            // Insert Item Division
            for($k=0; $k<$ItemDivArraylen; $k++) {
                $DivID = $ItemDivArray[$k][0];
                $DivPlantID = $ItemDivArray[$k][1];
                
                $InsAccountDiv = array(
                    'PlantID' =>$DivPlantID,
                    'AccountID' =>strtoupper($AccountID),
                    'ItemDivID' =>$DivID,
                    'plant_assign' =>$DivPlantID
                );
                $this->db->insert(db_prefix() . 'accountitemdiv',$InsAccountDiv);
            }
    ...
    public function UpdateAccountDetails($Data)
    {
        ...
        // Insert Item Division
            for($k=0; $k<$ItemDivArraylen; $k++) {
                $DivID = $ItemDivArray[$k][0];
                $DivPlantID = $ItemDivArray[$k][1];
                //if($DivPlantID == $selected_company){
                    $InsAccountDiv = array(
                        'PlantID' =>$DivPlantID,
                        'AccountID' =>strtoupper($AccountID),
                        'ItemDivID' =>$DivID,
    ...
    {
        $selected_company = $this->session->userdata('root_company');
        $this->db->select('tblclients.*, tblaccountlocations.LocationTypeID,tblcontacts.firstname,tblcontacts.lastname,tblcontacts.kms,tblcontacts.FLNO1,
    ...
    public function get_table_on_load_filter($data){
		...
		if(count($wh)>0)
		    ...
		    $this->db->select('tblclients.userid as userid,tblclients.AccountID as AccountID,tblclients.company,tblclients.itemdivision,tblcustomer_admins.staff_id as assigned_staff,tblclients.state,tblclients.address,tblclients.StationName,tblclients.city,tblclients.active,(SELECT GROUP_CONCAT(name SEPARATOR ",") FROM tblcustomers_groups WHERE tblcustomers_groups.id = tblclients.DistributorType) as customerGroups');
        ...
        $this->db->where('SubActGroupID', "60001004");
    ...
    public function getvehicle()
    {
        ...
        $selected_company = $this->session->userdata('root_company');
        //$this->db->select(db_prefix() . 'vehicle.*, '.db_prefix() . 'accountsld.EngageID');
        $this->db->select(db_prefix() . 'vehicle.*');
        //$this->db->join(db_prefix() . 'accountsld', '' . db_prefix() . 'accountsld.EngageID = ' . db_prefix() . 'vehicle.VehicleID AND ' . db_prefix() . 'accountsld.PlantID = ' . db_prefix() . 'vehicle.PlantID','LEFT');
