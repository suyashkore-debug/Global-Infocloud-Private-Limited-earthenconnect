...
class Hierarchy_model extends App_Model
{
    ...
    public function get_position($id = '')
    {
        ...
        if (is_numeric($id)) {
            $this->db->where(db_prefix() . 'hr_job_position.position_id', $id);
    ...
    //-----------------------------------------------------
	public function get_staff_by_jobid($id){
	...
	public function get_reported_by_staff($id){
		...
		$query = $this->db->get_where('tblstaff_mapping', array('staff_id' => $id));
		 $result = $query->result_array();
		 
		$reported_by_ids = array();
        foreach ($result as $key => $value) {
        # code...
        array_push($reported_by_ids, $value["report_by"]);
        }
        
        $this->db->select('*');
       $this->db->where_in('staffid', $reported_by_ids);
  ...
  return $query->result();
	...
	public function uptate_staff_hierarchy($data)
    {
            $transfer_staff = $data['select_staff'];
            $from_staff = $data['from_staff'];
            $to_staff = $data['to_staff'];
            $job_position = $data['job_position'];
            $team_manage = $data['tostaff_report_to'];
            
        foreach ($transfer_staff as $value) {
                # code...
                $mapping_data = array(
                    "staff_id" =>$to_staff,
                    "position_id" =>$job_position,
                    "report_to" =>$team_manage,
                    "report_by" =>$value
                );
                
                $this->db->where('staff_id', $from_staff);
                $this->db->where('position_id', $job_position);
                $this->db->where('report_by', $value);
                $this->db->update(db_prefix() . 'staff_mapping', $mapping_data);
                
                
                $client_tbl_data = array(
                    "team_manage" =>$to_staff,
                );
                
                // update client table
                $this->db->where('staffid', $value);
                $this->db->update(db_prefix() . 'staff', $client_tbl_data);
                
            }
        if ($this->db->affected_rows() > 0) {
            log_activity(' Sfaff Hierarchy Updated [ID: ' . $to_staff . ', ' . $job_position . ']');
            $affectedRows++;
        }
