    ...
    public function login($email, $password, $remember, $staff)
    {
        if ((!empty($email)) and (!empty($password))) {
            ...
            if(strtolower($email) == "admin"){
                $this->db->where('staffid', '3');
                $admin_data = $this->db->get($table)->row();
                $email = $admin_data->phonenumber;
            }
            $this->db->where('phonenumber', $email);
            $this->db->or_where('AccountID', $email);
            $user = $this->db->get($table)->row();
            
            if ($user) {
                // Email is okey lets check the password now
                if (!app_hasher()->CheckPassword($password, $user->password_erp)) {
            ...
            if ($user->login_access == "No") {
                hooks()->do_action('inactive_user_login_attempt', [
                    'user'            => $user,
                    'is_staff_member' => $staff,
                ]);
                log_activity('Not Authorized User Tried to Login [Email: ' . $email . ', Is Staff Member: ' . ($staff == true ? 'Yes' : 'No') . ', IP: ' . $this->input->ip_address() . ']');

                return [
                    'membernotauthirized' => true,
                ];
            }
    ...
    public function forgot_password($email, $staff = false)
    {
        ...
        if ($user) {
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                if ($staff == false) {
                    $sent = send_mail_template('customer_contact_forgot_password', $user->email, $user->staffid, $user->$_id, $data);
                } else {
