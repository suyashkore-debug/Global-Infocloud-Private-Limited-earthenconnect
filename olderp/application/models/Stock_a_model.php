...
// return $rs;
    ...
    //   echo $data['prd_date'];
        ...
        if($PlantID == "1"){
            ...
            $get_adj_stock = $this->get_adj_stock($item_c->item_code);
            $new_adj_stock = $get_adj_stock->ADJQTY + $value['QTY'];
    ...
    public function get_stock_order_detail($request,$group_id,$state){
        ...
        $this->db->select(db_prefix() . 'history.*,'.db_prefix() . 'items.*,'.db_prefix() . 'clients.*,'.db_prefix() . 'taxes.taxrate');
        $this->db->from(db_prefix() . 'history');
      ...
      foreach($data  as $key => $value){
         if($data[$key]['taxrate'] != NULL){
             if($data[$key]['state'] == 'UP'){
            $per =     $data[$key]['taxrate']/2;
              $data[$key]['cgst_per'] = $per;
              $data[$key]['sgst_per'] = $per;
              $data[$key]['igst_per'] = '';
          }else{
              $data[$key]['cgst_per'] = '';
              $data[$key]['sgst_per'] = '';
              $data[$key]['igst_per'] = $data[$key]['taxrate'];
          } 
          }
          $data[$key]['all_qty'] =  $data[$key]['BilledQty'];
          $data[$key]['CaseQty'] =  $data[$key]['BilledQty'] / $data[$key]['CaseQty'];
    ...
    public function update_stock_adj($data,$id){
        ...
        $old_item_code = array();
            ...
            foreach($old_item_code as $value1){
                $old_stock = $this->get_old_stock($id,$value1);
                $get_adj_stock = $this->get_adj_stock($value1);
                $new_adj_stock = $get_adj_stock->ADJQTY - $old_stock->BilledQty;
               
                $this->db->where('PlantID', $PlantID);
                $this->db->LIKE('FY', $FY);
                $this->db->where('ItemID', $value1);
                $this->db->where('GodownID',$GodownID);
                $this->db->update(db_prefix() . 'stockmaster', [
                                  'ADJQTY' => $new_adj_stock,
                              ]);
            ...
            if (in_array($item_c->item_code, $old_item_code)){
                $old_stock = $this->get_old_stock($id,$item_c->item_code);
                $get_adj_stock = $this->get_adj_stock($item_c->item_code);
                $new_adj_stock = $get_adj_stock->ADJQTY + $value['QTY'];
