...
function myFunction_for_crate_d(data) {
}
data = <?php echo json_encode($return_crate_list); ?>;
if(data != ''){
	...
	if(changes1 !== null){
	    changes1.forEach(([row1, prop1, oldValue, newValue]) => {
           ...
           if(prop1 == 'act_id'){
  	           ...
  	           var balance_c =  (hot1.getDataAtCell(row1,6))-newValue;
            ...
            for (var row_index = 0; row_index <= 10; row_index++) {
                if((hot1.getDataAtCell(row_index, 5)) != null){
                    total_return += (parseFloat(hot1.getDataAtCell(row_index, 5)));
                }
            }
...
//here end crate-details table 
 ...
 data2 = <?php echo json_encode($return_payment_list); ?>;
if(data2 != ''){
    var dataObject2 = data2;
...
hot2.addHook('afterChange', function(changes, src) {
  ...
  //here table for expense_details
   data3 = <?php echo json_encode($return_expense_list); ?>;
if(data3 != ''){
    var dataObject3 = data3;
...
<?php }?>
  ...
  var hotElement1 = document.querySelector('#fresh_stock_return');
    var hotElementContainer = hotElement1.parentNode;
