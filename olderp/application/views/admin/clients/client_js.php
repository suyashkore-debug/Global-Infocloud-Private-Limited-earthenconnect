...
//alert(today);
    //$('#custom_fields[customers][6]').val(today);
   ...
   initDataTable('.table-credit-notes', admin_url+'credit_notes/table/'+customer_id, ['undefined'], ['undefined'], undefined, [0, 'desc']);
    ...
    /*if(typeof(customer_id) == 'undefined'){
            ...
            .done(function(response) {
                if(response) {
                    ...
                    if(response.exists == true) {
                        ...
                        $companyExistsDiv.html('<div class="info-block mbot15" style="color:red;">'+response.message+'</div>');
                    } else {
                        $companyExistsDiv.addClass('hide');
                    }
                }
        ...
        $('#AccountID').on('blur', function() {
            ...
            var $companycodeExistsDiv = $('#companycode_exists_info1');

            if(AccountID == '') {
                $companycodeExistsDiv.addClass('hide');
                return;
            }

            $.post(admin_url+'clients/check_duplicate_customer_code', {AccountID:AccountID})
            .done(function(response) {
                if(response) {
                    response = JSON.parse(response);
                    if(response.exists == true) {
                        $companycodeExistsDiv.removeClass('hide');
                        $companycodeExistsDiv.html('<div class="info-block mbot15" style="color:red;">'+response.message+'</div>');
                    } else {
                        $companycodeExistsDiv.addClass('hide');
                    }
                }
    ...
    $('.billing-same-as-customer1').on('click', function(e) {
       ...
       // $('select[name="billing_city"]').selectpicker('val', $('select[name="city"]').selectpicker('val'));
        ...
        $('select[name="billing_country"]').selectpicker("");
            }
    });
    ...
    $('.customer-copy-billing-address1').on('click', function(e) {
        //e.preventDefault();
        if($(this).prop("checked") == true){
        $('input[name="shipping_street"]').val($('input[name="address"]').val());
        
        var selectedText = $("#city option:selected").html();
        $('input[name="shipping_city"]').val(selectedText);
       // $('input[name="shipping_city"]').val($('input[name="city"]').val());
       
       $('select[name="shipping_state"]').selectpicker('val', $('select[name="state"]').selectpicker('val'));
...
function beat() {
    ...
    requestGet('clients/form_beat/').done(function(response) {
        $('#contact_data').html(response);
        $('#contact').modal({
            show: true,
            backdrop: 'static'
