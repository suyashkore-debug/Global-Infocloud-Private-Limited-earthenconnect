							...
							<div class="col-md-3 leads-filter-column">
								...
								<select name="Staff_AccountID" class="selectpicker" id="Staff_AccountID" data-width="100%"  data-live-search="true" data-none-selected-text="<?php echo _l('Select Staff Name'); ?>"> 
									...
									foreach ($staff as $value) { ?>
										<option data-name="<?php echo $value['firstname'].' '.$value['lastname'] ?>" data-id="<?php echo html_entity_decode($value['AccountID']); ?>" value="<?php echo html_entity_decode($value['staffid']); ?>"><?php echo $value['firstname'].' '.$value['lastname'] ?></option>
									<?php }
									?>              
								</select>
							</div>
           ...
           &nbsp;<a class="btn btn-default  mleft5 buttons-html5"  tabindex="0" aria-controls="table-daily_report" href="#" id="caexcel"><span>Export to excel</span></a>
            ...
            <span id="searchh2" style="display:none;">
							...
							if (has_permission_new('staff_target', '', 'edit')) {	
							 ?>
								 <button type="submit"  class="btn btn-info pull-right mleft5 submit_button" style="margin-top: 19px; display:none;">Submit</button>
							<?php } ?>
							</div>
							</form>
						</div>
...
<style>
  ...
  {
    $.ajax({
      url:"<?php echo admin_url(); ?>misc_reports/get_targetList",
      dataType:"json",
      method:"POST",
    ...
    var msg = "Month: "+fullMonth+"-"+year+", Staff: "+account_name;
	    $(".report_for").text(msg);
	    $(".submit_button").show();
...
$('#staff_account_name').val(id);
        ...
        function myFunction_data(div_id,accountId) {
    var lowertotal =0;
    var lefttotal =0;
    var lefttotal_lower =0;
    $('.target_count_'+div_id).each(function() {
        ...
        lowertotal += parseFloat(quantity);
    });
     $('.target_data_account_'+accountId).each(function() {
        var total = $(this).val();
         if(total != ''){
            total = total
        }else{
        ...
        lefttotal += parseFloat(total);
    });
  
    $('.target_count_total_lower_'+div_id).html('<b>'+parseFloat(lowertotal)+'</b>');
    $('.target_count_total_left_'+accountId).html('<b class="left_lower_total">'+parseFloat(lefttotal)+'</b>');
    $('.left_lower_total').each(function() {
        var total_left = $(this).html();
        ...
        lefttotal_lower += parseFloat(total_left);
    });
    $('.left_lower_total_count').html(parseFloat(lefttotal_lower));
}
...
$("#caexcel").click(function(){
	    ...
	    $.ajax({
            url:"<?php echo admin_url(); ?>misc_reports/export_target_report",
            method:"POST",
            data:{staff_d:staff_d, month_data:month_data,staff_account_name:staff_account_name},
            beforeSend: function () {
