...
class Hierarchy extends AdminController
{
    ...
    {
        if (!has_permission('hierarchy', '', 'update')) {
         ...
         if ($this->input->post()) {
            ...
            $update = $this->hierarchy_model->uptate_staff_hierarchy($data);
            if($update == true){
                set_alert('success', _l('updated_successfully', "Staff Hierarchy11"));
                redirect(admin_url('hierarchy'));
            }
        ...
        $data['position'] = $this->hierarchy_model->get_position();
        
        /*echo "<pre>";
        print($data['route']);
        die;
*/
        $data['title'] = "Hierarchy";
        $this->load->view('admin/hierarchy/manage', $data);
    }
    ...
    public function select_staff()
    {
        ...
        $staff_data = $this->hierarchy_model->get_staff_by_jobid($id);
    ...
    //--------------------------------------------------
    public function reported_by_staff()
    {
        $id=$this->input->post('id'); 
        $reported_bystaff_data = $this->hierarchy_model->get_reported_by_staff($id);
        
        /*$html = '';
        foreach ($reported_bystaff_data as $key => $value) {
        # code...
        
        $html .= '<input type="checkbox" name="staff_id" class="chk" value="'.$value->staffid.'">   '.$value->firstname.' '.$value->lastname.'<br>';
    }*/
      
    echo json_encode($reported_bystaff_data);
    }
    ...
    public function bulk_action()
    {
        ...
        if ($this->input->post()) {
            ...
            $has_permission_delete = has_permission('hierarchy', '', 'update');
            if (is_array($ids)) {
                foreach ($ids as $id) {
