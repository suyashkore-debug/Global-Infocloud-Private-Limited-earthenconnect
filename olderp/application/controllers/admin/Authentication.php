...
class Authentication extends App_Controller
{
    ...
    public function admin()
    {
        ...
        $this->form_validation->set_rules('email1', _l('admin_auth_login_email'), 'trim|required');
        if (show_recaptcha()) {
        ...
        if ($this->input->post()) {
            if ($this->form_validation->run() !== false) {
                ...
                }elseif (is_array($data) && isset($data['membernotauthirized'])) {
                    set_alert('danger', "You are not authorized to login here...");
                    redirect(admin_url('authentication'));
