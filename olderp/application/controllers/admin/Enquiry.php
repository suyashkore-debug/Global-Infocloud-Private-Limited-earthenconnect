...
class Enquiry extends AdminController
{
    ...
    public function index()
    {
        ...
        $data['ss'] = $this->enquiry_model->get();
        /*echo "<pre>";
        print_r($data['ss']);
        die;*/
    ...
    public function client_add()
    {
        ...
        if ($this->input->post() && !$this->input->is_ajax_request()) {
            ...
            unset($data[$company_assigned_sales_p_id]);
             ...
             //echo "<pre>";
            //print_r($company_sales_person_assign_data);
           ...
           /* $company_assigned_sales_p = array();
                ...
                //print_r($Selected_company_assigned_sales_p_ids);
                    ...
                    if($Selected_company_assigned_sales_p_ids !== ""){
                        ...
                        foreach($Selected_company_assigned_sales_p_ids as $assigned_ids){ 
                            ...
                            //$company_assigned_sales_p[$r_company["id"]] = $data[$company_assigned_sales_p_id];
                        }
                        //array_push($itemDivision_comp,$Selected_item_division_comp_ids);
                        //$company_assigned_sales_p[$r_company["id"]] = $data[$company_assigned_sales_p_id];
                        unset($data[$company_assigned_sales_p_id]);
            ...
            if (isset($data['phonenumber_contact'])) {
            $contacts_fields["phonenumber"] = $data['phonenumber_contact'];
            unset($data['phonenumber_contact']);
            }
            
            if (isset($data['is_primary'])) {
            $contacts_fields["is_primary"] = $data['is_primary'];
          ...
          /* echo "<pre>";
            print_r($company_assigned_staff);
            print_r($data);
            die;*/
            $customer_id = $this->clients_model->add($data);
            if($customer_id){
                
                foreach ($itemDivision as $key => $value) {
                       
                            $this->db->insert(db_prefix() . 'customers_item_division', [
                                'client_id'   => $customer_id,
                                'division_id'      => $value,
                            ]);
                        
                    }
                $contact_id      = $this->clients_model->add_contact($contacts_fields, $customer_id);
           ...
           /* echo "<pre>";
            ...
            $successcontacts = $this->clients_model->update_contact_new($contacts_fields, $id);
                ...
                if ($success) {
                    ...
                    $current_division     = $this->get_current_division($id);
                    /*echo "<pre>";
                    print_r($itemDivision);
                    print_r($current_division);
                    die;*/
                    foreach ($current_division as $key => $value) {
                           
                            $this->db->where('client_id', $id);
                            $this->db->delete(db_prefix() . 'customers_item_division');
