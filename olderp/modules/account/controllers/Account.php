...
class account extends AdminController
{
    ...
    public function index()
    {
        ...
        $this->load->view('account_dashboard.php', $data);
    }
    /* List all Voucher */
    public function manage_voucher($id = '')
    {
        $this->load->model('departments_model');
        ...
        $data['positions'] = $this->account_model->get_job_position();
        $data['workplace'] = $this->account_model->get_workplace();
        $data['contract_type'] = $this->account_model->get_contracttype();
        $data['staff'] = $this->staff_model->get();
        $data['allowance_type'] = $this->account_model->get_allowance_type();
        $data['salary_form'] = $this->account_model->get_salary_form();
        $data['duration'] = $this->account_model->get_duration();

        $data['dep_tree'] = json_encode($this->account_model->get_department_tree());
        /*print_r($data['dep_tree']);
        die;*/

        $data['title']                 = "Manage Voucher";
        $this->load->view('voucher/manage', $data); 
    }

    public function table_voucher_register()
    {
        $this->app->get_table_data(module_views_path('account', 'voucher/table_voucher_register'));
    }

    /* List all Account Group Wise Balance  */
    public function account_group_wise_balance($id = '')
    {
        $this->load->model('departments_model');
        $this->load->model('staff_model');
        ...
        $data['title']                 = "Account Group Wise Balance";
        $this->load->view('account-group-wise-balance/manage', $data); 
    }

    public function table_group_wise_balance()
    {
        $this->app->get_table_data(module_views_path('account', 'account-group-wise-balance/table_group_wise_balance'));
    }

     /* List all Account Monitor  */
    public function account_monitor($id = '')
    {
        $this->load->model('departments_model');
        $this->load->model('staff_model');
        ...
        $this->load->view('account-monitor/manage', $data); 
    }

    /* List all Credit/Debit Note Report  */
    public function credit_debit_note($id = '')
    {
        $this->load->model('departments_model');
        $this->load->model('staff_model');
        ...
        $data['title']                 = "Credit/Debit Note Report";
        $this->load->view('creditdebit-note/manage', $data); 
    }

    /* List all Trial Balance Report  */
    public function trial_balance($id = '')
    {
        $this->load->model('departments_model');
        $this->load->model('staff_model');
        ...
        $this->load->view('trialbalance/manage', $data); 
    }

    public function table_trial_balance()
    {
        $this->app->get_table_data(module_views_path('account', 'trialbalance/table_trial_balance'));
    }

    /* List all Account Daily Balance  */
    public function account_daily_balance($id = '')
    {
        $this->load->model('departments_model');
        $this->load->model('staff_model');
        ...
        $data['title']                 = "Account Daily Balance";
        $this->load->view('daily-balance/manage', $data); 
    }

    public function table_daily_balance()
    {
        $this->app->get_table_data(module_views_path('account', 'daily-balance/table_daily_balance'));
    }

    /* List all TCS Reports  */
    public function tcs_reports($id = '')
    {
        $this->load->model('departments_model');
        $this->load->model('staff_model');
    ...
    public function table_tcs_reports()
    {
        $this->app->get_table_data(module_views_path('account', 'tcs/table_tcs_reports'));
    }

    /* List all Account Ledger  */
    public function account_ledger($id = '')
    {
        $this->load->model('departments_model');
        $this->load->model('staff_model');
        ...
        $this->load->view('account-ledger/manage', $data); 
    }

    public function table_account_ledger()
    {
        $this->app->get_table_data(module_views_path('account', 'account-ledger/table_account_ledger'));
    }

    

    /* List all Contra */
    public function manage_countra()
    {
        $this->load->model('departments_model');
        ...
        if (!has_permission('hrm', '', 'view')) {
         ...
         if ($this->input->is_ajax_request()) {
            $this->app->get_table_data(module_views_path('account', 'table_staff'));
        }
        $data['staff_members'] = $this->account_model->get_staff('', ['active' => 1]);
        $data['title']                 = _l('staff_infor');

        $data['dep_tree'] = json_encode($this->account_model->get_department_tree());
        $data['staff_role'] = $this->roles_model->get();
        
        $data['title']                 = "Contra Manage";
        $this->load->view('contra/contra_manage', $data);
    }

    /* Contra List update */
    public function contra_edit($id = '')
    {
        $this->load->model('departments_model');
        $this->load->model('roles_model');
        if (!has_permission('hrm', '', 'view')) {
        ...
        $data['title']                 = "Contra Voucher List Manage";
        $this->load->view('contra/contra_entry', $data);
    }
    ...
    public function contra($id = '')
    {
        ...
        $data['bodyclass']  = 'contra';
        //$data['currencies'] = $this->currencies_model->get();
        $data['title']      = "Add New Contra Voucher";
        $this->load->view('contra/contra', $data);
    } 
    ...
    public function manage_journal()
    {
        ...
        $data['title']                 = "Journal Manage";
        $this->load->view('journal/journal_manage', $data); 
    }
    ...
    public function journal_edit($id = '')
    {
        ...
        $data['title']                 = "Journal Voucher List Manage";
        $this->load->view('journal/journal_entry', $data);
    }
    ...
    public function journal($id = '')
    {
        ...
        $data['bodyclass']  = 'journal';
        //$data['currencies'] = $this->currencies_model->get();
        $data['title']      = "Add New Journal Voucher";
    ...
    public function manage_receipts()
    {
       $this->load->model('departments_model');
        ...
        $data['title']                 = "Receipts Manage";
        $this->load->view('receipts/receipts_manage', $data);
    }
    ...
    public function receipt_edit($id = '')
    {
        ...
        $data['title']                 = "Receipts Voucher List Manage";
        $this->load->view('receipts/receipts_entry', $data);
    }
    ...
    public function receipts($id = '')
    {
        ...
        $data['bodyclass']  = 'receipts';
        //$data['currencies'] = $this->currencies_model->get();
        $data['title']      = "Add New Receipts Voucher";
    ...
    public function manage_payment()
    {
       $this->load->model('departments_model');
        ...
        $data['title']                 = "Payment Manage";
        $this->load->view('payment/payment_manage', $data);
    }
    ...
    public function payment_edit($id = '')
    {
        ...
        $data['title']                 = "Payment Voucher List Manage";
    ...
    public function payment($id = '')
    {
        ...
        $data['bodyclass']  = 'payment';
        //$data['currencies'] = $this->currencies_model->get();
        $data['title']      = "Add New Payment Voucher";
    ...
    public function table_contra()
    {
        $this->app->get_table_data(module_views_path('account', 'contra/table_contra'));
    }
     public function table_contra_entry()
    {
        $this->app->get_table_data(module_views_path('account', 'contra/table_contra_entry'));
    }
     public function table_contra_entry2()
    {
        $this->app->get_table_data(module_views_path('account', 'contra/table_contra_entry2'));
    }
    public function table_journal()
    {
        $this->app->get_table_data(module_views_path('account', 'journal/table_journal'));
    }
    public function table_journal_entry()
    {
        $this->app->get_table_data(module_views_path('account', 'journal/table_journal_entry'));
    }

    public function table_receipts()
    {
        $this->app->get_table_data(module_views_path('account', 'receipts/table_receipts'));
    }

    public function table_receipts_entry()
    {
        $this->app->get_table_data(module_views_path('account', 'receipts/table_receipts_entry'));
    }

    public function table_payment()
    {
        $this->app->get_table_data(module_views_path('account', 'payment/table_payment'));
    }

    public function table_payment_entry()
    {
        $this->app->get_table_data(module_views_path('account', 'payment/table_payment_entry'));
    }
    ...
    public function payroll()
    {
        ...
        $data['payrolls'] = $this->account_model->get_payroll_table();
        $data['month'] = $this->account_model->get_month();
        $data['payroll_type']   = $this->account_model->get_payroll_type();
 ...
 public function delete_payroll_type($id)
    ...
    public function contract_type($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if (!$this->input->post('id')) {
                $id = $this->account_model->add_contract_type($data);
                if ($id) {
                    $success = true;
    ...
    {
        $this->app->get_table_data(module_views_path('account', 'voucher/voucher_table_html'));
    }
    ...
    public function contract($id = '')
    {
        ...
        $this->load->view('account/contract', $data);
    }
...
public function delete_hrm_staff_attachment($attachment_id)
    ...
    {
        $contract = $this->account_model->get_contract($id);
            $contract_detail = $this->account_model->get_contract_detail($id);
            if (!$contract) {
                blank_page('Contract Not Found', 'danger');
            }
            ...
            $data['contracts']            = $contract;
            if(isset($contract[0]['staff_delegate'])){
            $data['staff_delegate_role'] = $this->account_model->get_staff_role($contract[0]['staff_delegate']);
            }
            $data['contract_details']            = $contract_detail;
            $title                     = $this->account_model->get_contracttype_by_id($contract[0]['name_contract']);
            $data['title']         = $title[0]['name_contracttype'];
        $data['positions'] = $this->account_model->get_job_position();
        $data['workplace'] = $this->account_model->get_workplace();
        $data['contract_type'] = $this->account_model->get_contracttype();
        ...
        $this->load->view('account/contract_preview_template', $data);
    }
    ...
    public function insurances(){
        ...
        $this->load->view('account/insurance/manage_insurance', $data);
    }
