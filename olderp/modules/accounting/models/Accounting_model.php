...
class Accounting_model extends App_Model
{
    ...
    public function get_accounts_for_journal($id = '', $where = [])
    {
        ...
        //$subgroup = array('60001007','50003001');
        //$subgroup = array('60001007','60001008','50003001');
    ...
    public function get_data_account_to_select_for_contra() {
        ...
        foreach ($accounts as $key => $account) {
            ...
            //$note['label'] = $account['name'].' - '.$_account_type_name;
            $note['label'] = $account['company'].' - '.$account['AccountID'].' - '.$account['SubActGroupName'].' - '.$account['city'];
    ...
    public function load_data_for_payment($data)
     {  
        ...
        $sql1 = '('.db_prefix().'accountledger.Transdate BETWEEN "'.$from_date.' 00:00:00" AND "'.$to_date.' 23:59:59") AND PassedFrom = "PAYMENTS" AND '.db_prefix().'accountledger.FY = "'.$fy.'" AND '.db_prefix().'accountledger.PlantID = "'.$selected_company.'" ORDER BY ABS(tblaccountledger.VoucherID)';
        
        $sql ='SELECT '.db_prefix().'accountledger.*,  
        (SELECT GROUP_CONCAT(company SEPARATOR ",") FROM '.db_prefix().'clients WHERE '.db_prefix().'clients.AccountID = '.db_prefix().'accountledger.AccountID AND '.db_prefix().'clients.PlantID = '.$selected_company.') as AccountName, 
        (SELECT GROUP_CONCAT(firstname SEPARATOR ",") FROM '.db_prefix().'staff WHERE '.db_prefix().'staff.AccountID = '.db_prefix().'accountledger.AccountID AND tblstaff.PlantID = '.$selected_company.') as firstname,
        (SELECT GROUP_CONCAT(lastname SEPARATOR ",") FROM '.db_prefix().'staff WHERE '.db_prefix().'staff.AccountID = '.db_prefix().'accountledger.AccountID AND tblstaff.PlantID = '.$selected_company.') as lastname,
        (SELECT GROUP_CONCAT(AccountID SEPARATOR ",") FROM '.db_prefix().'staff WHERE '.db_prefix().'staff.AccountID = '.db_prefix().'accountledger.AccountID AND tblstaff.PlantID = '.$selected_company.') as staff_AccountID
        FROM '.db_prefix().'accountledger WHERE '.$sql1;
        //(SELECT GROUP_CONCAT(firstname SEPARATOR ",") FROM '.db_prefix().'staff WHERE '.db_prefix().'staff.AccountID = '.db_prefix().'accountledger.AccountID AND tblstaff.staff_comp REGEXP '.$regExp1.'"'.$selected_company.'"'.$regExp2.') as firstname,
        $result = $this->db->query($sql)->result_array();
        return $result;
     }
    ...
    public function load_data_for_receipts($data)
     {  
        ...
        $sql1 = '('.db_prefix().'accountledger.Transdate BETWEEN "'.$from_date.' 00:00:00" AND "'.$to_date.' 23:59:59") AND PassedFrom = "RECEIPTS" AND '.db_prefix().'accountledger.FY = "'.$fy.'" AND '.db_prefix().'accountledger.PlantID = "'.$selected_company.'" ORDER BY ABS(tblaccountledger.VoucherID)';
    ...
    public function add_journal_entry($data){
       ...
       /* echo "<pre>";
        ...
        foreach ($journal_entry as $key => $value) {
            if($value[0] != ''){
                $get_account_bal = $this->get_acc_bal($value[0]);
                if(empty($get_account_bal)){
                    ...
                    if(empty($value[3])){
                        $new_bal = $current_bal - $value[4];
                        $ttype = "C";
                        $amount = $value[4];
                    }else{
                        $new_bal = $current_bal + $value[3];
                        $ttype = "D";
                        $amount = $value[3];
                    }
                    $this->db->where('PlantID', $selected_company);
                    $this->db->LIKE('FY', $fy);
                    $this->db->where('AccountID', $value[0]);
    ...
    public function add_receipts_entry($data){
        ...
        if($month == "01"){
            ...
            $get_account_bal = $this->get_acc_bal($data['ganeral_account']);
            if(empty($get_account_bal)){
                $insertActBal = array(
                    'PlantID'=>$selected_company,
                ...
                $this->db->where('PlantID', $selected_company);
                $this->db->LIKE('FY', $fy);
                $this->db->where('AccountID', $data['ganeral_account']);
                $this->db->update(db_prefix() . 'accountbalances', [
                                            $mm => $new_bal,
                                        ]);
            }
            ...
            foreach ($receipts_entry as $key => $value) {
                if($value[0] != ''){
                    $get_account_bal2 = $this->get_acc_bal($value[0]);
    ...
    public function GetSaleIds($data_filter)
    {
        ...
        $this->db->select(db_prefix().'salesmaster.SalesID,'.db_prefix().'salesmaster.ChallanID');
...
/*echo "<pre>";
    ...
    public function get_contra_entry($id){
        ...
        $total_credit = 0;
            ...
            $contra_entrie->damt = $total_debit;
            $contra_entrie->camt = $total_credit;
/*echo "<pre>";
        print_r($journal_entrie);
        die;*/
...
/*echo "<pre>";
    ...
    public function delete_receipt_entry($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
           ...
           $sql = 'UPDATE tblaccountledger SET VoucherID = abs(VoucherID) - 1 where abs(VoucherID) > "'.$id.'" AND PassedFrom = "RECEIPTS" AND FY = "'.$fy.'" AND PlantID = '.$selected_company;
            $this->db->query($sql);
            /*if ($this->db->affected_rows() > 0) {*/
    ...
    public function delete_payment_entry($id)
    {
        ...
        $payments_entry_details = $this->get_payments_entry_detail($id);
    ...
    public function update_journal_entry($data,$id){
        ...
        foreach ($journal_details as $key => $value) {
           ...
           $get_account_bal_for_old = $this->get_acc_bal($value["AccountID"]);
           $cuurent_bal_for_old =  $get_account_bal_for_old->$mm;
           
           if($value["TType"] == "C"){
                $debit_pre_ledger_amt = $cuurent_bal_for_old + $value["Amount"];
                $this->db->where('PlantID', $selected_company);
                $this->db->LIKE('FY', $fy);
                $this->db->where('AccountID', $value["AccountID"]);
                $this->db->update(db_prefix() . 'accountbalances', [
                                            $mm => $debit_pre_ledger_amt,
                                        ]);
                
            }else{
                $credit_pre_ledger_amt = $cuurent_bal_for_old - $value["Amount"];
                $this->db->where('PlantID', $selected_company);
                $this->db->LIKE('FY', $fy);
                $this->db->where('AccountID', $value["AccountID"]);
                $this->db->update(db_prefix() . 'accountbalances', [
                                            $mm => $credit_pre_ledger_amt,
                                        ]);
            }
            $ledger_audit = array(
                    "PlantID"=>$value["PlantID"],
  ...
  public function get_account_details_by_AccountID($id)
    ...
    public function clientData($data){
        ...
        $selected_company = $this->session->userdata('root_company');
            ...
            $this->db->select(db_prefix() . 'clients.company,'.db_prefix() . 'clients.AccountID,'.db_prefix() . 'clients.address,'.db_prefix() . 'clients.Address3,'.db_prefix() . 'clients.zip,'.db_prefix() .'accountbalances.BAL1,'.db_prefix().'accountgroupssub.SubActGroupName,'.db_prefix().'xx_citylist.city_name,'.db_prefix().'xx_statelist.state_name');
    ...
    public function staffData($data){
        ...
        $selected_company = $this->session->userdata('root_company');
            ...
            $from_date_new = '20'.$fy.'-'.$m.'01';
            $regExp ='.*;s:[0-9]+:"'.$selected_company.'".*';
            $this->db->select(db_prefix() . 'staff.firstname,'.db_prefix() . 'staff.AccountID,'.db_prefix() . 'staff.firstname,'.db_prefix() . 'staff.lastname,'.db_prefix() . 'staff.current_address,'.db_prefix() . 'staff.pincode,'.db_prefix() . 'staff.home_town,'.db_prefix() .'accountbalances.BAL1,'.db_prefix().'accountgroupssub.SubActGroupName,'.db_prefix().'xx_citylist.city_name,'.db_prefix().'xx_statelist.state_name');
