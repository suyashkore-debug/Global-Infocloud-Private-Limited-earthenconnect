...
//end file
   public function headquarter_table_data($data){
    ...
    public function staff_table_dataNew()
    {
        $selected_company = $this->session->userdata('root_company');
        ...
        $this->db->select('tblstaff.*,a.firstname as reported_f_n,a.lastname as reported_l_n,' . db_prefix() . 'hr_job_position.position_name');
        $this->db->from(db_prefix() . 'staff');
    ...
    public function get_AccountDetails($AccountID)
    {
        ...
        $FY = $this->session->userdata('finacial_year');
            $regExp ='.*;s:[0-9]+:"'.$selected_company.'".*';
            $this->db->select(db_prefix() . 'staff.*,'.db_prefix() . 'accountsld.SLDTypeID,'.db_prefix() . 'accountbalances.BAL1,'.db_prefix() . 'staff_departments.departmentid');
            ...
            if($result){
                ...
                $HeadQurterList = $this->getHeadQurterList($result->state);
                $result->Type = 'staff';
                $result->CityList = $CityList;
                $result->HeadQurterList = $HeadQurterList;
            }else{
                ...
                $result = $this->db->get('staff')->row();
                if($result){
                    $result->Type = 'othercompstaff';
                }else{
                    $this->db->select('tblclients.AccountID,'.db_prefix() . 'accountbalances.BAL1');
                    $this->db->join(db_prefix() . 'accountbalances', db_prefix() . 'accountbalances.AccountID = '.db_prefix() . 'clients.AccountID AND tblaccountbalances.PlantID = '.$selected_company.' AND tblaccountbalances.FY = '.$FY,'LEFT');
    ...
    public function get_AccountDetailsPOPUP($AccountID)
    {
        $selected_company = $this->session->userdata('root_company');
        $FY = $this->session->userdata('finacial_year');
            $this->db->select(db_prefix() . 'staff.*,'.db_prefix() . 'accountsld.SLDTypeID,'.db_prefix() . 'accountbalances.BAL1,'.db_prefix() . 'staff_departments.departmentid');
   ...
   public function SaveAccountDetails($Data)
    {
        ...
        if($Data["SubActGroupID"] == "30001002"){
            $Department = 2;
        }else{
            $Department = 9;
        }
        ...
        /*$company_id1 = $Data["company_id1"];
        $company_id1Array = $company_id1;
        $staff_complen = count($company_id1Array);
        $staff_comp = serialize($company_id1Array);*/
        
        $StaffArray = array(
            
            'AccountID' =>strtoupper($Data["AccountID"]),
            'SubActGroupID' =>$Data["SubActGroupID"],
            'firstname' =>$Data["firstname"],
            ...
            'datecreated' =>to_sql_date($Data["datecreated"]),
        ...
        if($LastId){
            ...
            /*for($k=0; $k<$staff_complen; $k++) {
                $PlantID = $company_id1Array[$k];
                //return $PlantID;
                $balArray = array(
                    'PlantID' =>$PlantID,
                    'AccountID' =>strtoupper($AccountID),
    ...
    public function UpdateAccountDetails($Data)
    {
        ...
        $CheckSldtRecord = $this->ChkSldtRecord($AccountID,$selected_company,$FY);
        if($CheckSldtRecord){
            $sldtData = array(
        		'SLDTypeID'=>$Data["sldtype"],
        		'UserID2'=>$this->session->userdata('username'),
