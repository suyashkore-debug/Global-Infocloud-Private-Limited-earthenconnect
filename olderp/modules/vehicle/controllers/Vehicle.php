...
class Vehicle extends AdminController
{
    ...
    public function index()
    {
        ...
        $this->load->view('vehicle_dashboard', $data);
    }
    ...
    public function vehicle_manage()
    {
        ...
        $data['staff_members'] = $this->vehicle_model->get_staff('', ['active' => 1]);
        $data['title']                 = "Vehicle Manage";

        $data['dep_tree'] = json_encode($this->vehicle_model->get_department_tree());
        $data['staff_role'] = $this->roles_model->get();
        
        $this->load->view('vehicle_list/manage', $data);
    }

    public function vehicle_add($id = '')
    {
        {
        if ($this->input->post()) {
            if ($id == '') {
        ...
        $data['bodyclass']  = 'vehicle';
        $data['title']      = "Add New Vehicle";
        $this->load->view('vehicle_list/vehicle_add', $data);
    }
    ...
    public function route_manage($id = '')
    {
        $this->load->model('departments_model');
        $this->load->model('roles_model');
        if (!has_permission('hrm', '', 'view')) {
        ...
        $data['staff_members'] = $this->vehicle_model->get_staff('', ['active' => 1]);
        $data['title']                 = "Vehicle Route Manage";

        $data['dep_tree'] = json_encode($this->vehicle_model->get_department_tree());
        $data['staff_role'] = $this->roles_model->get();
        
        $this->load->view('vehicle_list/route_manage', $data);

    }

    public function route_challan()
    {
        $this->load->model('departments_model');
        $this->load->model('roles_model');
        if (!has_permission('hrm', '', 'view')) {
        ...
        $data['staff_members'] = $this->vehicle_model->get_staff('', ['active' => 1]);
        $data['title']                 = "Route Challan";

        $data['dep_tree'] = json_encode($this->vehicle_model->get_department_tree());
        $data['staff_role'] = $this->roles_model->get();
        
        $this->load->view('route_challan/route_challan', $data);
    }

    public function route_challan_add()
    {
        $this->load->model('departments_model');
        $this->load->model('roles_model');
        if (!has_permission('hrm', '', 'view')) {
        ...
        $data['staff_members'] = $this->vehicle_model->get_staff('', ['active' => 1]);
        $data['title']                 = "Create New Route Challan";

        $data['dep_tree'] = json_encode($this->vehicle_model->get_department_tree());
        $data['staff_role'] = $this->roles_model->get();
        
        $this->load->view('route_challan/route_challan_add', $data);
    }
    ...
    public function VehicleReturn()
    {
        ...
        $data['staff_members'] = $this->vehicle_model->get_staff('', ['active' => 1]);
        $data['title']                 = "Create Vehicle Return";

        $data['dep_tree'] = json_encode($this->vehicle_model->get_department_tree());
        $data['staff_role'] = $this->roles_model->get();
        
        $this->load->view('vehicle_return/add_vehicle_return', $data);
    }
    ...
    public function staff_infor()
    {
        ...
        $data['staff_members'] = $this->vehical_model->get_staff('', ['active' => 1]);
        $data['title']                 = _l('staff_infor');

        $data['dep_tree'] = json_encode($this->vehical_model->get_department_tree());
        $data['staff_role'] = $this->roles_model->get();
    ...
    {
        $this->app->get_table_data(module_views_path('vehicle', 'table_staff'));
    }
    ...
    public function setting()
    {
        ...
        $data['month'] = $this->vehical_model->get_month();
        $data['contract_type'] = $this->vehical_model->get_contracttype();
        $data['contract']  = $this->vehical_model->get_contracttype();
        $data['positions'] = $this->vehical_model->get_job_position();
        $data['workplace'] = $this->vehical_model->get_workplace();
        $data['allowance_type'] = $this->vehical_model->get_allowance_type();
        $data['salary_form'] = $this->vehical_model->get_salary_form();
        $data['insurance_type'] = $this->vehical_model->get_insurance_type();
        $data['province'] = $this->vehical_model->get_province();
    ...
    public function payroll()
    {
        ...
        $data['payrolls'] = $this->vehical_model->get_payroll_table();
        $data['month'] = $this->vehical_model->get_month();
        $data['payroll_type']   = $this->vehical_model->get_payroll_type();
    ...
    if ($this->input->post()) {
        ...
        if ($id == '') {
            $id = $this->vehical_model->add_payroll_type($data);
            if ($id) {
                $success = true;
            ...
            $success = $this->vehical_model->update_payroll_type($data, $id);
            if ($success) {
    ...
    $data['payroll_id'] = $id;
    $data['payrolls']   = $this->vehical_model->get_payroll_type($id);
    ...
    $data['str_allowance_type'] = $this->vehical_model->get_allowance_type();
    $data['str_salary_form'] = $this->vehical_model->get_salary_form();

    $data['roles']         = $this->roles_model->get();
    $data['departments'] = $this->departments_model->get();
    $data['staffs'] = $this->staff_model->get();
    $data['positions'] = $this->vehical_model->get_job_position();
    $data['salary_forms'] = $this->vehical_model->get_salary_form();
    ...
    if ($this->input->post()) {
        ...
        $data             = $this->input->post();
        if ($id == '') {
            $id = $this->vehical_model->add_payroll_table($data);
            if ($id) {
                $success = true;
    ...
    if($id != 0){
    $pt = $this->vehical_model->get_payroll_table($id);
    ...
    $data['payslip_name'] = $this->vehical_model->get_payroll_type($pt->payroll_type)->payroll_type_name;
    $data['column'] = json_encode($this->vehical_model->column_type($header_key));
...
public function latch_payslip(){
    ...
    $obj = array();
    $obj['status'] = 1;
    $success = $this->vehical_model->update_payroll_table_status($id,$obj);
    if ($success) {
 ...
 public function delete_payroll_type($id)
    {
        ...
        $response = $this->vehical_model->delete_payroll_type($id);
    ...
    public function member($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                $id = $this->vehical_model->add_staff($data);
                if ($id) {
                ...
                handle_staff_profile_image_upload($id);
                $response = $this->vehical_model->update_staff($data, $id);
        ...
        if ($id == '') {
            ...
            $data['insurances']            = $this->vehical_model->get_insurance_form_staffid($id);
            $data['insurance_history']            = $this->vehical_model->get_insurance_history_from_staffid($id);
            $data['month'] = $this->vehical_model->get_month();

            $data['hrm_staff']   = $this->vehical_model->get_hrm_attachments($id);
            $recordsreceived = $this->vehical_model->get_records_received($id);
            $payslip = $this->vehical_model->get_paysplip_bystafff($id);
            if(isset($payslip)){
                $data['paysplip_month'] = $payslip[0];
                $data['paysplip_header'] = $payslip[1];
            }
            $data['payroll_column'] = $this->vehical_model->column_type('', 1);
    ...
    public function delete_staff()
    {
        ...
        if (has_permission('staff', '', 'delete')) {
            $success = $this->vehical_model->delete_staff($this->input->post('id'), $this->input->post('transfer_data_to'));
            if ($success) {
    ...
    public function job_position($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if (!$this->input->post('id')) {
                $id = $this->vehical_model->add_job_position($data);
                if ($id) {
                    $success = true;
                ...
                $id = $data['id'];
                unset($data['id']);
                $success = $this->vehical_model->update_job_position($data, $id);
                if ($success) {
    ...
    public function delete_job_position($id)
    {
        ...
        $response = $this->vehical_model->delete_job_position($id);
        if (is_array($response) && isset($response['referenced'])) {
    ...
    public function department($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if (!$this->input->post('id')) {
                $id = $this->vehical_model->add_department($data);
                if ($id) {
                    $success = true;
                ...
                $id = $data['id'];
                unset($data['id']);
                $success = $this->vehical_model->update_department($data, $id);
                if ($success) {
    ...
    public function delete_department($id)
    {
        ...
        $response = $this->vehical_model->delete_department($id);
        if (is_array($response) && isset($response['referenced'])) {
    ...
    public function workplace($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if (!$this->input->post('id')) {
                $id = $this->vehical_model->add_workplace($data);
                if ($id) {
                    $success = true;
                ...
                $id = $data['id'];
                unset($data['id']);
                $success = $this->vehical_model->update_workplace($data, $id);
                if ($success) {
    ...
    public function delete_workplace($id)
    {
        ...
        $response = $this->vehical_model->delete_workplace($id);
        if (is_array($response) && isset($response['referenced'])) {
    ...
    public function contract_type($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if (!$this->input->post('id')) {
                $id = $this->vehical_model->add_contract_type($data);
                if ($id) {
                    $success = true;
                ...
                $id = $data['id'];
                unset($data['id']);
                $success = $this->vehical_model->update_contract_type($data, $id);
                if ($success) {
    ...
    public function delete_contract_type($id)
    {
        ...
        $response = $this->vehical_model->delete_contract_type($id);
        if (is_array($response) && isset($response['referenced'])) {
    ...
    public function allowance_type($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if (!$this->input->post('id')) {
                $id = $this->vehical_model->add_allowance_type($data);
                if ($id) {
                    $success = true;
                ...
                $id = $data['id'];
                unset($data['id']);
                $success = $this->vehical_model->update_allowance_type($data, $id);
                if ($success) {
    ...
    public function delete_allowance_type($id)
    {
        ...
        $response = $this->vehical_model->delete_allowance_type($id);
        if (is_array($response) && isset($response['referenced'])) {
    ...
    public function salary_form($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if (!$this->input->post('id')) {
                $id = $this->vehical_model->add_salary_form($data);
                if ($id) {
                    $success = true;
               ...
               redirect(admin_url('hrm/setting?group=payroll'));
                ...
                $id = $data['id'];
                unset($data['id']);
                $success = $this->vehical_model->update_salary_form($data, $id);
                if ($success) {
    ...
    public function delete_salary_form($id)
    {
        ...
        $response = $this->vehical_model->delete_salary_form($id);
        if (is_array($response) && isset($response['referenced'])) {
    ...
    {
        $this->app->get_table_data(module_views_path('vehicle', 'vehicle_list/challan_list_html'));
    }
    public function table_route()
    {
        $this->app->get_table_data(module_views_path('vehicle', 'vehicle_list/table_route_html'));
    }
    public function table_vehicle()
    {
        $this->app->get_table_data(module_views_path('vehicle', 'vehicle_list/table_vehicle_html'));
    }
    public function table_challan()
    {
        $this->app->get_table_data(module_views_path('vehicle', 'route_challan/challan_list_html'));
    ...
    public function contracts($id = '')
    {
        ...
        $data['allowance_type'] = $this->vehical_model->get_allowance_type();
        $data['salary_form'] = $this->vehical_model->get_salary_form();
        $data['duration'] = $this->vehical_model->get_duration();
    ...
    public function contract($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                $id = $this->vehical_model->add_contract($data);
                if ($id) {
                ...
                $response = $this->vehical_model->update_contract($data, $id);
                if (is_array($response)) {
                    if (isset($response['cant_remove_main_admin'])) {
                        set_alert('warning', _l('staff_cant_remove_main_admin'));
                    } elseif (isset($response['cant_remove_yourself_from_admin'])) {
        ...
        if ($id == '') {
            ...
            $contract = $this->vehical_model->get_contract($id);
            $contract_detail = $this->vehical_model->get_contract_detail($id);
            if (!$contract) {
            ...
            $data['staff_delegate_role'] = $this->vehical_model->get_staff_role($contract[0]['staff_delegate']);
            }
            $data['contract_details']            = $contract_detail;
            if(isset($contract[0]['name_contract'])){

            $title                     = $this->vehical_model->get_contracttype_by_id($contract[0]['name_contract']);
    ...
    public function delete_contract($id)
    {
        ...
        $response = $this->vehical_model->delete_contract($id);
        redirect(admin_url('hrm/contracts'));
        if (is_array($response) && isset($response['referenced'])) {
    ...
    public function contract_form(){
        if($this->input->post('contract_form')){
            $this->vehical_model->contract_form($this->input->post('contract_form'));
            $success = true;
    ...
    if ($this->input->post()) {
        ...
        if ($files) {
            ...
            foreach ($files as $file) {
               $insert_id = $this->vehical_model->add_attachment_to_database($staffid, 'hrm_staff_file', [$file], false);
               if($insert_id > 0){
                $count_id ++ ;
               }
                $i++;
        ...
        $hrm_staff   = $this->vehical_model->get_hrm_attachments($staffid);
        $data ='';
 ...
 public function hrm_file($id, $rel_id)
    {
        ...
        $data['file'] = $this->vehical_model->get_file($id, $rel_id);
        if (!$data['file']) {
...
public function delete_hrm_staff_attachment($attachment_id)
    {
        ...
        if ($file->staffid == get_staff_user_id() || is_admin()) {
            echo json_encode([
                'success' => $this->vehical_model->delete_hrm_staff_attachment($attachment_id),
            ]);
        }
    }
    ...
    public function get_hrm_contract_data_ajax($id)
    {
        ...
        $this->load->view('vehicle/contract_preview_template', $data);
    }

    public function insurance_conditions_setting(){
        if($this->input->post()){
            $data = $this->input->post();
            $success = $this->vehical_model->update_insurance_conditions($data);
            if($success > 0){
     ...
     public function insurance($id = ''){
        ...
        if ($this->input->post()) {
            ...
            if ($this->input->post('insurance_id') == '') {
                ...
                $id = $this->vehical_model->add_insurance($data);
                if ($id) {
                ...
                $response = $this->vehical_model->update_insurance($data, $this->input->post('insurance_id'));
                if (is_array($response)) {
                    if (isset($response['cant_remove_main_admin'])) {
                        set_alert('warning', _l('staff_cant_remove_main_admin'));
                    } elseif (isset($response['cant_remove_yourself_from_admin'])) {
        ...
        if ($id == '') {
            ...
            $insurance = $this->vehical_model->get_insurance($id);
            $insurance_history = $this->vehical_model->get_insurance_history($id);
    ...
    public function insurance_type(){
        if($this->input->post()){
            $data = $this->input->post();
            if (!$this->input->post('id')) {
                $add = $this->vehical_model->add_insurance_type($data); 
                if($add){
                ...
                $id = $data['id'];
                unset($data['id']);
                $success = $this->vehical_model->update_insurance_type($data,$id);
                if($success == true){
    ...
    public function delete_insurance_type($id){
        ...
        $response = $this->vehical_model->delete_insurance_type($id);
        if (is_array($response) && isset($response['referenced'])) {
    ...
    public function get_hrm_staff(){
        if ($this->input->is_ajax_request()) {
            ...
            $total_rows = $this->db->query('select si.insurance_id from '.db_prefix().'staff_insurance as si where si.staff_id = '.$staffid)->result_array();
                ...
                $insurance_history = $this->vehical_model->get_insurance_history($id);
                $month = $this->vehical_model->get_month();
    ...
    public function timekeeping(){
        ...
        $data['departments'] = $this->departments_model->get();
        $data['positions'] = $this->vehical_model->get_job_position();
        $data['holiday'] = $this->vehical_model->get_break_dates('holiday');
        $data['event_break'] = $this->vehical_model->get_break_dates('event_break');
        $data['unexpected_break'] = $this->vehical_model->get_break_dates('unexpected_break');
        $data['shifts'] = $this->vehical_model->get_shifts();
        ...
        $data_ts = $this->vehical_model->get_hrm_ts_by_month(date('m'));

        if(isset($data['shifts'][0])){
            $work_shift = $data['shifts'][0];
            $work_shift['shift_s'] = $this->vehical_model->get_data_edit_shift($work_shift['id']);
        }
        ...
        foreach($data_ts as $ts){
            ...
            $ts_type = $this->vehical_model->get_ts_by_date_and_staff($ts['date_work'],$ts['staff_id']);
            if(count($ts_type) <= 1){
    ...
    public function day_off(){
        if($this->input->post()){
            $data = $this->input->post();
            if (!$this->input->post('id')) {
                $add = $this->vehical_model->add_day_off($data); 
                if($add > 0){
                ...
                $id = $data['id'];
                unset($data['id']);
                $success = $this->vehical_model->update_day_off($data,$id);
                if($success == true){
    ...
    public function delete_day_off($id){
        ...
        $response = $this->vehical_model->delete_day_off($id);
        if (is_array($response) && isset($response['referenced'])) {
    ...
    public function shifts(){
        if($this->input->post()){
            $data = $this->input->post();
            if (!$this->input->post('id')) {
                $add = $this->vehical_model->add_work_shift($data); 
                if($add > 0){
                ...
                $id = $data['id'];
                unset($data['id']);
                $success = $this->vehical_model->update_work_shift($data,$id);
                if($success == true){
    ...
    public function get_data_edit_shift($id){
        $shift_handson = $this->vehical_model->get_data_edit_shift($id);
        $result = [];
        $node = [];
    ...
    public function delete_shift($id){
        ...
        $response = $this->vehical_model->delete_shift($id);
        if (is_array($response) && isset($response['referenced'])) {
    ...
    public function delete_payroll_table($id){
        ...
        $response = $this->vehical_model->delete_payroll_table($id);
        if (is_array($response) && isset($response['referenced'])) {
