...
function add_item_to_preview1(e,l,t,s) {
    requestGetJSON("invoice_items/get_item_by_id/" + e + "/" + t + "/" + s).done(function(e) {
        clear_item_preview_values1(),  $('.main input[name="item_code"]').val(e.item_code), $('.main input[name="hsn_code"]').val(e.hsn_code),$('.main input[name="autouser"]').val(e.description), $('.main input[name="dis"]').val(0), $('.main input[name="dis_amt"]').val(0), $('.main input[name="taxrate1"]').val(e.taxrate), $('.main textarea[name="description"]').val(e.description), $('.main input[name="quantity"]').val("");
        var t = [];
        e.taxname && e.taxrate && t.push(e.taxname + "|" + e.taxrate), e.taxname_2 && e.taxrate_2 && t.push(e.taxname_2 + "|" + e.taxrate_2), $(".main select.tax").selectpicker("val", t), $('.main input[name="unit"]').val(e.unit);
        var a = $("body").find('.accounting-template select[name="currency"]'),
            i = a.attr("data-base"),
...
function calculate_total() {
    ...
    $(".tax-area").remove(), $.each(p, function() {
        if($(this).find("[data-quantity]").val() <=0){
          ...
          "" === (t = $(this).find("[data-quantity]").val()) && (t = 1, $(this).find("[data-quantity]").val(1)), s = accounting.toFixed($(this).find("td.rate input").val() * t * $(this).find("td.pack_qty input").val() , app.options.decimal_places), cs_cr = $(this).find("td.order_qty input").val() * $(this).find("td.pack_qty input").val(),  cs_cr_new = cs_cr / $(this).find("td.pack_qty input").val(), cs_cr_value = $(this).find("td.cs_cr input").val(), n = $(this), tax = s / 100 * $(this).find("td.taxrate input").val(), s = parseFloat(s), ss = s, s += tax, $(this).find("td.amount").html(format_money(s, !0)), d += ss, (e = $(this).find("select.tax").selectpicker("val")) && $.each(e, function(e, t) {
            
            if(cs_cr_value == "CS"){ total_cases += cs_cr_new } else { total_crates += cs_cr_new }
            
            i = n.find('select.tax [value="' + t + '"]').data("taxrate"), a = ss / 100 * i, l.hasOwnProperty(t) ? l[t] = l[t] += a : 0 != i && (o = t.split("|"),/* tax_row = '<tr class="tax-area"><td>' + o[0] + "(" + i + '%)</td><td id="tax_id_' + slugify(t) + '"></td></tr>', $(_).after(tax_row), l[t] = a,*/ tgst +=a)
        })  
        }
    ...
    $('input[name="discount_total"]').val(accounting.toFixed(c, app.options.decimal_places)),$('input[name="total_cases"]').val(total_cases),$('input[name="total_crates"]').val(total_crates), $('input[name="total_tax"]').val(tgst), $(".discount-total").html(b), $(".adjustment").html(format_money(m)), $(".igsttotal").html(format_money(igst)), $(".sgsttotal").html(format_money(sgst)), $(".cgsttotal").html(format_money(cgst)), $(".cases").html(total_cases), $(".crates").html(total_crates), $(".subtotal").html(format_money(d) + hidden_input("subtotal", accounting.toFixed(d, app.options.decimal_places))), $(".tcstotal").html(format_money(Y) + hidden_input("tcstotal", accounting.toFixed(Y, app.options.decimal_places))), $(".total").html(format_money(rr) + hidden_input("total", accounting.toFixed(rr, app.options.decimal_places))), $(document).trigger("sales-total-calculated")
}
...
$("body").on("loaded.bs.select change", "select.ajax-search", function(e) {
    ...
    $("body").on("submit", "._transaction_form", function() {
        ...
        requestGetJSON("order/client_change_data/" + e + "/" + (a = void 0 === a ? "" : a)).done(function(e) {
            ...
            //$('select[name="item_select"]').selectpicker("val", e.item_data);
